...
## 分享优秀的 App Clip
在去年的 Session [Configure and link your App Clips](https://developer.apple.com/videos/play/wwdc2020/10146/)，App Clip的各种唤醒方式是极其惊艳的。包括但不限于 Safari、iMessage、Map、Spotlight search、Siri suggestion widget、NFC、QR Code、App Clip code。自发布以来，我们看到了来自世界各地开发人员的 App Clip，接下来让我为你展示这些例子。
...
### App Clip Code 的样式
苹果支持两种样式的 App Clip Code， 内置 NFC 功能的和仅用于扫描的。
内置 NFC 功能的 Code，中心使用了 iPhone 的 icon，引导用户将手机贴近 Code 来读取它的内容，或者是通过相机或控制中心的的二维码扫描器来扫描这类内置 NFC 的二维码。进用于扫描的 Code，中心使用了相机的 icon，让用户知道可以使用相机和控制中心的二维码扫描器来扫描。

我们可以看出来 App Clip Code 的特征是很鲜明的，与传统的二维码样式差别较大。如同微信小程序码一样，一眼就可以让用户知道这个二维码包含的内容源于哪里。

### 自定义 App Clip Code 的样式
开发者可以自定义 App Clip Code 的样式来满足特定的使用场景。
- 可以选择专属的前景色和背景色。
- 可以隐藏 App Clip logo。但是苹果建议如果有足够的空间的话，显示 App Clip logo。这会给用户一个明确的信息，表明这个 Code 是属于 App Clip。
...
打开我们的终端，输入 `AppClipCodeGenerator templates`，就可以浏览所有可用的模板。
那么这些模板都长什么样子呢？我们可以使用该路径 `/Library/Developer/AppClipCodeGenerator/SampleTemplates` 预览所有模板的样式。

Template_ID_16 看起来不错，所以我准备使用这个模板来生成 WWDC20 内参小专栏的 App Clip Code。在终端输入
```
AppClipCodeGenerator generate --type nfc --url https://xiaozhuanlan.com/wwdc20 --index 16 --output wwdc20.svg
```
- --type 后的参数可以填写 cam 或者 nfc，来选择性生成只支持扫描或内置 NFC 的轻应用码。默认为 cam。
- --url 后的参数填写绑定到轻应用码的链接。
- --index 后的参数可填写 0 - 17 的数字，来选择 Apple 已有的模板生成轻应用码。
- --output 后的参数表明轻应用码的名字和格式。Apple 建议我们使用 SVG 格式，SVG是一种矢量图，在放大后仍会保持清晰。

只需一行命令，我们的轻应用码就生成成功了，是不是非常简单？
...
> Tips：使用相机和控制中心的二维码扫描器这个轻应用码，会展示相关的动画。但是因为该 Code 没有关联上架的 App Clip，所以并不会触发完整的体验流程。
