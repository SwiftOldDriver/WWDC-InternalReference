...
本文主要是讲了苹果在 Nearby Interaction 框架上的最新动作，全文主要分为三部分：
...
在 iOS 15 上对权限进行了改进，弹窗如下图。这是 iOS 15 中新的“附近交互”权限提示。看起来很相似，但是却和之前的权限授予流程有以下不同。
...
上面的回顾中我们在了两部 iPhone 之间启动会话时创建了 NearbyPeerConfiguration。如果要要开始与硬件的会话，需要创建 NearbyAccessoryConfiguration。这是 iOS 15 中新的 NIConfiguration 类型。如果要实例化一个 NearbyAccessoryConfiguration。需要为它提供一些配置数据，系统接受的一种特定的用来描述这个设备的数据格式。但是我们如何获得这个配置数据，这个特定的格式是什么？与 U1 兼容的硬件设备（认证过的供应商）将知道如何根据请求生成此配置数据。这意味着您在硬件设备本身上运行的代码需要生成此数据然后通过 part1 的数据通道将其发送到您的应用程序。代码如下：
```Swift
...
### 总结
Nearby Interaction 在 iOS 14 上还是只能是手机与手机进行交互，在 iOS 15 上扩展到了手机与第三方的硬件。不断增强了 Nearby Interaction 框架的能力。无论是在智能家具还是 AR/MR，室内定位，钱包，地铁检票应该都会有广泛的应用。