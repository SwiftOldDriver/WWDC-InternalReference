...
`actor` 的出现，让我们得以用一种新的方式来进行异步与并发编程，同时不需要关心数据竞争的问题。
...
并发模型中，除了 `Actor` 模型，还有 `CSP` 模型。

`Actor` 模型是 `actor` 之间直接进行通信，不需要借助中介，消息是异步发送和处理的。消息的发送方是异步的，无论接收方是否将消息读取出来，发送方都不会阻塞，而是通过邮箱进行消息的处理。正因如此，邮箱在理论上是无限大的。

`CSP` 模型则是 `worker` 之间通信需要借助一个 `Channel`，消息被发送发写入通道直到被接收方读取之前都是阻塞的。这就意味着一个通道只需要保存一条消息。

如需了解更多，推荐阅读 [并发模型：Actors 与 CSP](https://jingwei.link/2018/07/08/actor-and-csp-model.html)。

### 旧版本系统是否可以兼容

虽然我们心心念念的 Concurrency 终于来了，但是 iOS 15 的版本要求却是望而却步。究其原因，是因为 `await` 会中断当前的函数调用，但是调用栈并没有丢失，可以在以后被恢复。要做到这一点，就必须使用新的 Swift 运行时，这就导致了无法在低版本直接支持。

如需了解更多，推荐阅读 [Await：从 Swift 到 C++](https://techsingular.net/2021/06/16/await%ef%bc%9a%e4%bb%8e-swift-%e5%88%b0-c/)。
