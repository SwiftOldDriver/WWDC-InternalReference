...
当然，也不是所有 app 的所有功能都适合提供 action。评判标准在于功能是否足够简单，比如好呈现、好操作、所需要的参数或者上下文少。拿 Apple Watch app 举例，自定义表盘需要很多参数，也需要呈现一个 UI 给用户预览，这就不适合作为 action。而切换已有的表盘，这个功能很简单，就可以作为 action。
