...
当然现在还可以传入任意的参数到 ``custom`` 中。方便追溯业务订单，暂时没有大小的限制，可以把业务现存的预订单号进行传入。  
```swift
...
现在可以通过新的 ``all`` 变量，在 ``Transaction`` 上获取所有的交易信息，进而可以在 APP 启动后调用，实现发货检查 / 自动补发的效果。当然需要自行判断商品是否退款等信息，做出对应的惩罚。是取消订阅服务、收回游戏道具，还是扣除虚拟货币都，就都在开发者的一念之间了。这自然也就让多年以来，开发者流血、消费者占便宜，黑产借机挣钱的局面扭转了过来。
而至于 “小孩误操作”、“游戏更新后闪退严重导致无法体验”、“网络不佳导致重复购买”这样的退款话术，或许也要随着苹果在退款规则方面的不断严密，而成为回忆了。  
当然我们也可以借此检查哪些订单被退款了，因而来决定
另外一个较为有用的是 ``currentEntitlements``，可以看现在生效的，包括在订阅期内的，买过一次就永久生效的非消耗型商品 (Non-consumable products）。如果已经退款的，或者已经过期的，则不会返回。  
```swift
extension Transaction {
