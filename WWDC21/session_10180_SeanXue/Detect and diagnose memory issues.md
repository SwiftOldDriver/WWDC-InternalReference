...
系统的内存是有限的，如果你的 app 切换到后台却占用大量的内存空间时，很有可能被系统终止你的 app 的运行来回收内存空间。所有**我们应该在 App 进入后台时释放内存占用较大的资源，进入前台时重新加载**。因为资源加载到内存是需要时间的，所以保持内存占用的紧凑，可以有效提高你的 app 保留在内存的几率，这样你就能够获得更快的应用程序激活。
...
当用户浏览你的新功能时，他们想要更快速的响应。而减少内存占用就可以让你的 app 获得更快的响应。战略性地考虑 app 已经加载到内存的内容，可以有效地减少用户在和你的 app 交互时,系统对内存的回收。
...
苹果提供了一个名为 Meal Planner 的 app 来测量内存的使用情况。(这个 app 可以帮助安排我们一周的饮食😂)  当点击保存按钮时，会下载对应的食谱到用户的设备上。case 如下：
...
下载并且解压后，我们会发现如下两个内存图，这是因为苹果给我们增加了一次额外的迭代来让我们测试启用malloc堆栈的日志记录，苹果收集了最初的内存图并在名称前面添加了pre，并收集了最后一次迭代的内存图，并在名称前添加了post。这些可以让我们在需要时分析单个迭代期间的内存增长。
...
下面让我们进入正题，苹果在这个 session 将讲解两种内存问题，Leaks 和 Heap size issues, 注意上图列出的并不是一个详尽的内存问题列表，但是涵盖了大多数常见的内存问题，这里也会使用到一些命令行工具来诊断这些内存问题，如果想要了解更多的关于命令行工具的总结，请参阅 [WWDC18 iOS Memory Deep Dive](https://developer.apple.com/videos/play/wwdc2018/416) 。
...
注意A和B之间的虚线，这表示此时我们把A对B的引用置为nil，并且移除A上的B对象。
