...
本 session 讲解了如何使用 Xcode 理解和诊断内存性能问题。带你了解 Xcode 工具的最新更新，探索 Metrics，查看 XCTest 中的 memgraph 集合，并学习如何使用 Performance XCTest 捕获回归。



[TOC]

### Impact of memory footprint(内存占用的影响)

为什么我们需要关注应用的内存占用(Memory footprint)？

**为了更好的用户体验。**

[^Memory footprint]: 这里的Memory Footprint指的是虚拟内存：Virtual Memory。
...
#### 1.Faster application activations(更快的应用程序激活)

系统的内存是有限的，如果你的 app 切换到后台却占用大量的内存空间时，很有可能被系统终止你的 app 的运行来回收内存空间。所有**我们应该在 App 进入后台时释放内存占用较大的资源，进入前台时重新加载**。因为资源加载到内存是需要时间的，所以保持内存占用的紧凑，可以有效提高你的 app 保留在内存几率，这样你就能够获得更快的应用程序激活。
...
#### 2.Responsive experience(快速响应的经验)

当你的用户浏览你的新功能时，他们想要更快速的响应。而减少内存占用就可以让你的 app 获得更快的响应。战略性地考虑你的 app 已经加载到内存的内容，可以有效地减少用户在和你的 app 交互时,系统对内存的回收。

#### 3.Complex  features(复杂的功能)

对内存使用采取策略也为更广泛的功能打开了大门，你可以为你的 app 增加很多功能，比如加载视频，做动画等等。

#### 4.Wider device compatibility(广泛的设备的兼容性)

最后，Apple 的设备会随着时间不断发展，新设备拥有比以前更多的物理内存。通过减少内存占用，你的应用将与旧设备上的表现一样，从而增加能够欣赏你的应用的用户。

##### 总结

通过监控你 app 的内存占用，你的 app 将获得更快的应用程序激活，更快速的响应，处理更复杂的功能，能在更多的设备上运行。

### Tools for profiling memory(分析内存的一些工具)

#### 内存占用的组成

在了解内存分析工具之前，让我们先来复习一些内存的基础知识。让我们看看是什么组成了内存占用(memory footprint)。我们用三个类别来对你的 app 的内存占用进行分类。脏内存(Dirty memory)，压缩内存(Compressed memory), 干净的内存(Clean memory)。让我们快速的看一下每一项都包含什么。
...
Dirty memory 是 app 写入的内存，包含如下：

1. 它包括所有的 heap allocations：当你用 malloc时，申请的就是堆上的存储空间。
2. 图像解码的buffer。
3. 以及 frameworks 中的 `__DATA` 和 `__DATA_DIRTY` 部分也同样存储在堆上。
...
苹果最初只是公开了从 OS X Mavericks 开始使用 Compressed memory 技术，但 iOS 系统也从 iOS 7 开始悄悄地使用。从 [OSX_Mavericks_Core_Technology_Overview](https://images.apple.com/media/us/osx/2013/docs/OSX_Mavericks_Core_Technology_Overview.pdf) 文档中可以了解到该技术在内存紧张时能够将最近使用过的内存占用压缩至原有大小的一半以下，并且能够在需要时解压复用。它在节省内存的同时提高了系统的响应速度，其特点可以归结为：
...
Compressed memory 将 Dirty memory 中最近没有访问过得被内存压缩器压缩的 Dirty page。这些 page 会在被访问时解压缩。注意 iOS 是没有交换内存(Disk swap)技术的，交换内存是 MacOS 特有的。
...
> Tis：iOS在内存紧张的时候会使用到内存压缩技术，而MacOS在内存紧张的时候会使用到内存压缩技术及磁盘交换技术
...
Clean Memory 是还没有被写入的内存或可以被 page out 的内存。列如内存映射文件：加载到内存中的磁盘上的图像，也可以是 frameworks 。

> page out是虚拟内存从物理内存中拷贝到硬盘，page in是反过来。只有在物理内存不够用的时候才会出现page in/out（需要使用的页面page in，空闲的页面page out）。因为 iOS 没有 Disk swap 技术，所有对应的page in/out 是将硬盘的相关技术替换成解压缩技术。
...
现在让我们来看看你可以使用到分析内存占用的一些工具。Xcode 提供了一套工具来协助我们监控开发阶段和线上的 app 内存性能。
...
XCTest 框架帮助我们直接在项目的单元测试和 UI 测试中监控 app 的内存占用， MetricKit 和 Xcode Organize帮助我们自定义的监控生产环境上的内存指标。
...
为了继续这个演讲，我们将使用 XCTests 做性能测试，但是注意这些技术依旧可以应用在一般内存问题分类和调查中。 使用 XCTests 做性能测试, 你能测量系统资源，比如：内存利用率, CPU使用率，磁盘写入等等。 
...
苹果提供了一个名为 Meal Planner 的 app 来测量内存的使用情况。(这个 app 可以帮忙安排我们一周的饮食😂)  当点击保存按钮时，会下载对应的食谱到用户的设备上。
...
```
 ...
 `measure(metrics:options:block:)`需要指定对应的 app，在 `block` 中 启动 app， 调用 `startMeasuring` 开始测量，点击 `Save meal` 按钮， 使用 `waitForExistence` 等待下载食谱完成，并检查 UI 是否更新。执行测试代码之后，点击测试case旁边的菱形，弹出测量面板, 选择物理内存选项。
...
我们可以看到case执行了五次，内存均值在 116000 KB, Set Baseline 文字下方可以看到 case 每次运行的的详细数据，一般情况下我们会参考平均值设置我们的 baseline，设置完 baseline 后再次运行，可以在 Result 看到回归。
...
苹果在Xcode 13增加了收集诊断数据的新功能，来帮忙分类测试回归。`Ktrace files`  和 `Memory graphs`。
...
`Ktrace files` 即强大又灵活。它可以用于一般的系统调查也能聚焦于一些特殊的问题，比如可以深入渲染管线调查 `hitches` 问题，或者查找阻塞主线程的原因,挂起率(Hang rate)。 在日常工作中，这些 `Ktrace files` 可以用  instruments 打开并分析。
...
`Memory graphs` 对于特定的问题查询很有用，`Memory graphs` 即可以在 Xcode 的可视化调试工具中使用，也可以作为多个命令行工具使用。其中一些我们将会在后面讨论。 `Memory graphs` 本质上是一份进程地址空间的快照，`Memory graphs` 记录了每一个虚拟内存 region 的地址和大小和每一个分配地址的 block 。以及这些 region 和 blocks 的指向。这些足以支撑你去检查每一个堆上对象。查看与链接框架关联的数据区域等等。
...
![image-20210614233814132](https://www.yuque.com/api/filetransfer/images?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2021%2Fpng%2F21867895%2F1623935416378-13c1b2b8-4600-445a-ad6a-54e001a6ed42.png&sign=1e468a9a47e512090e2bed35fd9785705c6aa823f240fe9c01722bcd465861ba)
...
下载并且解压后，我们会发现如下两个内存图，这是因为苹果给我们添加了一次额外的迭代来让我们测试启用malloc堆栈的日志记录，苹果收集了最初的内存图并在名称前面添加了pre，并收集了最后一次迭代的内存图，并在名称前添加了post。这些可以让我们在需要时分析单个迭代期间的内存增长。
...
有了`Ktrace files`  和 `Memory graphs` 并且设置了 malloc stack 为 YES，你不仅可以知道回归([regression](https://zh.wikipedia.org/wiki/%E8%BF%B4%E6%AD%B8%E5%88%86%E6%9E%90))出现了问题，还会知道为什么回归([regression](https://zh.wikipedia.org/wiki/%E8%BF%B4%E6%AD%B8%E5%88%86%E6%9E%90))会出现问题。

### Types of memory issues(内存问题的一些分类)
...
![image-20210615004650524](https://www.yuque.com/api/filetransfer/images?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2021%2Fpng%2F21867895%2F1623935419335-17683bd9-9fec-4745-8245-f9040dae1d57.png&sign=8439e7f2d0f35773e950c2aef0cf49009e772ddf1503a03b14a84fbb73614f9d)

下面让我们进入正题，苹果在这个session将讲解两种内存问题，Leaks 和 Heap size issues, 注意上图并不是一个详尽的内存问题列表，但是涵盖了大多数常见的内存问题，这里也会使用到一些命令行工具来诊断这些内存问题，如果想要了解更多的关于命令行工具的总结，请参阅 [WWDC18 iOS Memory Deep Dive](https://developer.apple.com/videos/play/wwdc2018/416) 。
...
当进程创建了对象,在失去了所有指向该对象的指针的时候，并没有回收该对象。我们称这种情况为泄露(Leak)。

我们用灰色的箭头表示对象之间的引用，注意，每个对象都有至少有一个引用。
...
主要A和B之间的虚线，这表示此时我们把A对B的引用置为nil，并且移除A上的B对象。
...
> 这种情况的泄露，只会发生在MRC下，在ARC下当移除指针时，我们一般就会认为此对象已经被释放。
>
...
循环引用也会引起泄露。Swift中的最常见对象泄露就是循环引用引起的。在上图中，对象 A 和 B 就是循环引用。它们相互引用，但没有外部引用。这意味着进程不能访问或者释放他们。所以它们被认定为泄露。
...
leaks 命令可以帮查找已经产生的泄露，输出展示了我们有 4 个泄露，一共 240 byte。
...
拖下来，输出包含了每一个泄露的详细的情况，这些信息可以给我们一些线索，帮助我们找到是什么引起了泄露。最上面的对象图指出 ROOT CYCLE,这表示我们面对的是循环引用。
...
这个函数就是我们在leaks的输出中看到的，这里我分别创建了 MealPlan 对象和 MealItem 对象，日志里面说这两个对象有循环引用。
...
那么 addMealToMealPlan 这个函数看起来就有点可疑，让我们看一下。
...
我们应该寻找一种方案来解决这个问题，这里使用了最快的解决方式，我将改变的 MenuItem 对 MealPlan 为弱引用。这就打破了引用循环。因为这里已经不存在强引用回环了。
...
我们再次回到 Meal Planner 执行失败的 XCtest,检查一下堆的回归。
...
这里我们会对 pre 和 post memegraph 文件使用 vmmap，这样可以得到一个内存使用的概览。
...
输出显示 MALLOC_LARGE 块大概持有 13 MB 的 dirty memory。这大概就等于我们回归的大小。我们需要看一下，是谁贡献了这 13 MB。
...
最上面显示了这些 diff 大概有 13MB，下面是按照类名来进行划分的。每一种对象都会有个数和大小的总结。这里我们可以看到我们大概有 13 MB 的 `non-object` 类型，在 Swift 中，这种类型通常是 raw 分配的bytes,这种对象可以用一点小技巧去追查。要追查，首要要拿到这些 `non-object` 的地址。
...
使用如上 `heap -addresses` 命令来抓住他们，我特别注明我只要 `non-object` 类型，并且大小至少 500 kb。如输出所示，`0x11380000` 地址的 `non-object` 大概有13 MB。所以他就是这次调查的主要嫌疑人。记录一下这个地址来找一些线索，在这个节点，我们有几个选择。根据具体情况，每种方法都有其好处,我将简要地逐一介绍。
...
```
  ...
  This gives me a tree of objects the reference this address. This is useful if I have a specific object I want to get more information about and my memgraph doesn't have malloc stack logging or MSL enabled.

这个命令可以得到这个地址的对象引用树，这个对查找特殊对象更多信息的时候很有用，特别是在 memgraph 没有打开 malloc stack logging 或者没有启用 MSL的时候 。注意 XCTest memgraph 会自动启用 MSL。
...
我们可以传递 `--groupByType` 参数来把相同类型做一个聚类，来让输出更简洁，更容易看懂。large trunk regression通常会在树中分组到一个节点下, 这让我们可以更容易发现那块内存是什么。注意上图高亮部分，同样是 mealData 大概有 13 MB。
...
因为memgraph 启用了 MSL，所以可以使用 `malloc_hisatory -fullStacks` 来弄清楚这个对象怎么被创建的。
...
找到罪魁祸首了，函数在一个自定义的 cell 的 view  里，这里创建了 raw buffer 并包装成了 mealData 对象。为了填充数据和保存数据到磁盘，这里创建了这个 buffer。一旦保存到磁盘，我们就不再需要这个 buffer 了。所以不应该一直用 mealData 属性保存着这个数据，因为只要这个 view 实例存在，这个 buffer 就会一直存在。这意味着，当我点击任意 cell 上的 saveMeal 按钮，这个 cell 就会创建并持有一个很大的 buffer，直到这个 cell 被销毁。当我点击多个 cell 上的保存按钮时，内存加起来就会很大。所以我们该如何解决？
...
page 是系统授予进程的固定大小、不可分割的最小内存块。因为 page 是不可分割的，当进程写入page 的任意部分，整个 page 都会被认为是 dirty 的并且进程将会管理它，即使 page 的大部分没有被使用到。

当进程的 dirty page 没有被 100% 占用时，就会产生碎片化。为了理解为什么出现碎片，我们来看一个例子：

 ![image-20210616215810552](https://www.yuque.com/api/filetransfer/images?url=https%3A%2F%2Fcdn.nlark.com%2Fyuque%2F0%2F2021%2Fpng%2F21867895%2F1623935433747-c2e1deee-40e9-4fc6-b0b7-e19f1b791273.png&sign=35dcbb3c6b38c8a35e407ea510ea5b58e69f6c1573ae432d83de946a4fc996b8)
...
我手动创建了对象都没标记为 my object,我打算同时释放它们，我没有太关注我的代码，系统最终交错安插了我的对象和其他对象，
...
当我释放掉所有的 my object，进程就会空出两个 clean page 给系统。结果会得到两个 clean page 两个 dirty page 以及0%的碎片化。

注意碎片化是怎样成为占用空间的倍增器。50%的碎片化就会让我们的内存占用翻倍。从两个 dirty page 变成 4 个 dirty page。
...
在大多数真实的场景中，一些碎片是不可避免的, 所以作为经验法则，把我们的**碎片化降低到25%或者更少**。

**使用 autorelease pool 也是一种减少碎片的方式**，自动释放池会在执行超出释放池范围时告诉系统释放在它内部分配的所有对象,这有助于确保创建所有在释放池内的对象具有**相似的生命周期。**

尽管碎片化可能是所有进程的问题，**长时间运行的进程尤其容易产生碎片化**。因为他们有许多创建和销毁的对象，分割地址空间的可能性可能性会更大。比如：如果你的 app 使用长时间运行 extensions，一定要看一看这些进程的碎片化。
...
% FRAG 这一列展示了我的内存在每一个分配的 zone 上因为碎片产生浪费的百分比。他们中的一些数真的比较大。但是我只需关心 `MallocStackLoggingLiteZone`，这个因为 `MallocStackLoggingLiteZone` 有最多的脏内存份额。脏内存总共5 MB，`MallocStackLoggingLiteZone` 占用4.3 MB。所以这种情况下，我可以忽略其他 zone。 `dirty + swap frag size` 这一列精确的展示了因为碎片每一个 `malloc zone` 有多少内存被浪费了 。
...
使用在执行 `XCTest` 失败生成的 memgraph 文件来帮忙诊断你的内存问题，首先你应该检查泄露，使用 `leaks` 工具并且使用 MSL 堆栈来帮忙找到需要修复的泄露。如果回归不包含泄露，再去检查堆。使用 `vmmap -summary` 来确认堆上的内存。如果需要,使用 `heap -diffFrom` 来查看那个对象类型需要对内存的增长负责。如果罪魁祸首看起来很明显, 使用 `heap -addresses` 来获取地址。如果罪魁祸首看起来并不明显，尝试使用 `leaks -referenceTree` 来找到一些线索。最后,搭配使用 `leaks -traceTree`  `malloc_history` 来调查犯罪对象地址。
