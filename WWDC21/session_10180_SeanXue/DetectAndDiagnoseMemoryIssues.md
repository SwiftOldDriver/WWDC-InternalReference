...
> 审核：
>
> Leo，iOS开发，老司机技术周报编辑，抖音iOS基础体验负责人
>
...
阅读指导：为了保证文章的完整性，我们会对一些概念进行详细的解释，你可以速读已了解的部分，或者直接跳至下一小节继续阅读。
...
![image-20210615001122429](https://images.xiaozhuanlan.com/photo/2021/b3295c97868028ec49392aace1720f56.jpg)
...
![image-20210615001213304](https://images.xiaozhuanlan.com/photo/2021/44aa0f9809f8b1a33de18d6f9a9d827b.jpg)
...
![image-20210615001236922](https://images.xiaozhuanlan.com/photo/2021/42289f80b09d3d2744018a792e5cfcb6.jpg)
...
![image-20210615001328356](https://images.xiaozhuanlan.com/photo/2021/d612928530518e2ca65f2205bcd2052f.jpg)
...
![image-20210613150748853](https://images.xiaozhuanlan.com/photo/2021/e0434ad026028fc78815313366909efc.jpg)
...
![image-20210615105635144](https://images.xiaozhuanlan.com/photo/2021/7bac88c0f21104306d2646d526f47f68.jpg)

当进程创建了对象,在失去了所有指向该对象的指针的时候，并没有回收该对象。我们称这种情况为泄露 (Leak)。我们用灰色的箭头表示对象之间的引用，每个对象都有至少有一个引用。

![image-20210615105830292](https://images.xiaozhuanlan.com/photo/2021/e650cf317123b1dd4b289c2acde6b284.jpg)

注意 A 和 B 之间的虚线，这表示此时我们把 A 对 B 的引用置为 nil，并且移除 A 上的 B 对象。

![image-20210615110643754](https://images.xiaozhuanlan.com/photo/2021/c473b9fba40cb079de61cc09b6581410.jpg)
...
![image-20210615112430981](https://images.xiaozhuanlan.com/photo/2021/d0dff195c775926c64c62bda133874ed.jpg)

循环引用也会引起泄露。Swift 中的最常见对象泄露就是循环引用引起的。在上图中，对象 A 和 B 就是循环引用。它们相互引用，但没有外部引用。这意味着进程不能访问或者释放他们。所以它们被认定为泄露。

![image-20210615151156529](https://images.xiaozhuanlan.com/photo/2021/ddfe68b70b7dff168f9d1d00d771f2b2.jpg)
...
![image-20210615204512092](https://images.xiaozhuanlan.com/photo/2021/08e65f8bc88da87686d009318cd78cd6.jpg)
...
![image-20210616215204050](https://images.xiaozhuanlan.com/photo/2021/22fa0f5381e9f31f97ce704857e8176a.jpg)

page 是系统授予进程的固定大小、不可分割的最小内存块。因为 page 是不可分割的，当进程写入 page 的任意部分，整个 page 都会被认为是 dirty 的并且进程将会管理它，即使 page 的大部分没有被使用到。

当进程的 dirty page 没有被 100% 占用时，就会产生碎片化。为了理解为什么出现碎片，我们来看一个例子： ![image-20210616215810552](https://images.xiaozhuanlan.com/photo/2021/388d4bcc24d9c3adbba2a60683f692f4.jpg)

首先有三页 clean page。

![image-20210616215948349](https://images.xiaozhuanlan.com/photo/2021/4d56c6d102f5f722b108c0fdedff49d7.jpg)

当进程运行的时候，创建的对象会填满这些 page。此时 clean page 就变成了 dirty page。

![image-20210616220217236](https://images.xiaozhuanlan.com/photo/2021/2c185907f175ecc32ccba89d2bac1b20.jpg)

当部分对象被释放,它们填充过得地方就会变成空槽,在上图中被标记为 free memory。因为依旧填充着对象，这两个 page 依旧被标记为 dirty。

![image-20210616220624885](https://images.xiaozhuanlan.com/photo/2021/0000be48f76d8bb8b93b9601651a277d.jpg)

系统想用将要创建的对象填充空槽，右侧蓝色方块是即将要创建的对象，不幸的是，即将创建的对象太大而不能插到空槽里，即使空槽的大小加起来足够大，但是空槽不是连续的。它们不能给一整个对象使用。

![image-20210616222503351](https://images.xiaozhuanlan.com/photo/2021/fdc7025b0e6384549691fa80a46fa699.jpg)
...
* Memory Report
  ...
  ![Memory Report](https://images.xiaozhuanlan.com/photo/2021/fefe10491726e15168d49afcd28fc7a1.jpg)
...
* Debug Memory Graph
  ...
  ![image-20210614231737001](https://images.xiaozhuanlan.com/photo/2021/0735a3329f5a7814b1f5326905dbe804.jpg)
  ...
  **进阶使用**：
  ...
  ![malloc stack logging](https://images.xiaozhuanlan.com/photo/2021/486cbcfd1cbc6c46282bb746d6c07acd.jpg)
...
![image-20210614163556168](https://images.xiaozhuanlan.com/photo/2021/5f98569574282ed75ac8f9d8858cc821.jpg)

XCTest 框架帮助我们直接在项目的单元测试和 UI 测试中监控 app 的内存占用， MetricKit 和 Xcode Organize 帮助我们自定义的监控生产环境上的内存指标。

![image-20210614165019022](https://images.xiaozhuanlan.com/photo/2021/a13bb66577d5e585ee49da7d5f892590.jpg)

我们将使用 XCTests 做性能测试，但是注意这些技术还可以应用在一般内存问题分类和调查中。 使用 XCTests 做性能测试, 你能测量系统资源，比如：内存利用率, CPU 使用率，磁盘写入等等。 

![image-20210614223611948](https://images.xiaozhuanlan.com/photo/2021/ac2552ffcee3e35a54f7352dfa7f901b.jpg)

苹果在Xcode 13 **新增**了使用 **XCTest** 收集诊断数据的新功能，来帮忙分类测试回归。通过执行 **XCTest** 用例来生成 `Ktrace files`  和 `Memory graphs`。

#### Ktrace file

![image-20210614231308710](https://images.xiaozhuanlan.com/photo/2021/bf90eb6642008670b123d0c915ed7db5.jpg)
...
![image-20210614231737001](https://images.xiaozhuanlan.com/photo/2021/60ec1bef534976e8020a11b5e381af4d.jpg)
...
![image-20210614232529918](https://images.xiaozhuanlan.com/photo/2021/a66c3a351db8b882be26a40eaae265d2.jpg)
...
![image-20210614170804690](https://images.xiaozhuanlan.com/photo/2021/99ff0011c79c19fbca4bf3892e9ab640.jpg)
...
![image-20210614220821604](https://images.xiaozhuanlan.com/photo/2021/e951b870f0ec8c75018e3c03701edb11.jpg)
...
![image-20210614222606040](https://images.xiaozhuanlan.com/photo/2021/cb36fdeed99b0c20e033ca398d2bea86.jpg)
...
![image-20210614235156514](https://images.xiaozhuanlan.com/photo/2021/eda893578902a4cbfd3bb17eeb2d5170.jpg)
展开测试日志，可以找到可以获取的内存图。

![image-20210614235423403](https://images.xiaozhuanlan.com/photo/2021/1f3d84aaabb4fd8540c8df6d847e4059.jpg)

下载并且解压后，我们会发现如下两个内存图，苹果收集了最初的内存图并在名称前面添加了 pre，收集了最后一次迭代的内存图，并在名称前添加了 post。我们可以通过前后两个内存快照分析期间的内存增长。

![image-20210615000432894](https://images.xiaozhuanlan.com/photo/2021/46a61883aa25fbff2d609bf357e69db2.jpg)
...
![image-20210615151419176](https://images.xiaozhuanlan.com/photo/2021/c5d8d14aa0274557e873e1abb27f35b1.jpg)

大家还记得我们之前使用 XCTest 执行 case 失败时生成的两份文件吧，我们将使用这两份文件来检查泄露。

![image-20210615151716402](https://images.xiaozhuanlan.com/photo/2021/b6c46d10cfdb4ee3c40cfc18d95db075.jpg)
...
![4leaksfor240bytes](./image/34.jpg)

输出展示了我们有 4 个泄露，一共 240 byte。

![image-20210615152124699](https://images.xiaozhuanlan.com/photo/2021/8115ee170b6e406c83bcdd5eec4f5140.jpg)

接下来，输出包含了每一个泄露的详细的情况，这些信息可以给我们一些线索，帮助我们找到是什么引起了泄露。最上面的对象图指出 ROOT CYCLE,这表示我们面对的是循环引用。

![image-20210615152802392](https://images.xiaozhuanlan.com/photo/2021/d323fdc05a1152cfea2d8506089fa09f.jpg)
...
![image-20210615181249641](https://images.xiaozhuanlan.com/photo/2021/7daeed6aa432e140571a9173b53a43f9.jpg)

通常，你会希望从代码中找到具有符号的调用堆栈,这个是我代码中的一部分调用。正在泄露的 MealPlan 对象是在 populateMealData 中创建的。我们打开 Xcode 来看一下我们是否能够修复这个问题。

![image-20210615181611352](./image/38jpg)

这个函数就是我们在 leaks 的输出中看到的，这里我分别创建了 MealPlan 对象和 MealItem 对象，日志里面说这两个对象有循环引用。

![image-20210615183622686](https://images.xiaozhuanlan.com/photo/2021/88ecb228cb958dba9e3ef555a341d32c.jpg)

 `addMealToMealPlan` 这个函数看起来就有点可疑，让我们看一下。

![image-20210615183738662](https://images.xiaozhuanlan.com/photo/2021/544fd25ec373e8772ae83394d2cbc287.jpg)

这里我们调用了 addItem 到 mealPlan，也调用了 addPlan 到 menuItem。

![image-20210615184000286](https://images.xiaozhuanlan.com/photo/2021/5a2f224e79459f28a951a1e4944b99a9.jpg)
...
![image-20210615184444778](https://images.xiaozhuanlan.com/photo/2021/1848fa73d89469edd0f8f3429c018e06.jpg)
...
![image-20210615211510398](https://images.xiaozhuanlan.com/photo/2021/5f47fd5199f6f5837266cb4779a87e3a.jpg)
...
![image-20210615212406816](https://images.xiaozhuanlan.com/photo/2021/ea7ba5715528fae7efbdeb5a7abb49d5.jpg)

在 pre memegraph 中物理占用在 112 MB 左右。

![image-20210615212632183](https://images.xiaozhuanlan.com/photo/2021/bb5b38c5ba42cff25b8af6506a38d8d5.jpg)

而 post memegraph 中物理占用在 125 MB 左右。两次结果差值大概有 13 MB 左右。

![image-20210615213215716](https://images.xiaozhuanlan.com/photo/2021/7a3b9cabcd630b0c48223144d6b352f6.jpg)

向下滚动输出，可以看见进程的内存占用被按 region 进行了划分。

![image-20210615213419470](https://images.xiaozhuanlan.com/photo/2021/b916bc2618c428185a174cc1c6074d86.jpg)

因为我怀疑这块是一个堆分配的问题，所以我想从  MALLOC 范围的看看这些 regions。因为这些 regions 包含我所有的堆对象。

![image-20210615214206310](https://images.xiaozhuanlan.com/photo/2021/1b939c872bd835397b74b6c76e6f9946.jpg)

还记得上面说过的 **memory footprint = dirty size + compressed size** 在这个工具中 swapped 代表 compressed, 所以这些列我们只需要关心 **dirty size** 和 **swapped size** 。

![image-20210615214254471](https://images.xiaozhuanlan.com/photo/2021/5cafb7a0647fedd08bf2b44e117dcb1d.jpg)

图上显示 MALLOC_LARGE 块大概持有 13 MB 的 dirty memory。这大概就等于我们回归的大小。所以我们需要查一下到底是谁贡献了这 13 MB。

![image-20210615214621447](https://images.xiaozhuanlan.com/photo/2021/38a09377212a7407b7244dfb047183a4.jpg)

> 这里可以使用 `heap --diffFrom` 来看一下 pre 和 post 的差别。这个命令可以得到 post 中存在但 pre 中没有的对象。

最下面显示了这些 diff 大概有 13MB (13680384 bytes)。

![image-20210615215202648](https://images.xiaozhuanlan.com/photo/2021/7dfbc2391dc9a8786ab89f4f75f6c725.jpg)
...
![image-20210615215855078](https://images.xiaozhuanlan.com/photo/2021/184e72216e8574f8ba75088b127c4cbb.jpg)
...
![image-20210616110444685](https://images.xiaozhuanlan.com/photo/2021/f6b1b1d91e4ca01eba7af76b2a31074f.jpg)
...
![image-20210616114028220](https://images.xiaozhuanlan.com/photo/2021/180d8a7ddca400c1d26c534b4873cc14.jpg)
...
![image-20210616120714875](https://images.xiaozhuanlan.com/photo/2021/b0e19f1f0e4f7052d6ea84899d14aeec.jpg)
...
![image-20210616164455729](https://images.xiaozhuanlan.com/photo/2021/1273d834846ba7ed8dcf63016316e773.jpg)
...
![image-20210616170232527](https://images.xiaozhuanlan.com/photo/2021/afa51c858dde1ce1c0bc7d7a7c6cf83b.jpg)

**方法二**：另一个方法就是，当 mealData 保存到磁盘之后，手动置 nil。swift中的数据对象管理很聪明，一旦这个对象没有任何引用的时候，它就会被自动释放。

### 检测和诊断碎片化

![image-20210616223523732](https://images.xiaozhuanlan.com/photo/2021/aa952c9dc749e221a5a640c95c8d5636.jpg)

我手动创建了一些对象，被标记为 my object，由于我没有太关注我的代码，系统最终交错安插了我的对象和其他对象，

![image-20210616223633731](https://images.xiaozhuanlan.com/photo/2021/8306720d5c10eb6e80ac6de96b268dc6.jpg)

现在我释放掉了我的对象，出现了四个空闲的空槽，因为 allocated object 的存在，它们都没有连续，这将导致50%的碎片化和四个 dirty page。

![image-20210616224228526](https://images.xiaozhuanlan.com/photo/2021/29022872ba94fba3247a23fe77e8a3f9.jpg)

假如我写的代码**一起创建了所有 my object**, 它们最终就会只会占用两个 page。

![image-20210616224248384](https://images.xiaozhuanlan.com/photo/2021/fd2ff6d52b21f46be8b1a5d683390efb.jpg)
...
![image-20210617103651724](https://images.xiaozhuanlan.com/photo/2021/57f5081700cda90d28c10e09f9b3dc2b.jpg)
...
![image-20210617103956837](https://images.xiaozhuanlan.com/photo/2021/74fe75393d7c015ef540aa3b156c5464.jpg)

下面来快速的看一下我的进程碎片，我使用 `vmmap -sunmmary`,并且滚动到输出的最下面。

![image-20210617104153630](https://images.xiaozhuanlan.com/photo/2021/b706059096b44deb797b4f18e5db87d2.jpg)
...
![image-20210617105552030](https://images.xiaozhuanlan.com/photo/2021/48b32c375511337836e156d96e6ec29a.jpg)
...
![image-20210617170856971](https://images.xiaozhuanlan.com/photo/2021/a9caa13938defe91ea30505391b1c6ce.jpg)

 在这个 `case` 中，我因为碎片浪费了大约 800 KB。这个看起来很多，但是我们之前提过，一些碎片化问题是无法避免的，所以只要我还在 **25%** 碎片化以下，我就认为这个浪费是可以接受的。

![image-20210617171420011](https://images.xiaozhuanlan.com/photo/2021/411a2566d1770e72cca6b64c71d691cb.jpg)

目前`MallocStackLoggingLiteZone` 的碎片化还在 19% 左右,这显然低于 25% 的经验法则,所以我还不用担心。

![image-20210617195647203](https://images.xiaozhuanlan.com/photo/2021/25aef6666143aecad144f7cd62b674f1.jpg)

如果我真的有碎片化问题，我可以使用 `instruments` 的工具 `Allocations`  去追踪这个问题，具体来说，我希望查看分配列表视图，看看在我感兴趣的区域中哪些对象被持久化和销毁了。

![image-20210617195714870](https://images.xiaozhuanlan.com/photo/2021/c6842211337697a01b6a0441ab017019.jpg)
...
![image-20210617200756322](https://images.xiaozhuanlan.com/photo/2021/cc0d7a6cbf2749140eab2087868107f5.jpg)
...
![image-20210617200949635](https://images.xiaozhuanlan.com/photo/2021/32778df6051f2b347df0aa5798f574ae.jpg)

现在，我已经解决了泄露和堆回归，验证了碎片化不是一个问题。在此运行 `Xcode`。太棒了，现在测试通过了，并且回归问题也被解决。现在你已经学习到了关于 检测和诊断内存问题，让我们来回顾一下在你自己 app 可以使用哪些工作流程。

#### 检测流程

![image-20210617201641291](https://images.xiaozhuanlan.com/photo/2021/5f331b2e1c3d7cce2eed6e372060c6c1.jpg)

开发一个新功能之后，用 `XCTest` 写一个性能测试来监测内存,和其他系统指标。为每个测试设置 baseline。然后用测试来捕获回归，并使用收集到的 `ktrace` 和 `memgraph` 文件进行调查。

#### 诊断流程

![image-20210617202711003](https://images.xiaozhuanlan.com/photo/2021/9a77fa31d7b06d770463eedee6680734.jpg)
