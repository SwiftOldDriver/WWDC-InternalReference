...
为了最大限度地减少编码时间，低延时模式`去除帧重新排序`，遵循一进一出编码模式。其实就是不使用 B 帧编码，编码帧都向前参考，去除因编码 B 帧带来的等待后面参考帧的延时。此外，该模式下的`码率控制器`对网络变化的适应速度也更快，因此也最大限度地减少了网络拥塞造成的延时。通过这两个优化，我们已经可以看到与默认模式相比有明显的性能提升。对于 720 p 30 fps 的视频，低延时编码可以减少高达 100 毫秒的延时。这种节省对于视频会议至关重要。
