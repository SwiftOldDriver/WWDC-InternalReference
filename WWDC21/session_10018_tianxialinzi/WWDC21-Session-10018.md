...
AsyncImage也可以自定义。例如，我们可以为加载的图像添加修饰符，也可以定义自定义占位符.我们甚至可以添加自定义动画和错误处理。
...
讲述 await 之前，我们先说下我们开发中常说的**并发**，我们可以把某几个操作的分时运行叫做并发，比如在同一个线程中的两个操作交替运行 (这需要这类操作能够暂时被置于暂停状态)，其实并发主要解决的问题主要有2点：

* 如何确保不同运算运行步骤间的交互或是通信按照正确的顺序执行
* 如何确保运算资源在不同运算之间被安全地共享和访问

为了更容易和优雅地解决上面两个问题，Swift引入了两个重要的关键字async 和 await。
...
```
在异步函数中加上 async 关键字，我们可以在函数体内部使用 await 关键字,同时它也要求其他人在调用这个函数时，使用 await 关键字。

异步函数的 async，和与它处于同样位置的 throws 关键字有点相似。在使用 throws 时，它允许我们在函数内部使用 throw 抛出错误，并要求调用者使用 try 来处理可能的抛出。
async 要求在特定情况下对当前函数进行标记，这是对于开发者的一种提示，表明函数在此处有一些特别的性质：try/throw 代表了函数在这个地方可以被抛出，而 await 则代表了函数在这个地方可能会放弃当前线程，它是程序的潜在暂停点。
