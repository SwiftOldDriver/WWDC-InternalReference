...
使用时间分层的方法非常直接，在低延迟模式下设置 BaseLayerFrameRateFraction 会话属性为 0.5，这意味着一半的输入帧被分配到基础层，剩下的分配到增强层。另外，对于基础层，设置 `CMSampleAttachmentKey_IsDependedOnByOthers` 属性为 `true`，增强层设为 `false`。
...
我们可以把时域分层和最大帧 QP 用于群组屏幕共享 App。时间分层可以为每个接收方有效地生成输出视频，而我们可以降低最大帧 QP 以获得更清晰的图像。
