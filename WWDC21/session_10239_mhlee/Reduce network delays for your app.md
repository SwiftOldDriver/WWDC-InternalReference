...
如上面介绍，苹果在此次的发布会才介绍将 `HTTP3/QUIC` 引入在 iOS 15.0+ 系统。 `NSURLSession` 和 `Network Framewokr` 在新系统将默认支持 `HTTP3/QUIC`。
...
通过上述方式，即使 app 被挂起，也可以让请求一直在后台运行。如果请求数据时效性不高，可以将 `isDiscretionary` 置为 `true`，这样系统可以等到较理想状态再处理该请求。比如，系统会将延迟大文件传输的请求，直到手机变为充电状态或由移动网络切换为 WIFI 状态。
