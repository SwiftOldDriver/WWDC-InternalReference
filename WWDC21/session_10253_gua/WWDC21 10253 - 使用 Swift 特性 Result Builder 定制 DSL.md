...
DSL 语言通常是**声明式**（Declarative）的，从而通过最简单的描述来完成代码的编写。

### 嵌入 DSL （Embedded DSLs）

目前我们可以使用 Swift 语言作为宿主语言的语法特性来编写 DSL 的语法规则，然后在 Swift 的代码中来引入 DSL，这种方式我们称之为**嵌入 DSL**（Embedded DSLs）。

嵌入 DSL 的方式显然要比设计整个语言并为之编写编译器要容易的多，因为这是站在巨人的肩膀上，我们的基础就是一个高级语言，并且它已经确定了语法并自带编译器。

使用嵌入 DSL 也是目前项目中更合理的方式，因为往往我们仅用 DSL 语法来实现项目中的部分需求问题，而这个问题仅是 App 中的一个特定领域的问题。

Swift 旨在支持嵌入 DSL，如果你使用过 SwiftUI，那其实你已经接触了嵌入 DSL。SwiftUI 的视图描述语法，是和我们设备屏幕上的布局视图相对应的。所以当你使用 SwiftUI DSL 进行代码编写时，我们实现的仅仅是对于视图的描述，而底层的构建，全部交予 SwiftUI 进行处理。

使用嵌入 DSL 的场景我们可以看看下面这个例子：

![image.png](https://cdn.nlark.com/yuque/0/2021/png/227152/1625403107245-08a78ea4-8bc1-4a0f-a160-89dfa983766a.png#align=left&display=inline&height=899&margin=%5Bobject%20Object%5D&name=image.png&originHeight=899&originWidth=1617&size=563309&status=done&style=none&width=1617)

使用 SwiftUI 可以很好的描述布局，有着更高的可读性。于此相比，我们可以来看一下不适用 SwiftUI 的 DSL，我们的代码是什么样子。

![image.png](https://cdn.nlark.com/yuque/0/2021/png/227152/1625403217895-d27d658a-8dbe-4b9d-9439-ae6cf0f7925c.png#align=left&display=inline&height=1229&margin=%5Bobject%20Object%5D&name=image.png&originHeight=1229&originWidth=2316&size=1205565&status=done&style=none&width=2316)

比较之下，使用 Swift 的原生语法需要创建大量的临时变量来实现，而且其代码无法展示出视图布局的层级关系。而 SwiftUI 更加接近视图的描述，可读性更强。

### SwiftUI 中的 DSL 包括什么

我们用下面代码为例：

![image.png](https://cdn.nlark.com/yuque/0/2021/png/227152/1625403319525-f5ea151d-ac71-4db1-9bc9-bfe72f076984.png#align=left&display=inline&height=892&margin=%5Bobject%20Object%5D&name=image.png&originHeight=892&originWidth=1599&size=476866&status=done&style=none&width=1599)

- ① 这个是 Swift 中的 _Property Wrapper_ 特性，通过这种标记方式，可以对 `struct` 或 `class` 中的成员添加某些操作行为。

- ② 这种 `func` 后直接跟随一个闭包的语法是 Swift 中的 _Trailing Closure Argument_，这种闭包的语法更加简洁。

- ③ 这是 Swift 中的 _Result Builders_ 特性，它将末尾的实例作为当前方法的返回值进行处理。

- ④ 这是 SwiftUI 中的 _Modifier-style Method_，通过这种方式可以构造出一种链式调用的表达，更加利于代码可读性。
...
1. 用来实现 DSL 中的特定表达；
1. 用来简化常规函数、getter 方法或闭包的语法；
1. 用来封装方法调用，语义上是使用返回值；
1. 接管方法的返回值；
1. 是一个编译时的特性，可无视系统版本差异；

### 如何运作？

我们用一个 SwiftUI 的例子来举例说明 SwiftUI 中 DSL 的运作原理，其中包括了 _Result Builders_ 和其他的几个特性。
...
我们发现 `@ViewBuilder` 是一个 _Result Builder_，它会告诉编译器它描述的是一个**结果值**，且该结果是一个 `View` 类型。所以在花括号中的所有内容，将会去捕获闭包中变量的结果，合成一个值进行返回。
...
在上述代码中，我们还需要解释一个关键字 - `.font(.title)` ，这其实是 Swift 当中的另一个新特性 - _Modifier-style Method_。其实在这个方法中，它会修改 `Text("Title")`  返回值的一些属性，再次返回相同的类型结果。这种做法在 SwiftUI 中十分常见，例如我们可以继续增加一个修改字体颜色的方法：

```swift
let v0 = Text("Title").font(.title).foregroundColor(.green)
```

于是我们可以发现，在 SwiftUI 中，*Modifier-style Method *特性和* Result Builder *特性很好的协同工作，共同构成了一个 DSL 描述性语言。从这点出发，以下两点也是我们开发者在创建 DSL 的一个原则：**语法特性和用户预期表现保持平衡**。

### Result Builder 的限制

虽然我们可以使用 Swift 的 Result Builder 进行一些 DSL 的设计，但是其并不是完全开放的。因为我们需要收到以下的限制条件：

1. Result Builder 方法中，**仍需遵守 Swift 的语法设计**：例如花括号匹配规则等；
...
其实设计一个 DSL ，和设计 API 十分类似。因为首先，我们需要考虑如何使用 Swift 来表达对应的想法和行为，设计 DSL 时从某些角度来说，就是**使用了区别于设计 API 时的常规语法特性**。

另外设计 DSL 更多的是为了提升自己的开发体验，所以设计 DSL 是一个主观的，区别于 API 中需求方是来自他人。由于 DSL 是我们开发者角度的主观表达，所以目标相对来说更加清晰。
...
1. 将冰沙的定义合并到 `list`  中；
1. 让成分属性 `measuredIngredients`  的描述更加精简；
1. 分离 `description`  和 `measuredIngredients`  的声明，并置之于数组之外；
...
然后针对于第 3 点，我们打算使用 *Modifier-style Method *这种链式表达的方式，来描述每个沙冰的成分，它看起来像下面这样：
...
我们可以看到 `title`  正好在 `navigationBar`  上，是处于 UI 的最上方， `description`  紧凑的在 `title`  之下，所以我们用一个 `String`  紧跟在闭包的第一个位置。再往下，就是哥哥成分列表，使用我们上面精简后的语法就得到了如图样式。
...
![image.png](https://cdn.nlark.com/yuque/0/2021/png/227152/1625383369114-ac344e10-b9cc-45dd-bebd-7783c72418b6.png#align=left&display=inline&height=1106&margin=%5Bobject%20Object%5D&name=image.png&originHeight=1106&originWidth=1511&size=396088&status=done&style=none&width=1511)
...
![image.png](https://cdn.nlark.com/yuque/0/2021/png/227152/1625389560821-5719dadc-0e21-4142-bf13-690e505ac1b1.png#align=left&display=inline&height=880&margin=%5Bobject%20Object%5D&name=image.png&originHeight=880&originWidth=1626&size=708427&status=done&style=none&width=1626)
...
前面我们完成了大多数情况的 DSL 语句处理，但是当我们编译发现虽然我们使用了 `buildOptional`  处理了单 `if...`  形式的条件语句，但是目前无法处理具有 `else`  或者 `else if`  这种子句的情况。

![image.png](https://cdn.nlark.com/yuque/0/2021/png/227152/1625390619827-adff3710-557b-4820-8811-497e880be286.png#align=left&display=inline&height=1150&margin=%5Bobject%20Object%5D&name=image.png&originHeight=1150&originWidth=1555&size=476133&status=done&style=none&width=1555)
...
通过 `buildEither(first component:)` 和 `buildEither(second component:)` 来标记你使用的是哪个方法。

那如果是多条件的语句呢，需要写一个 `third`  吗？其实目前是不提供这种超出 3 个条件的方法的，Result Builder 在多条件的场景下，会逐一拆解成双条件语句进行解释。下面这个图展示了多条件语句时的原理：
...
如此，我们打工构成，完成了整个 DSL 的设计！

### 定制编译器诊断说明

由于 DSL 是我们自己定义的语法，所以在与他人协助开发的时候，由于配合者并不一定参与 DSL 的全部设计，所以他们对 DSL 会有一些错误使用。DSL 是我们自己设计的，所以 Xcode 并不能给出有效的诊断提示。而 Result Builder 中，我们可以使用相关 `Attribute`  来增加编译器诊断说明。
...
![image.png](https://cdn.nlark.com/yuque/0/2021/png/227152/1625399879230-d4a32b5f-ceaa-4224-8d0f-0cef182ba6b7.png#align=left&display=inline&height=899&margin=%5Bobject%20Object%5D&name=image.png&originHeight=899&originWidth=1685&size=860871&status=done&style=none&width=1685)
...
1. Result Builder 特性可以在 DSL 中充分利用，让语义描述更加清晰；
1. Modifier-style Method 与 Result Builder 两个特性结合使用，可以大量的简化代码，并增强可读性和可拓展性；
1. Swift 也提供了 DSL 在协助开发时的诊断定制化提示，让他人更加容易上手。
