...
> 本篇来源为 WWDC2021 中 [What’s new in AVFoundation](https://developer.apple.com/videos/play/wwdc2021/10146/) 

本Session会介绍三个主题：音频元信息，视频元数据合成和字幕创作

## 音频元信息
首先，我们会多花一些时间讨论 AVAsset 检查元数据的新能力：

我们先说说 AVAsset异步查看元信息 ，为了方便理解，先了解一下 AVAsset 的背景知识：
...
当拥有媒体资源时，除了播放之外，经常遇到的需求是检查元数据，比如：
- 媒体的持续时间是多少？
- 包含的音频和视频的格式是什么？

这就是所谓的媒体资源检查

每当我们试图获取媒体资源元数据时，都需要明确一件事：
**媒体资源检查要按需进行，因为媒体文件会非常大**——一部长篇电影的大小可能动辄几个GB

所以一般并不会立刻下载整个文件，而会等待到需要时才加载属性值

其次，媒体资源检查是一个``异步过程``——这很重要，因为网络 I/O 也有开销；如果媒体资源存储在网络中， AVAsset 将在准备好时通过异步回调告知结果
...
>先让我吐槽一下，实在不能忍字符串，拼错个字母，就万事皆休了，但这些东西苹果没上新 API 之前，也不会说不好～

:warning: 例如，容易忘记一些基本的检查步骤：


![](https://images.xiaozhuanlan.com/photo/2021/5ca9524990ba9d0315fba8bb52dcdd3a.png)

- 如果我们在没有完成加载的情况下使用这些同步的属性/方法，就会最终阻塞 I/O
- 假如这一切发生在主线程上，这就意味着我们的 App 会被挂起不确定的时间


>新 API 除了更易用，会还消除上面这些常见的误用，而iOS也可能在未来版本中逐渐废弃旧的同步 API

下面，我们看看新的 API 是如何工作的，当我们需要检查媒体资源属性时，可以这样使用：
...
1. 每个属性标识符在编译时都与一个结果类型相关联，它决定了  ``load``  方法的返回类型。
...
- 一部分操作发生在异步操作开始之前——先创建一个媒体资源并要求加载其持续时间
  - 此时，会执行相关的 I/O 和解析动作以确定其持续时间，这时我们开始进入等待
- 在等待时，调用函数被挂起，即在 await 之后编写的代码不会立即执行

**注意：当前线程并没有被阻塞** ——在等待时可以做其他工作
...
当``AVAsset``明确了需要的所有属性，它会自动进行批量处理

>加载多个属性值的返回结果是元组，返回值相关类型和属性标识符传入的顺序相同

在上面例子中，返回元组的第一个元素是 ``CMTime``，第二个元素是 ``AVAssetTracks`` 数组——都是异步操作

如果需要检查属性的状态，可以使用新的 ``status(of:)`` 方法来确认，无需等待值加载
...
- 在加载过程中检查状态，将会获得 ``.loading`` 状态
- 该属性已加载，将获得 ``.loaded`` 状态
- 最后，如果遇到网络异常等故障，会得到 ``.failed`` 状态
  - 失败时，会关联一个error，用来描述出了什么问题：注意，这和加载请求失败时，  ``load``  方法抛出的错误相同
...
另外在这些新 API 中，还增加了异步方法集合，可以使用它们来获取某些属性值的特定子集：
- 例如，可使用上面的方法来只加载部分曲目（而不是加载所有曲目），或仅加载音轨
...
以上这些就是 WWDC21 为媒体资源检查准备的新 API ，为了更方便地迁移到这些新接口，这里有一个简短的迁移指南：
...
2. 如果我们还在使用旧的 ``statusOfValue(forKey:)`` 方法并通过 ``switch`` 分发不同状态，现在推荐使用新的状态枚举来完成工作
...
4. 如果代码没有加载和状态检查步骤，只是依赖属性和方法的当前行为，比如一直阻塞直到结果可用——**首先官方不推荐这样使用，因此也没有提供替代的方式**

苹果已经将新的属性加载 API 设计得与获取简单属性一样易用——推荐大家迁移到新 API ！
...
> WWDC21 对自定义视频合成器进行了增强，可以通过在合适的位置插入代码来直接合成视频

在今年新增的能力中，应用程序可以在自定义合成器的回调中获得每帧元数据。
...
![](https://images.xiaozhuanlan.com/photo/2021/1467d6520b40093d752b6ffe512e3b49.png)
...
首先要完成两个设置项：
1. 使用新的``sourceSampleDataTrackIDs`` 属性告诉视频合成对象合成相关的轨道ID；
2. 设置``AVMutableVideoCompositionInstruction``的``requiredSourceSampleDataTrackIDs`` 属性以告诉它与此相关的轨道 ID

**执行这两步很重要，否则就无法在合成回调中获得任何元数据了。**
...
- ``AVTimedMetadataGroup`` 是元数据的高级表示，其值可以被解析为字符串、日期或其他对象
    - 如果需要使用原始数据，可以使用 ``sourceSampleBuffer(byTrackID:)`` 方法来获取 ``CMSampleBuffer`` 

一旦有了元数据，下一步就可以使用元数据和视频帧来生产 ``最终视频帧`` 并完成请求。

以上就是将元数据放入自定义视频合成器回调中所需的全部步骤了，希望读者自己也能试一试！

>译者注：上面👆用GPS举例并不偶然， Apple 近期正好推出了空间音频，我们想象一下用位置信息应该如何实现：
- 音频文件中除了左/右两声道，还包括声源每一帧的相对位置信息，可以作为一条单独的 track
-  AirPodsPro 有内置陀螺仪，并实时通过蓝牙告知播放应用
...
我们先从 ``AVCaption`` 开始——代表单个标题，具有文本、位置、样式和其他相关字幕的属性的对象：
...
此外，我们在 ``AVCaptionConversionValidator`` 类中有一个新的校验能力，能帮助我们确保编写的字幕实际上与所选择的文件格式兼容——这为什么这很重要呢？

比如， ``.scc`` 文件，它包含  ``CEA-608`` 字幕（最早是一个广播电视标准，但在线视频领域也很常见，详见[这里](https://www.theoplayer.com/blog/cea-608708-closed-captions-make-content-accessible-everyone)） ，对在给定时间内可以拥有多少字幕具有非常具体的限制——细致到对表示单个字符及其样式的数据都有格式规定

验证器不仅会帮助确保字幕流与输出文件格式兼容，还会给出对字幕的调整建议：比如，调整时间戳来保证兼容性

上面我们说了创作字幕，下面我们再说说如何读取字幕：
...
![](https://images.xiaozhuanlan.com/photo/2021/78d2823c9fbffbd14b417c834439480f.png)
