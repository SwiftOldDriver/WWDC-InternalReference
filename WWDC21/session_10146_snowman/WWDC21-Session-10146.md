...
其次媒体资源检查是一个``异步过程``——这非常重要，因为网络 I/O 也有开销；如果媒体资源存储在网络中， AVAsset 将在准备好时通过异步回调告知结果
...
> 有关 ``async/await`` 的详细介绍，可以参考Session 10132: [Meet async/await in Swift](https://developer.apple.com/videos/play/wwdc2021/10132/)
 ...
 我们通常将 await 关键字视为将调用函数分为两部分：

- 有一部分操作发生在异步操作开始之前——先创建一个媒体资源并要求加载其持续时间
  - 此时，会执行相关的 I/O 和解析动作以确定其持续时间，这时我们开始进入等待
- 在我们等待时，调用函数被挂起，即在 await 之后编写的代码不会立即执行

**注意：当前线程并没有被阻塞** ——在等待时可以自由地做更多的工作
...
- 最后，如果遇到网络异常等故障，会得到 ``.failed``状态
  - 失败的case中，会关联一个error，用来描述出了什么问题；注意，这和加载请求失败时， load 方法抛出的错误相同
...
在今年新增的 feature 中，app 可以在自定义合成器的回调中获得每帧元数据。
...
比如，.scc 文件，它包含 CEA-608字幕（最早是一个广播电视标准，但在线视频领域也很常见，详细见[这里](https://www.theoplayer.com/blog/cea-608708-closed-captions-make-content-accessible-everyone)） ，对在给定时间内可以拥有多少字幕具有非常具体的限制——细致到对表示单个字符及其样式的数据都有格式规定
