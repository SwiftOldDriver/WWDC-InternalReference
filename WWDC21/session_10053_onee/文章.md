...
Catalyst 这项技术是苹果于 2019 年的 WWDC 上推出的新技术，其目的在于让开发者能够很快的将一个 iPad App 转换成一个能够跑在 Mac 上的 App。三年来的积累也给这项技术带来了足够丰富的特性。那么，面对 Catalyst 中丰富的各种能力，如何才能让我们所创建的 App 在 Catalyst 下有更好的体验呢？在 [WWDC21-10053-Qualities of a great Mac Catalyst app](https://wwdc.io/share/wwdc21/10053) 中，苹果的工程师向我们指明了一些我们在使用 Catalyst 过程中需要关注的点。
...
除此之外，如果我们手边还有 M1 的 Mac，我们也可以检验一下我们的 App 是否可以直接在没有任何修改的情况下运行在 M1 的 Mac 上。对应的，WWDC21 中也有一个关于如何在 M1 上提高 App 质量的 [Session](https://developer.apple.com/videos/play/wwdc2021/10056/)，感兴趣的小伙伴可以看一看。
> 小 Tips：
...
当我们开启了 Mac Idiom 后，我们所使用的 UIControl 如果可能会以 Mac AppKit 风格的形式被展示出来。不过针对 button 和 slider，我们可以单独对其外观进行配置，以让其在 Mac 上不以 Mac 风格的控件展示（参见 [WWDC21-10052-What's new in Mac Catalyst - 支持在 Mac Idiom 下保持自定义的 button 和 slider 样式](https://xiaozhuanlan.com/topic/9734105682)），尤其是针对 button，我们可以实现更多细节上的控制（参见 [WWDC21-10052-What's new in Mac Catalyst - 更多的按钮样式](https://xiaozhuanlan.com/topic/9734105682) ）
...
> Tips：
> 快捷键的支持可以在 [WWDC20-10109-Support hardware keyboards in your app](https://developer.apple.com/videos/play/wwdc2020/10109/) 中了解具体使用方式；
...
```
另外，由于一些快捷键是否能够启用是和当前的 First Responder 有关的，如果我们的 App 有更多的 View 可以成为 First Responder，那么当用户选中或者聚焦在某些 View 上时，用户就能够使用更多的快捷键，所以如果能够让更多的 View 利用  [canBecomeFirstResponder](https://developer.apple.com/documentation/uikit/uiresponder/1621130-canbecomefirstresponder#) 和 [canBecomeFocused](https://developer.apple.com/documentation/uikit/uiview/1622584-canbecomefocused#) 响应用户的操作，对 App 的整体体验也是大有裨益的。更多的细节，可以参考 [WWDC21-10260-Focus on iPad keyboard navigation](https://developer.apple.com/videos/play/wwdc2021/10260)。
...
```
接下来，我们可以通过 `scene(_:willConnectTo:options:)`，在 UIViewController 展示在用户之前，将 NSUserActivity 中保存的 itemID 设置到该 UIViewController 中 ：
```swift
...
```
当 App 被退出时，系统会调用 `stateRestorationActivity(for:)` 方法，我们可以在其中返回一个 NSUserActivity 对象并在其中存储此时 App 的一些状态信息。下一次应用启动的时候，系统会调用 AppDelegate 的 `application(_:configurationForConnecting:options:)` 方法。这样一来，我们就可以复用之前在 AppDelegate 中的代码：
```swift
// State restoration
final class AppDelegate: UIApplicationDelegate {
    func application(_ application: UIApplication, 
...
```
以及 SceneDelegate 中的代码（需要做一些微调，此时 NSUserActivity 不存于 `connectionOptions.userActivities` 而是存于 `session.stateRestorationActivity` 中）：
...
> 小 Tips：
>
> 以上内容主要和 iPad App 多窗口功能有关，对于相关 API 不熟悉的同学看了可能会有些云里雾里，因此可以借助下面这张图来帮助理解整个过程：
>
> ![](https://cdn.nlark.com/yuque/0/2021/png/292393/1626280817225-420548ca-44df-41d7-b267-02ef1aa81e7f.png?x-oss-process=image%2Fresize%2Cw_1504)
>
> 当用户点击打开新的 Window 时，整个过程为：
>
> 1. 用户点击 View
> 2. View 的事件处理方法调用 `requestSceneSessionActivation(_:userActivity:options:errorHandler:)` ，并将当前点击的 itemId 通过 NSUserActivity 的 userInfo 属性传给 UIKit，UIKit 创建新的 UISceneSession（可以不严谨的认为 UIScene 是 Window 的抽象，属于 MVC 的 View 层，而 UISceneSession 则属于 Model 层）
> 3. UIKit 调用 AppDelegate 的 `application(_:configurationForConnecting:options:)` 方法，获得 UISceneConfiguration（当我们创建 UISceneConfiguration 的新实例时，UIKit 就会读取 Info.plist 中对应 name 的配置信息）
> 4. UIKit 在创建新的 UIScene 后，会调用 SceneDelegate  `scene(_:willConnectTo:options:)` 方法
> 5. 开发者通过 NSUserActivity 中的 itemId 来设置当前的 ViewController
>
> State Restore 的整个过程和上述过程类似，仅仅是第一步和第二部略有不同：
>
> 1. 当一个 UIScene 被 disconnect 时，UIKit 会调用 `stateRestorationActivity(for:)` 方法获取当前 Scene 对应的 NSUserActivity 对象，并将其保存在磁盘中
> 2. App 重新启动的时候，UIKit 会将 NSUserActivity 中读取出来
> 3. 同上
> 4. 同上
> 5. 同上
...
更多详细的内容可以在 [WWDC19-212-Introducing Multiple Windows on iPad](https://developer.apple.com/videos/play/wwdc2019/212/) 查看（或者是查看 [WWDC 内参 - iPad 上的多窗口](https://xiaozhuanlan.com/topic/0342159876)）。
