...
一般对仓库的配置都是当且仅当打分超过两分的时候，当前的 `Pull Request` 才能通过，代码才可以进行合并，这样就完成了一次完整的 `Code Review`，别的公司可能没有 `iCode` 之类的代码服务平台，但是 `Code Review` 的操作大同小异。下面我们就跟随 Kieran 看看集成了版本控制工具的 `Xcode 13` 是如何提升我们的工作效率的。
