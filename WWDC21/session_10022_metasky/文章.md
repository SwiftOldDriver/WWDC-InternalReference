...
所有声明式的 UI  框架，都是用数据来驱动，那么这些数据声明方式，就是在开发者和框架之间搭建桥梁，非常重要。SwiftUI 官方给出了以下数据声明方式，它们全都是通过 PropertyWrapper 来包装使用。看到它们，大家的第一反应是不找到如何选择它们，这里有个口诀：`状态自己管 绑定要双向 管理靠对象 变化才监听` 来帮助你战胜它们。
...
如上图代码，分支里的两个 CatRecorder 由于结构性 identity 不同，所以它们被 SwiftUI 视为是两个不同的视图。之前说过这样会影响动画效果，其实也会影响内部数据状态的维持。
...
下面我们来回答文章开头的问题。SwiftUI View 和 视图元素之间采用 Identity 关联起来，它们之间并非一一对应。在 SwiftUI 中每当状态发生变化，都会调用 body 生成新的 View 值，但是否生成新的视图则完全由 Identity 来决定。如果 Identity 一致，就会根据 Identity 去内存中查找之前创建的视图，换言之，相当于保持之前视图的生命周期，并且在内存中用类维持住之前的数据状态，只对更改数据后，视图变化部分进行渲染操作。如果 Identity 不一致，则会新建视图元素，同时视图所依赖的状态也会被重新分配，回到初始值。
