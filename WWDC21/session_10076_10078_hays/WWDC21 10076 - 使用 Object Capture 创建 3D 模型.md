...
> 作者：作者：Hays，iOS 开发者，就职于阅文集团，负责基础业务与项目优化。
>
...
## WWDC21 10076 - 使用 Object Capture 创建 3D 模型
Apple 自 2017 年推出 ARKit 以来一直在推动增强现实技术在其各个平台上的应用和发展，但 AR 内容的匮乏与高成本一直是制约开发者们投入 AR 的重要因素。在本次 WWDC 上，Apple 将 3D 模型的构建效率提升了到了前所未有的高度，RealityKit 2 框架中的新 Object Capture API 为我们提供了一种非常便捷的方式来为生活中的物体创建 3D 模型，而你需要的仅仅是为目标物拍摄几张照片。此外，配合 AR Quick Look 可以轻松将创建好的模型集成到你的应用之中。
...
![](https://images.xiaozhuanlan.com/photo/2021/374484e8fdd61fb2a827f54a18b46390.png)

### 拍摄照片
创建模型的第一步是为你的目标物拍摄全角度的照片，拍摄设备可以是 iPhone 、 iPad 、单反相机、微单相机甚至无人机。确保你的照片足以清晰的覆盖到目标物的所有角度。如果你使用 iPhone 或 iPad 进行拍摄，还可根据立体深度数据复原物体的真实尺寸，并能利用重力方向自动让模型朝上。
...
如果你导出了 **Reduced** 或 **Medium** 级别的 USDZ 模型，便可以在 iPhone 或 iPad 上使用 AR Quick Look 快捷地将模型展示到你的 App 或网页中。
...
![](https://images.xiaozhuanlan.com/photo/2021/75a53890ee53b31bc541bb83f5f40ec6.png)
...
此外，Object Capture 还提供更“高阶”的初始化方式，即除了图片素材外，你还可以额外提供一系列元数据，如深度缓冲、重力信息等，这些元数据在 Photogrammetry 中被称为 **Sample** ，将会影响 Object Capture 的构建算法，具体提供的 Sample 序列如下：

![](https://images.xiaozhuanlan.com/photo/2021/7a7a9905726989653b128d85479276a3.png)
...
#### 不同类型的请求
**Model File Request** 是最常用的请求，提供两种类型并根据你传入的 URL 地址参数来进行区分，如果你直接传入了一个以 USDZ 为后缀名的文件路径，那么会直接输出 USDZ model 文件；如果你传入的是一个文件夹路径，则会输出 USDA + OBJ 文件以及素材文件夹。
...
最后，**Model File Request** 的创建还提供一个可选参数，类型是 `PhotogrammetrySession.Request.Geometry`。用来控制模型的显示范围以及对其进行缩放、旋转、平移等操作。
...
![](https://images.xiaozhuanlan.com/photo/2021/82d7b2a084b35ebe74ca99ccba656cc3.png)
...
![](https://images.xiaozhuanlan.com/photo/2021/39d549c6d9db7902c8fd7ca6dbe49f2e.png)

在左边的操作界面可以看到，点击 **Folder** 图标选择素材，点击 **Output** 图标选择输出路径。在左下角使我们前面介绍的 **detail level** 选项，而两个条形按钮分别是 **Preview** 和 **Create Model**。而我们点击 **Preview** 按钮，过一会就会生成预览模型了，需要注意的是，这里生成的是原始预览模型，即可以理解为发起了一个 `geometry` 为空的 **preview request**：

![](https://images.xiaozhuanlan.com/photo/2021/82f8ce76286c525fe87522f698d2750e.png)

接下来就可以在预览界面上直接对模型的 `geometry` 参数进行调整了，`geometry` 参数内部由 `BoundingBox` 和 `Transform` 两个结构体组成。`BoundingBox` 的作用是控制模型的显示范围，可以看到右边的预览模型正处在一个 bounding box 中，通过上面的锚点，可以对 bounding box 的大小进行调整，只有身处 box 内部的模型会被保留下来。而负责对模型进行缩放、平移、旋转等操作的 `Transform` 参数，也可以直接在预览中进行。调整过程中会不断生成新的 `geometry` 参数并发起 **preview request** 。

最后，选择你所需的 **detail level** ，配合我们在预览过程中调整生成的 `geometry` 参数，点击 **Create Model** 按钮发起 **Model File Request**，最终的成果会保留在你的输出路径下。
...
* 表面尽量少一些反光的部分
* 如果需要翻转拍照，确保物体不会发生变形
* 如果表面有很精细的结构，那么你需要使用高分辨率的相机进行拍摄
...
> 根据不同目标物的特点，构建一个 3D 模型一般需要 20~200 张照片。此外，Apple 提供了一个用 SwiftUI 编写的 [CaptureSample App](https://developer.apple.com/documentation/realitykit/taking_pictures_for_3d_object_capture) ，让你用 iPhone/iPad 为 Object Capture 拍摄出高质量的照片。

### 输出合适的模型
我们之前提过，Object Capture 的输出模型可以选择不同的 detail level ，你需要根据自己的使用场景和对精细程度的要求来进行选择：

![](https://images.xiaozhuanlan.com/photo/2021/69f47dc4b67d861470f8204152fde3bb.png)

此外，不同的 detail level 下生成的模型，其贴图也是不同的：

![](https://images.xiaozhuanlan.com/photo/2021/2ae80e7fa497539712d11b66e0fb1d16.png)
...
![](https://images.xiaozhuanlan.com/photo/2021/64d91e517556cc8b70e4329004b2aa73.png)

## Object Capture + AR Quick Look
我们已经了解如何通过 Object Capture 来便捷的生成 3D 模型，如果你想同样便捷将自己的成果展示到 App 或网页之中，AR Quick Look 绝对是不二之选。AR Quick Look 是 iOS 系统内置的 AR 内容展示工具，同时支持 Safari 、 Message 、 Flie 等系统应用。
...
![](https://images.xiaozhuanlan.com/photo/2021/994463ab93467c030f94eb5f72dc02fa.png)
