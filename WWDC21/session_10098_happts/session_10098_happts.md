...
Tag 实体也是类似操作。这样设置后，Photo 就可以在 Spotlight 中通过 `userSpecifiedName` 被索引，Tag 可以通过 `name` 被索引。
...
调用 `startSpotlightIndexing` 来使 `SpotlightDelegate` 能够开始工作。
...
为了更精准地控制 `spotlightDelegate`  何时执行索引工作，框架提供了 `stopSpotlightIndexing` 方法。协同使用这两个方法，在必要时开始和停止索引。比如在你的应用程序正在执行密集的CPU或磁盘活动操作时，可以先停止索引，待操作完成后再开启索引。
