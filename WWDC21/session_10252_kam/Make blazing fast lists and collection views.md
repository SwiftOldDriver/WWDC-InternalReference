...
`UICollectionCellRegistration` 是在 iOS 14 上推出的 cell 注册及配置 API，通过声明一个泛型结构体就能包揽 register cell，configure cell 等工作。不同类型 cell 的注册配置能分割开，不会在 cellProvider 闭包中糅合成一团，让 `UICollectionView` 的 API 更加现代化。
