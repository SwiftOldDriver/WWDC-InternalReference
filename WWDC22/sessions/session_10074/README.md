...
2. 如果窗口是浮动面板（floating panel)、或模态窗口（modal window）或 `toolbarStyle` 的值为 `.preference`，Stage Manager 默认不会收起这样的窗口。
...
> Session 中提到，在以往开发者可能会使用 `NSSegmentedControl` 实现类似 `NSComboButton` 的按钮控件。在 Mail.app 中，有多和 `NSComboButton` 行为一致的按钮，例如 toolbar 上的「过滤」和「旗标」按钮。在 macOS Ventura Beta 1 中，这几个按钮仍然是使用 `NSSegmentedControl` 实现的。
