...
>
> - 如果你使用 SwiftUI 或 Mac Catalyst 开发了 Mac 应用，同时希望了解 WWDC22 中相关技术的演进，可以参考下面的 Sessions：
>
...
>
>   本文在结尾处对 AppKit 和其他 Apple 的 Mac 应用框架之间的关系进行了介绍，并给出开发者选择这些框架的一些建议。
>
...
AppKit 最初是 NeXTSTEP 操作系统的图形应用框架，其很多设计理念在当时具有前瞻性，例如面向对象的设计和 Interface Builder（后被集成到 Xcode 中）。AppKit 随 Apple 收购 NeXT 进入到 Mac OS X 操作系统中，并演进至今。

![AppKit on NeXTSTEP](./images/appkit-on-nextstep.png)

> Mac OS X 的图形界面也被称为「Aqua」。除了 Apple 的 Aqua 实现以外，AppKit 还有其他的实现，例如目前仍在活跃维护的开源实现 GNUstep（OpenStep 的继承者）。

AppKit 是面向基于鼠标和键盘交互的桌面电脑设计的，随着 Mac OS X 的诸多历史版本的迭代沉淀了很多独特且创新的桌面端交互模式，与此同时也发展得庞大且复杂。开发者通常需要特别细心并配合一定的经验技巧才能正确使用它。

#### UIKit 和 Mac Catalyst

UIKit 是 Apple 以 AppKit 为基础。针对基于触摸的交互的移动设备和进行简化和重构而成的应用框架。UIKit 的设计与 AppKit 有诸多的共性，但去除了历史糟粕，使开发者更容易学习和使用。

通过 Mac Catalyst，基于 UIKit 的 iOS 应用只需要少量的适配工作即可运行在 Mac 上。UIKit 在 Mac 的实现侧重窗口内的交互实现，完成了例如控件的绘制等核心能力，但在对 Mac 特有交互的特性的支持上有所欠缺，例如多窗口、浮动面板和状态栏菜单项等。

iPad 的发展使得 UIKit 突破了原有的设计，和 AppKit 的一些交互能力开始重叠。例如基于键盘和鼠标的交互、上下文菜单、快捷键和拖拽操作等。个人认为 UIKit 与 AppKit 未来在视觉设计上可能会趋同，两个框架会更进一步地融合（尤其是在 Mac 上），但两者不会相互替代。

#### SwiftUI

SwiftUI 通过声明式和响应式设计大大降低了图形界面代码的开发和维护难度，同时具有很好的跨平台特性。SwiftUI 对不同平台之间的交互概念进行了统一，能够大幅降低开发 Mac 应用的门槛。SwiftUI 也是 Apple 目前主推的构建 Mac 应用程序的方案，可以预见未来会出现大量使用 SwiftUI 的 Mac 应用。

相比 AppKit 框架，SwiftUI 目前非常年轻，实现细节和 API 接口可能还在不断的变动中。目前 SwiftUI 足以搭建常规 Mac 应用的基本界面结构，但对于一些的复杂交互场景，仍然需要 AppKit 提供的细粒度 API 才能胜任。

> 值得注意的是，SwiftUI 的控件 Mac 上可能可能会以原生 AppKit 控件实现，也可能完全以 SwiftUI 来绘制。例如本文提到的 `Form` 在 `.grouped` 样式直接通过 SwiftUI 进行绘制，而在 `.columns` 样式下会使用 AppKit 的控件实现。
>
> ![SwiftUI Form Styles](./images/swiftui-form-styles.png)
...
AppKit 仍然代表着 macOS 区别于其他平台和系统的核心体验，在短时间内很难被取代，也是原生 Mac 应用开发者必须学习和掌握的框架。

### 总结

本文详细介绍了 AppKit 在 WWDC22 中的演进，包括 Stage Manager、全新的系统设置、控件更新、SF Symbols 4 以及共享相关的新特性。其中 Stage Manager 能够给用户带来更沉浸的交互体验，而对 `NSTableView` 和 `NSToolbar` 等控件的更新解决了以往开发者的痛点。同时，开发者只需要进行简单适配，就能使用全新设计的分享面板。

相比于 UIKit 和 SwiftUI 等新兴的应用框架，AppKit 的发展显得有些疲态，Apple 对其的投入也不及其他框架。尽管如此，AppKit 的这些特性变化，映射了 Apple 对于跨设备、跨平台体验一致性的布局，也包含对 Mac 特有的桌面体验的持续改进。
