...
> 值得注意的是，通过 Xcode 的 View Debugging 不难发现，新的设置样式是完全使用 Swift UI 绘制的，没有使用任何 AppKit 的原生交互控件。开发者使用 AppKit 难以还原出一致视觉和交互效果。
...
Alert 的样式得到了优化，当其中的文案很长时，或其包含的 `accessoryView` 宽度很大时， `NSAlert` 会自动进行使用类似于 macOS Big Sur 之前的样式，该样式横向布局按钮，同时让其内容更容易阅读。开发者无需针对该特性做任何适配。

![Style Updates to Alerts](./images/alert-style-update.png)

## `NSTableView` 性能提升

在过去，`NSTableView` 的实现会预先计算自身的总高度和每一行的高度，当 tableView 的每一行高度不一致时，这会使得首次加载的性能较差。macOS Ventura 对这一项行为进行了优化，`NSTableView` 现在会根据滚动区内和附近的行的高度动态计算出预估的行高（running estimated height），并在滚动过程中根据实际需求动态请求实际的行高并对预估的行高进行替换，同时正确维护每一行的滚动位置。

当 tableView 的内容行数很多时，这项优化可以大幅提升其加载性能，但这项优化会改变 `NSTableViewDelegate.tableView(_:, heightOfRow:)` 的调用时序。因此，开发者不应当对 `NSTableView` 请求行高的时机做假设。

这项优化将自动在 macOS Ventura 上对全部的 `NSTableView` 和 Swift UI `List` 生效。
