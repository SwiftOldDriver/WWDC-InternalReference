...
AppKit 构建了 macOS 的核心用户体验，是大多数 Mac 应用开发者使用最多的框架之一。AppKit 框架拥有相当长的历史，Apple 对其的演进代表着对 macOS 交互的体验的思考和期望。和 WWDC21 一样，WWDC22 对 AppKit 框架的更新对 macOS Big Sur 以来的新设计语言进行了进一步的完善，同时也包含了平台一致性相关的演进。
...
>
> - 如果你使用 SwiftUI 或 Catalyst 技术开发了 Mac 应用，同时希望了解 WWDC22 中相关技术的演进，可以参考下面的 Session：
>
...
当用户开启 Stage Manager 时，打开新的窗口会使当前交互的窗口被收起。Stage Manager 没有引入新的公开 API，开发者可以通过现有的 API 使窗口正确适配 Stage Manager 的行为：

1. 窗口（`NSWindow`）的各项特性将遵从原有的 API 行为。

2. 打开一个新窗口时，如果被打开的窗口是浮动面板（floating panel)、或模态窗口（modal window）或 `toolbarStyle` 的值为 `.preference`，Stage Manager 不会收起当前活跃的窗口。
...
3. 如果窗口的 `collectionBehavior` 包含了 `.fullScreenAuxiliary`、`.moveToActiveSpace`、`.stationary` 或者 `.transient` 的任何一个，则 Stage Manager 不会对其进行管理。
...
> 在 macOS Ventura Beta 1 上，自定义安装的设置面板都没有显示到侧边栏上，该问题在 macOS Ventura Beta 2 得到修复。
...
![Preferences Renamed to Settings in the Main Menu](./images/new-settings-in-macos-ventura.png)
...
相较于仅限于 macOS 平台的 AppKit，SwiftUI，具有天生的跨平台属性。使用 SwiftUI `Form` 编写的表单界面可以自动适配包括 iOS、WatchOS 和 tvOS 在内的全部 Apple 平台。

![SwiftUI Form on iOS and Mac](./images/swiftui-form-on-ios-and-mac.png)

> - `FormStyle` 和 `.formStyle(_:)` 是 SwiftUI 在 WWDC22 上的新 API。`Form` 的优化仅仅是 WWDC22 对 SwiftUI 的多项改进的一部分，详细的内容可以参考 [Session 10052 - What's new in SwiftUI](https://developer.apple.com/videos/play/wwdc2022/10052)。
> - 尽管 Session 示例使用了 `FormStyle.insetGrouped` macOS Ventura Beta 的 SDK 提供的对应枚举名是 `FormStyle.grouped`。

> 值得注意的是，通过 Xcode 的 View Debugging 不难发现，新的设置样式是完全使用 SwiftUI 绘制的，没有使用任何 AppKit 的原生交互控件。开发者使用 AppKit 难以还原出一致视觉和交互效果。
...
`NSComboButton` 有 `.split` 和 `unified` 两种样式。`.split` 样式的将按钮呈现为一个普通按钮、分割线和一个下拉按钮，而 `.unified` 样式的按钮看起来和普通按钮无异，但长按时可以弹出对应的操作菜单。
...
> 本 Session 中提到，在以往开发者可能会使用 `NSSegmentedControl` 来实现类似 `NSComboButton` 的控件。在 Mail.app 中，有一些按钮与 `NSComboButton` 提供的行为一致，例如 toolbar 上的「过滤」和「旗标」按钮，但实际上在 macOS Ventura Beta 中，这些按钮仍然是使用 `NSSegmentedControl` 实现的。

### `NSColorWell` 新设计

`NSColorWell` 的默认样式得到了重新的设计，其样式与其他控件统一，同时 `NSColorWell` 得到 `.minimal` 和 `.expanded` 两种新样式，这两种新的样式自带下拉式菜单。

![NSColorWell Styles](./images/color-well-styles.png)

`NSColorWell` 的下拉菜单可以通过独立的 `pulldownTarget` 和 `pulldownAction`，来自定义其行为。

### `NSToolbar` 新特性

#### `NSToolbarDelegate` 的新增方法

- `NSToolbarDelegate.toolbarImmovableItemIdentifiers(_:)` 可以用来指定不可被移动的 toolbar 元素。

  ![NSToolbar with Immovable Items](./images/toolbar-immovable-items.png)

- `NSToolbarDelegate.toolbar(_:itemIdentifier:canBeInsertedAt:)` 允许开发者细粒度地控制 toolbar 元素的顺序，当用户在拖动 toolbar 上的元素进行排序时，AppKit 会持续通过该委托方法确认元素是否允许被放置在对应的下标位置。
...
- 在以往，toolbar 元素的文案变化会导致元素的整体尺寸发生变化，为了避免这一现象的发生，开发者现在可以通过 `NSToolbarItem.possibleLabels` 预先指定元素可能的文案，AppKit 会预先计算出最大长度并在布局时留出空间，从而避免元素的大小随文案发生变化。

  ![Toolbar Item with Possible Labels](./images/toolbar-item-with-possible-labels.png)

## `NSAlert` 的新设计

Alert 的样式得到了优化，当其中的文案很长时，或其包含的 `accessoryView` 宽度很大时，`NSAlert` 会自动进行使用类似于 macOS Big Sur 之前的样式，该样式横向布局按钮，同时让其内容更容易阅读。开发者无需针对该特性做任何适配，值得注意的是，动态更新 alert 的内容不会导致其样式的切换。

![Style Updates to Alerts](./images/alert-style-update.png)

## `NSTableView` 性能提升

在过去，`NSTableView` 的实现会预先计算自身的总高度和每一行的高度，当 table 实例的各行行高度不一致时，首次加载的性能会比较差。macOS Ventura 对这一项行为进行了优化，table 现在会根据滚动区内和附近的行的高度动态计算出预估的行高（Running Estimated Height）。Table 在被滚动的过程中会根据需要动态请求实际的行高，并对预估的行高进行替换，同时正确地维护每一行的位置。

当 table 的内容行数很多时，这项优化会大幅提升其加载性能，但这会改变 `NSTableViewDelegate.tableView(_:, heightOfRow:)` 委托方法的调用时序。因此，开发者的业务逻辑不应当依赖 table 请求行高的时机，或者进行其他的一些 tricky 的操作。

这项优化将自动在 macOS Ventura 上对全部的 `NSTableView` 和 SwiftUI `List` 生效。
...
SF Symbol 中的符号有包括单色、分层、调色盘和多色在内的四种渲染模式。SF Symbol 4 为每一个符号都拥有一个偏好的渲染模式，指定的模式会被 AppKit 自动选用。开发者可以选择使用 `NSImage.NSImageSymbolConfiguration` 对其进行覆盖。

### 可变符号（Variable Symbols）

SF Symbol 4 新增了可变符号。一些符号例如 Wi-Fi 信号和音量等可能被用来传达数值。现在，可以直接通过 0-1 之间的浮点数值来构造 `NSImage` 来得到用于表达数值的符号图片。
...
对于不支持这项特性的符号，`variableValue` 数值将被忽略。
...
macOS Ventura 重新设计了分享面板，相比原先的分享菜单，新的分享面板和 iOS 上的样式更类似，例如列出一些建议的接收者。`NSSharingServicePicker` 原有的能力将保持不变。

![The New Sharing Picker](./images/new-sharing-picker.png)

分享 URL 时，分享面板会自动在标题区域显示对应的文件名、类型、标题和图标。对于自定义的分享项，开发者可以通过新协议实现新协议 `NSPreviewRepresentableActivityItem` 来指定分享面板标题区域的内容。
...
在以往，一些应用可能需要在菜单中进行分享，对于这种场景，开发者通常会实现自定义的子菜单用来分享。现在，通过使用 `NSSharingServicePicker.standardShareMenuItem`，应用可以直接通过菜单项打开标准的分享面板，对于上下文菜单，分享面板会自动指向菜单对应的视图：
...
> 除了对分享面板的改进，WWDC22 还提供了很多与协同工作有关的新特性，这些特性在下面的 Session 中有详细的介绍：
>
...
本文详细介绍了 AppKit 在 WWDC22 中的演进，包括 Stage Manager、全新的系统设置、控件更新、SFSymbol 4 以及共享相关的新特性。AppKit 作为 macOS 平台特有的应用框架，经历了诸多版本的历史迭代后，已经非常庞大和复杂。开发者通常需要特别细心并配合一定的经验和技巧才能正确使用它。相比于 UIKit 和 SwiftUI 等新兴的应用框架，AppKit 的发展显得有些疲态，Apple 对其的投入也不及其他框架。

尽管如此，AppKit 的这些特性变化，映射了 Apple 对于跨设备、跨平台体验一致性的布局，也包含对 Mac 特有的桌面体验的重视。SwiftUI 和 Catalyst 技术已经能够大大降低构建 Mac 应用的门槛。SwiftUI 是 Apple 主推的构建 Mac 应用程序的方案，可以预见未来会出现大量使用 SwiftUI 开发的 Mac 应用。但 AppKit 仍然代表着 macOS 系统区别于其他平台的核心体验，也是体验优秀的原生 Mac 程序绕不开的应用框架。
