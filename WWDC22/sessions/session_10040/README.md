...
> 以上接口的用法，可以参考我们之前的文章：[IAP 用户退款与客诉处理优化](https://mp.weixin.qq.com/s/MtytymgkcP3_oAH7JyI1og)。
...
| bid | Bundle ID，套装 ID | 您的 app 的套装ID，即 app 包名 |
...
- 提供改进管理和跟踪 IAP 的能力。
- 适用于所有的客户端，兼容 StoreKit v1 和 v2 版本，并提高监控订阅生命周期的能力。
- 以上功能在 Sandbox（沙盒） 和 Production（生产）环境中都可用，并且是开发者现有服务和系统的重要补充。

## 总结

从去年推出 StoreKit v2、App Store Server API v1 和 App Store Server Notifications v2，到今年的优化完善，都是开发者们一直以来期待的功能，现在苹果已逐步完善应用内购买（IAP），相信大家都有所感受。另外，随着应用市场的存量饱和，竞争越来越激烈，获取一个新用户或者挽留一个付费用户的成本很高，所以，苹果也希望开发者能根据这么新 API 的特性，尽早的推进服务端使用这些新的 Server API，对于获得用户的行为和感知体验，有更敏捷的洞察力和更准确的预流失风险评估等。总之，善待每一个用户，提升用户体验，是突出重围和赢得用户的绝佳方式。
