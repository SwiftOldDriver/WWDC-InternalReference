...
为了更好的理解为什么 WebKit，我们需要一些基础知识，认识一下 WebKit  是做什么的，以及解决我一直以来听到的某个论点—— `iOS` 和 `Android`  的浏览器内核都是 WebKit。
...
历史轮转，微软这边，IE 一度落后于其他主流浏览器。于是推出了 Edge 浏览器，使用 EdgeHTML 和 Chakra 作为排版引擎和 JS 引擎。但仍然没有太多起色。之后微软宣布将 Chakra 开源，新的 Edge 转为 Chromium 内核。
...
在详细介绍相关内容之前，需要你有一定的 HTML 和 CSS 基础知识：
