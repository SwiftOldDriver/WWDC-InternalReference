...
今年最值得瞩目的是网页检查器的更新是 —— [Create Safari Web Inspector Extensions](https://developer.apple.com/videos/play/wwdc2022/10100)，开发者使用最新的 Web 扩展 API 将自己的工具直接添加到 Web 检查器中
...
最令人瞩目好就是 Web Push 的支持。使用 Web Push 可以提升 macOS 上的 Safari 中网站和 Web 应用程序的通知体验，不过 iOS 和 iPadOS 得在下一个版本才能体验。

 具体技术细节参照 [Meet Web Push for Safari](https://developer.apple.com/videos/play/wwdc2022/10098)
...
在苹果设备上将网站添加到主屏幕上，其展示的图标优先级最高的是在`<head></head>` 区域添加  `apple-touch-icon`，如果你需要再不同的苹果设备上展示不同的网站图标，这是唯一可能满足条件的方式。
...
参照 [Broadcast Channel API](https://developer.mozilla.org/zh-CN/docs/Web/API/Broadcast_Channel_API)，通过构造 [`BroadcastChannel`](https://developer.mozilla.org/zh-CN/docs/Web/API/BroadcastChannel) 来简单地“订阅”特定频道，并在它们之间进行全双工（双向）通信。
