...
不同的广告有不同的转化目标，比如对于电商广告来说下单购买是转化目标，对于一个 App 的推广广告来说，下载安装就是转化目标了。

我们要介绍的 SKAdNetwork 就是对 iOS App 下载安装进行归因的框架。

在介绍 SKAdNetwork 之前，我们先来看看下载归因的一些相关概念。

#### **链路式归因**和**匹配式归因**

广告归因存在**链路式归因**和**匹配式归因**两种方式。

> **链路式归因**：通过在广告的关键链路上带上一个唯一的追踪 id（比如广告 id），这个追踪 id 会将广告的曝光、点击、跳转到落地页以及转化都串联起来，实现广告链路的完整归因，链路闭环。
>
> **匹配式归因**：在广告曝光、点击时获取设备 id（比如 IDFA ）进行上报，在广告转化时也将设备 id 进行上报，两侧信息对比匹配上即完成归因。
>

链路式归因的关键是要在广告链路上透传追踪 id。

对于前面小米手机广告的例子，通过广告链接里的广告 id 来进行归因的方式就是链路式归因。

![](./images/chain_attr.png)

对于 iOS 上 App 下载安装场景的归因，通过广告 id 串联整个链路无法实现，因为在用户点击广告去到 App Store 下载安装广告主 App 这一步时，广告 id 无法透传过去。

这个时候我们就需要用到匹配式归因：

![](./images/mapping_attr.png)

匹配式归因对获取的设备 id 有严格的要求，需要有唯一性、持久性，并且容易获得，在 iOS 上一般采用 IDFA 作为设备 id。

App 下载的场景目前主流的两种方案：

* 基于 IDFA 的匹配式归因
* 基于 SKAdNetwork 的链路式归因

> 更多的方案可以参考 [AppsFlyer 归因模型](https://support.appsflyer.com/hc/zh-cn/articles/207447053-AppsFlyer)
...
IDFA 匹配的过程如下：
其中关键步骤就是曝光、点击、和转化(对应图中 2/4/7 )时上报设备 IDFA，由归因平台进行匹配归因。
![](./images/IDFA_mapping.png)
...
> SKAdNetwork 里 SK 代表 StoreKit，其仅适用于 App 下载场景，对于其他场景的广告归因(比如下单、购买)不支持。
...
> 如何让一滴水永不干涸？答案是放到大海中
 ...
 人群匿名性是 Apple 用来指代 SKAdNetwork 提供归因数据的隐私保护方式的术语。
 安装次数决定了对用户隐私的保护级别。听名字感觉很高级，实际上其概念非常简单：

在安装人数很少时，Apple 发回的归因数据将是比较"模糊"的，避免用户信息被跟踪；

随着安装人数的增加，发回的归因数据会变得精确起来，在这种情况下用户的隐私依然得到了保护。

Apple 根据安装人数将人群匿名性分为低/中/高三个级别，下面要介绍的分层源标识符、分层转换价值就会受人群匿名性层级的影响。

![](./images/crowd_anonymity.png)

### 分层源标识符 (Hierarchical IDs)

> 活动标识符(campaign identifier)：是一个数字，取值范围0~99，用于区分不同的广告营销活动。
>
> 这是我们唯一可以用来标识广告的值，如果广告网络的广告超过100个，则将无法满足归因需求。

SKAdNetwork 4.0 将活动标识符 (campaign identifier) 字段由两位数字扩充至四位数字，并且重命名为**源标识符**(source identifier)。

广告网络可以根据广告目标定义这个四位数的意义：例如，活动价值、广告位置或广告创意类型。

需要注意的是，为了保护隐私， 源标识符发送数字的位数和人群匿名性层级相关：
...
> 什么是转化值？
>
> 转化值就是一个数字，取值范围0~63，可以用来表示用户下载 App 后的一些转化行为。
>
> 其值的意义是可以自己定义的，比如可以定义用户登录账号为值1，购买会员为值2...
>
> 当用户发生对应的转化行为时，我们通过 SKAdNetwork 更新相应的转化值。当 Apple 回传归因数据时会带上该转化值， 我们可以通过转化值衡量不同广告的转化效果。
