...
人像画面在直播、视频会议等业务都有不错的运用场景；但画面的质量的上限受限于摄像头硬件，在没有特殊需求下，我们往往使用的就是设备内置或一般的网络相机。在 WWDC22 Session 10018 中，Apple 推出了 Continuity Camera（连续互通相机）-- 将 iPhone 与 Mac 连接后，iPhone 的后置摄像头会作为外设摄像头被 Mac 使用，向用户提供一个高质量的智能摄像头。

本文的内容分为三个部分:

- Continuity Camera（连续互通相机）的前世今生。
- 「优雅」的相机体验--自动切换。
- 深入 AVFoundation 的新 API。
...
正如在开头提到的，如果没有强需求，我们不常会因为居家办公视频会议而买一个网络摄像头连接到我们的电脑上，有内置用内置，无内置就两手一摊。

而从下面的数据上看，不难发现，笔记本相机的质量远不及手机的后置摄像头：

- Microsoft Surface Laptop Studio: 1080p 前置摄像头
- iMac 24: 1080p FaceTime 高清摄像头
- iPhone SE: 1200 万像素广角、最高可达 5 倍数码变焦、各种特效

结合来看，对于一个拥有新款 iPhone 的用户，除非去购买昂贵的高质量网络摄像头，否则很难在 Mac 上获得超过连续互通相机的人像画面体验 -- 且不提连续互通相机并不需要额外的花费。

#### 本来就难以触达的 iPhone 后摄

那么，在一开始就使用 iPhone 的后置摄像头不就好了吗！但很遗憾这是非常困难的：

1. 如果想要让 Mac 将 iPhone 识别为视频输入，就需要用到采集卡或 Quick Timer Player，且输入的是屏幕上的内容，无法调整分辨率等参数，十分不便。

2. 结合本地投屏技术，将 iPhone 后摄采集到的视频数据发布出来，再被 Mac 上的软件捕获显示；这种方法需要在 iPhone 和 Mac 上都开启额外的软件，且根据实现有安全隐患。

而无论使用哪种方法，都无法直接和视频会议等其他软件结合，仅能从理论上达到连续互通相机的能力；且不说连续互通相机不仅仅只是提供了摄像头硬件而已。

如果你觉得，「将移动设备连接至电脑上，并能够直接作为外置摄像头使用，无需任何额外的开发。」是一个浪漫的功能、务实的需求，那这就是 Apple 在自己的生态之中所做到的。

#### 优秀的连续互通相机

除了摄像头内容外，连续互通相机还提供了 iPhone 支持的一些视频特效：

1. Center Stage 中央舞台：借助超广角镜头使一定范围内移动的人体能够始终在画面的中间。

2. Portrait 人像模式：虚化背景，突出人像

3. Studio Light：在光照条件恶劣的场景调整画面的明暗。

如果你为如何在视频会议中向同事展示自己的手或者桌面上的草稿而苦恼、手忙脚乱，那么 Apple 提供了一种新的“特效”来解决这个问题。

Desk View：借助超广角镜头捕捉显示相机前下方的内容，类似高架机位。
...
1. Center Stage 借助于超广角镜头，能够让不断移动位置的人物一直处于视频画面的中间位置。
...
如果想和同事共同工作并共享桌面上的内容，现在可以使用 Desk View（桌面视图）。Desk View 应用程序随 macOS 13 一起提供，可以在控制中心的启动。

![](./images/wwdc2022-10018_hd-0005.png)

它就像一个高架机位的摄像机，但不需要复杂的设备。 iPhone 会将超广角镜头画面分成两部分，同时展示桌面和脸的画面；这样就可以在不调整摄像头位置的情况下，既展示人像，又可以展示一个类似投影仪的画面，在需要展示现实中的演示内容（例如：习题、绘图、特定物品等）相较于电子白板更加灵活；有意思的是，视频中虽然演示的是在桌面上的纸张上写字，但是在举例上提到了「编织针法」，这意味着 Desk View 的画面识别并不仅仅受限于背景分割十分明显的画面，可以预料到通过后续的算法迭代，这个功能对于画面中主要的部分捕捉会更加自由。

它利用超广角相机来扩展垂直视野，将透视失真校正应用于裁剪每一帧，然后旋转画面来创建桌面视图。用户可以使用大多数视频会议应用程序中提供的共享窗口功能来获得桌面的视频源，并且与主摄像机的视频源并行运行。

> 由于 Desk View 实际会在 API 层产生一个新的 `AVCaptureDevice`，所以对于大部分软件来说相当于多了一个视频源，即便这是来自同一个外置设备；因此可以做到无需一行代码的改动就享有这个能力。
...
刚刚讨论了开发者无需在应用程序中编写一行新代码即可获得的所有出色体验。但是，通过采用一些新的 API，可以让连续互通相机的体验在应用程序中更加神奇和精致。
...
`userPreferredCamera` 是一个读/写属性。每当用户在应用程序中选择相机时，都需要设置此属性。这允许 `AVCaptureDevice` 类了解用户的偏好，在应用程序启动和重新启动时为每个应用程序存储相机列表，并使用该信息来建议相机。它还考虑到是否有任何相机连接或断开连接。此属性是键值可观察的，并根据用户偏好智能地返回最佳选择。当最近的首选设备断开连接时，它会自动更改为列表中的下一个可用摄像机。即使没有用户选择历史记录或未连接任何首选设备，它也会始终尝试返回可供使用的摄像头设备，并优先考虑曾经传输过视频流的摄像头。仅当系统上没有可用的摄像头时才返回 `nil`。
...
即便是在开启自动选择模式下，也需要保留 UI 让用户可以手动选择自己的摄像头；同时，在关闭自动选择模式时，依然希望应用程序在用户选择摄像头时对 `userPreferredCamera` 进行赋值以便能够记录用户的使用习惯，并在下次开启自动选择模式时运用。
...
从这里可以发现，Apple 不仅将 iPhone 连接到 Mac 后提供了 iPhone 摄像头的视频源，从而为 Mac 用户提供一个触手可得的高质量网络相机；同时也包含了分布式计算的思想：将视频特效、相机控制等过程放在 iPhone 端上进行，降低 Mac 的算力依赖，提高其他硬件的利用率。
...
Apple 还在 macOS 上提供了 `AVCaptureMetadataOutput`，以允许处理捕获中的会话生成的定时元数据。现在可以从 iPhone 中流式传输面部元数据对象和人体元数据对象，这些数据都可以运用在特效等方面。
...
此外，结合 ScreenCaptureKit，可以实现一个 Apple 生态下、更加高效、功能丰富的 OBS/视频会议软件，并且受惠于触手可得的高质量的硬件和算法而无需专门的适配。相关内容可以参考：[Meet ScreenCaptureKit](https://developer.apple.com/videos/play/wwdc2022/10156/) （Session 10156）。

### One More Thing

如今元宇宙大火大热，而借助连续互动相机，能够让 Mac 获取到相机采集到的人脸和人体数据（`AVCaptureMetadataOutput`），这为虚拟人像和虚拟形象提供了更加亲民的动捕脸捕的能力。或许在系统正式更新不久就能看到使用连续互动相机的“虚拟主播软件”。
