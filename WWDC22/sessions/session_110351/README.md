...
前面我们介绍了 Swift 并发为了防止数据竞争做的很多检查，但这些检查只能保护某一个变量或者某一个实例对象的数据，这样的保护措施防止了一些底层数据竞争 ( low level data racing )。 如果写程序时在错误的时间位置挂起，仍然有可能某一组对象或变量与另一组对象或变量的数据不一致，这种情况就是顶层数据竞争 ( high level data racing)。
