...
首先，我们需要子类都重写 eat(food:) 方法，但如果子类没有重写，直到运行时才会被发现。
...
你会不会有个疑问，为什么这种方式出现的问题，都只能在运行时才会被发现呢？因为包含多态是一种动态多态，编译阶段是无法检测到这些问题的。
...
这看起来确实不错，用 Food 作为泛型的类型参数占位符，在子类声明的时候，再传入具体的类型。

不过我们离拥抱泛型还早着呢，这最多算是泛型初见。看看下面的代码有没有什么不妥之处吧。
...
不过一回生二回熟，哪里跌倒就在哪里爬起来。上一次我们就尝试使用过泛型，而且效果还不错，只是因为公式选对带错了数(用了类而不是协议)，这回我们还是用泛型试试。
...
some 正如它的字面含义那样，某一个，意味着这里的 animal 参数它是某一种动物，可以是奶牛，可以是马，也可以是鸡。我不关心它是什么，只要是某一种动物就行。
...
除了 body 属性将 some 用在返回值位置的类型修饰上，不透明类型还可以用在其他位置上。
...
当 some 修饰返回值位置的类型时，返回值的具体类型是由方法内部提供的，外部无需知晓。
...
用 some 关键字修饰方法参数位置的类型是 Swift 5.7 新增的特性，参数的具体类型是由调用方法的外部提供的，内部对其是一无所知。

在完成农场的 feed(animal:) 方法前，我们先来补全 AnimalFeed 协议。
...
在 feed(animal:) 方法中，通过 animal 的类型可以确定它的关联类型 Feed，也就是当前动物进食的食物类型，进而可以调用其叫做 grow() 的静态方法，种植对应的植物，然后调用这种植物的 harvest() 方法来收获对应的食物，最后将其喂给动物吃。
...
Swift 编译团队并不满足于将 any 设计在 some 类似的功能定位上，any 除了可以表示它字面意义上的任意一个，还加入了包含多态的特性。
...
我们可以把 any 修饰的类型想象为一个盒子，通常情况下，盒子里可以装得下这个类型的实例对象，但有时这个对象太大了，盒子装不下，可以在内存的其他位置给它分配空间，而盒子里只装着这个空间的地址，也就是用一个指针指向该空间。
...
在参数位置使用 any 修饰有关联类型的协议类型是 Swift 5.7 新增的特性。

![any_parameter_position](./images/any_parameter_position.png)

接下来，我们遍历 animals 数组，对每个动物实例进行喂食。

当我们对 animal 调用 eat(food: Animal.Feed) 方法时，编译器报错了，因为 any 不仅擦除了 animal 的类型，它的关联类型也被一并擦除了。

![any_parameter_position_wrong](./images/any_parameter_position_wrong.png)

我们要换个类型没有被擦除的方法去调用，也就是用 some 修饰 animal 参数类型的方法。
