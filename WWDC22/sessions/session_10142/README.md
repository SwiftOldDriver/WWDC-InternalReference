...
- 通过后台调度的 URLSession ，应用将会回到挂起状态并等待请求的完成；
...
为了达到这种效果，我们应该确保发起的请求是*后台网络请求*类型。这种类型的请求可以让我们立即完成 App 刷新任务，同时可以当网络请求完成时再次被分额外的后台执行时间。
...
> 并发特性是在 iOS 15+ ， Xcode 13+ ，及 Swift 5.5 后支持的一种新的语言特性，解决了异步代码造成的 completion\handler 之类的处理代码逻辑复杂，嵌套深等代码结构问题；同时更重要的是，使用了并发特性，在多核设备上更能发挥设备性能，例如一个 4 核心的处理器就可以同时处理 4 段标记了并发性的代码片段。
...
为了响应和取消处理程序的执行，我们可以使用 Swift 并发中内置的`withTaskCancellationHandler`函数，我们不在直接等待结果，而是将下载代码放到`withTaskCancellationHandler` 调用中，并等待这个结果。
...
至此，我们来简单总结下在实现这个 App 时，我们用到的技术点：
...
举个例子，就在几年前我们使用的一个三方 SDK 用到了后台任务，但是使用却是非常粗暴，申请了许多后台任务不说，还没有将任务和处理程序一一对应，这就造成了线上不断有客诉反馈“杀后台严重”。一开始我们也没有意识到是这个问题引起的，最后还是通过符号断点一个一个数才发现是这个 SDK 的锅。
