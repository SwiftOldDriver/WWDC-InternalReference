...
* 在 watchOS，iOS， tvOS，MacOS 和小组件中通用的系统 API
* `async`/`await` 与 Swift 并发性的关系
* 系统提供的优秀平台特性
...
我们将打造一个 Demo App， 该 App 在每天中午的时候会检查当时的天气情况，在特定的天气条件下会提醒用户进行拍照并上传图片。其业务流程如下：

1. 在特定时间以推送通知形式提醒用户拍照；

2. 用户点击该推送通知进入 App ，拍照并上传至服务器；

3. 上传成功后为用户发送另外一个推送通知

在整个过程中，有多个步骤涉及后台任务的执行和 App 挂起/激活状态的转换。
...
* 随后用户离开应用，此时应用处于*挂起状态*，系统将会以我们前面指定的时间再次唤起应用。系统会在午间唤起应用并同时发起一个后台刷新任务。但请注意，此时 App 状态变成*后台状态*；
...
到这一步，应用已经被唤起两次，同时也有了两次后台运行的机会。拿到请求结果后就可以判断当前外面的天气情况并决定是否需要提醒用户。
...
对于熟悉后台任务的同学应该非常清楚。其答案是系统会强制杀死应用，并且拒绝之后的后台任务请求。一个任务周期，应限制在 180s 以内。之所以苹果定义这样的的机制是出于对系统性能的考虑，因为在后台状态下仍有可能出现高负载的任务。其原则是后台任务不应耗费太多的系统资源，避免形成与前台任务的资源竞争，从而保障前台 App 的使用体验。

但是换个角度想想，如果在后台运行时间不足的情况下，系统会通知应用。这样就能让我们可以更优雅地处理后台任务，从而提高体验。

为了达到这种效果，我们应该确保发起的请求是*后台网络请求*类型。这种类型的请求可以让我们立即完成 App 刷新任务，同时可以在网络请求完成时再次分配到额外的后台执行时间。
...
在一个平常的 SwiftUI 应用中，增加了一个调度后台刷新任务的方法。

这个方法中我们创建了一个 `noon`这个日期对象，然后通过 `BGAppRefreshTaskRequest` 初始化了一个后台刷新请求并提交给了系统调度器。

值得注意的是这里我们为这个后台任务请求指定了一个"`StormyNoon`" 标识符，这个标识符后面我们还会用到。

以上步骤，我们就成功发起了一个在指定时间（明天中午）要被触发的后台应用刷新任务请求了。

只是发起了后台任务请求还不够，因为我们还需要处理由`系统-->后台`的过程。在这个过程中，我们需要执行回调任务。

这也很简单，如下：

![](./images/7.jpg)

注意到，这里我们就用上了上面的"`StormyNoon`" 标识符，这个标识符告诉了系统，当应用前面申请的后台任务被触发时，就会调用 `.backgroundTask 方法里面的闭包代码。

在这个后台应用刷新任务的回调中，我们应该实现天气状况检查和通知用户拍照的任务，代码如下：

![](./images/8.jpg)

为了在第二天能够继续被调度，在这段处理程序中，我们需要先调用 `scheduleAppRefresh` 方法再发起一个新的后台刷新请求，如此形成了一个周期重复性的定时任务。
...
我们使用了 `URLSession`的单例来初始化一个天气服务的请求。`URLSession`内置了并发性，并提供了一个异步下载数据的方法。有了这个方法我们就可以去服务器查询当前的天气状况了。

但是还记得前面提到的“后台任务周期”的概念吗？

网络是不可靠的，在一些情况下，请求的结果也不能确保能在执行周期内完成，怎么办？

答案就是，放弃使用 `URLSession` 的共享单例，并使用后台请求会话的配置来初始化一个新的 `URLSession` 对象。

![](./images/11.jpg)

通过把 `sessionSendsLaunchEvents` 属性为 `true` 来初始化一个新的 `URLSession` 对象。这样做的目的是告知系统，网络请求在应用挂起时需要继续运行，并在完成时唤醒 App。
