...
| 序号 | 功能点                        | 详细介绍                                                                                                                                                                                                                                                                                                                                   |
| ---- | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1    | 当前天气（currentWeather）    | 包括时间（date）、过期时间（expirationDate）、天气状况（condition）、温度（temperature）、风、能见度、紫外线指数、日出日落、降水强度、湿度、气压等数据                                                                                                                                                                                     |
| 2    | 每分钟预报（minuteForecast）  | 包括降水类型、降水概率、降水强度等数据                                                                                                                                                                                                                                                                                                     |
| 3    | 每小时预报（hourlyForecast）  | 包括预报小时数量（hours count）、时间范围（date range）、第一个预报小时的天气状况（first condition）、第一个预报小时的温度（fist temperature）、最后一个预报小时的天气状况（last condition）、最后一个预报小时的温度（last temperature）、湿度、气压、紫外线指数、风、降水等数据                                                           |
| 4    | 每日预报（dailyForecast）     | 包括预报日数量（days count）、时间范围（date range）、第一个预报日的天气状况（first condition）、第一个预报日的最高温度（first hight）、第一个预报日的最低温度（first low）、最后一个预报日的天气状况（last condition）、最后一个预报日的最高温度（last high）、最后一个预报日的最低温度（last low）、降水、日出日落、风、紫外线指数等数据 |
| 5    | 极端天气警报（weatherAlerts） | 包括受影响地区、严重程度、描述等数据                                                                                                                                                                                                                                                                                                       |
| 6    | 可用状态（availability）      | 包括每分钟预报（minute availability）、极端天气警报（alert availability）、空气质量（air quality availability）等数据                                                                                                                                                                                                                      |
| 7    | 历史天气                      | 包括气温、紫外线指数、风、降水强度、体感温度、湿度、能见度、气压等数据                                                                                                                                                                                                                                                                     |

> 详细介绍中带括号的数据为主要数据，通过 `String(describing: subject)` 得知。
>
> 每分钟预报和极端天气警报，测试了多个地点均无法返回对应数据，因此无法得知哪些是主要数据。
>
> 可用状态，文档及代码提示并不存在 `airQualityAvailability`，但打印实例可以查看该属性，猜测后续版本提供。
>
> 历史天气，在 Swift 中暂不提供，需要使用 REST API 进行查询。
