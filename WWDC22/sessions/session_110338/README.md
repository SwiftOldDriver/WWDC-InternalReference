...
类似地，配置快进和快退指令。该指令适用于大多数播放内容，但像是直播流无法快进的媒体除外。
...
另外，在带广告的视频中，可能不希望允许用户快进广告，这个场景中，可以通过禁用 `skipForwardCommand` 来实现暂时禁用指令的需求。
...
`MPRemoteCommandCenter` 提供了许多内置的 `MPFeedbackCommand`，注册不同的指令会有不同的 UI。与上面类似的思路，注册这些指令只是定义了控制中心或锁屏界面  Now Playing 要展示的 UI，或者准确地来说是声明了要展示的图标，甚至其文字都可以通过 `localizedTitle` 配置自定义的内容。所以就有了像“网易云音乐”的更多菜单的效果：

![](images/Now_Playing_More.png)

这里使用了 `MPRemoteCommandCenter` 的 `likeCommand` 作为“红心”的菜单选项，使用 `dislikeCommand` 作为“上一首”的菜单选项。这里不用 `previousTrackCommand` 作为“上一首”也是有原因的，因为当有上面提到的这些指令注册后，会“吃掉” `previousTrackCommand` 的位置，导致无法展示 `previousTrackCommand` 对应的菜单项。更多组合方式大家可以发挥脑洞试一试吧。
