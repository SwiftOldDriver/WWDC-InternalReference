...
![](images/WWDC22 - 110338 - Explore media metadata publishing and playback interactions-0001.png)

![](images/WWDC22 - 110338 - Explore media metadata publishing and playback interactions-0002.png)

![](images/WWDC22 - 110338 - Explore media metadata publishing and playback interactions-0004.png)

另外，无论使用什么设备进行播放，与之配对的 Apple Watch 上的 Now Playing App 都提供相同的体验。它甚至还内置了 Apple TV 遥控器。

![](images/WWDC22 - 110338 - Explore media metadata publishing and playback interactions-0005.png)

在 tvOS 上使用 AVKit 时，播放界面中以信息浮层将展示标题和章节信息。当下滑到信息面板时，会展示更多详细信息，如封面图片和描述。按住 Apple TV 遥控器上的 TV 按钮时会展示控制中心，也有类似 iOS 的 Now Playing 卡片，支持展开。当在 tvOS 上后台播放音频内容时，无论是按下遥控器上的播放按钮，还是从另一台设备上选择 Music App 中的曲目，都会展示带有 Now Playing 信息的通知。另外，在 tvOS 播放音频时，停止操作一会后，tvOS 会全屏展示当前 Now Playing 的内容。

![](images/WWDC22 - 110338 - Explore media metadata publishing and playback interactions-0006.png)

![](images/WWDC22 - 110338 - Explore media metadata publishing and playback interactions-0007.png)

![](images/WWDC22 - 110338 - Explore media metadata publishing and playback interactions-0008.png)

![](images/WWDC22 - 110338 - Explore media metadata publishing and playback interactions-0009.png)

最后，在 iOS 的“控制其他扬声器”（Control Other Speakers）和 TV 按钮上，可以浏览所有设备上 Now Playing 的信息，并控制其播放。

![](images/WWDC22 - 110338 - Explore media metadata publishing and playback interactions-0010.png)

随着在 Now Playing 中展示信息、可控制播放的设备的增加，正确发布 Now Playing 的信息和响应远程命令就尤为重要了。

![](images/WWDC22 - 110338 - Explore media metadata publishing and playback interactions-0011.png)
...
![](images/WWDC22 - 110338 - Explore media metadata publishing and playback interactions-0017.png)

![](images/WWDC22 - 110338 - Explore media metadata publishing and playback interactions-0018.png)

![](images/WWDC22 - 110338 - Explore media metadata publishing and playback interactions-0019.png)
...
其实声明真正接收和响应远程命令的还是 `MPRemoteCommandCenter` 对象，在 Apple 推出 `MPNowPlayingSession` 之前要响应远程命令是直接使用其单例（`MPRemoteCommandCenter.shared()`）进行操作的。如今每个 `MPNowPlayingSession` 实例都有自己的 `MPRemoteCommandCenter` 实例，意味着可以以播放会话为单位接收和处理远程命令，而不像之前使用单例只能处理一个，当有多个播放会话时，就可以对远程命令做不同的响应了。
...
类似地，配置快进和快退命令。该命令适用于大多数播放内容，除了如直播流无法快进的媒体。
...
另外，在带广告的视频中，可能不希望用户快进广告部分，这个场景中，可以通过禁用 `skipForwardCommand` 来实现暂时禁用命令的需求。
