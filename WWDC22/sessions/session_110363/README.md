...
ARM 61 有 31 个 64 位宽的寄存器，我们可以通过 x0 到 x30 来访问这些寄存器。如果只访问低 32 位部分的话就可以通过 w0 到 w30。寄存器 x0 到 x7 用于存储函数接收到的前 8 个参数。这也就是说 objc_msgSend 方法在 x0 中接收 self，在 x1 中接收 selector `_cmd`。
