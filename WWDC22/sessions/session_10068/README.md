...
在新增的这两种样式下，我们可以把不太重要的标题挤到左边展示，在中间提供一块区域放一些操作按钮，方便用户更便捷地创作。
...
- UIPageControl 之前只可以左右滑动，现在可以自定义控件的排列方向和滑动方向了。提供了一个 `direction` 属性，支持各种方向和方式去滚动了。
- UIPageControl 之前只支持设置 indicator 的图片，而具体展示出来当前的页面的 indicator 样式依赖于系统去处理深色和浅色的变化来做区别。现在支持了自定义当前的 indicator 图片。
...
不知道有多少读者在用系统的 UIPageControl，小编长年以来都是使用自定义的 Page Control 来满足 UE 同学们的需求，例如滑动过程中的水滴效果，此次的更新还是离这些需求的距离有些遥远，只能说是聊胜于无吧。
...
如果不想弹这个 alert ，开发者需要使用系统提供的粘贴接口来访问 pasteboard 的内容。
...
iOS 16  对额外展示提供了更多的支持。开发者如果没有使用 UIScreen 相关 API 的话，无需对此做任何改动。但如果使用了的话就需要注意了。开发者现在不能再假设 app 是在 main screen 上了，因为接下来会把 `UIScreen.main` 和 `UIScreen`生命周期的通知**废弃**掉。
...
2. 不能再通过设置 UIDevice.orientation 来控制横竖屏了，取而代之的是 UIViewController 里面的方法`preferredInterfaceOrientation` 等 API，参考[文档](https://developer.apple.com/documentation/uikit/uiwindowscene/geometrypreferences/ios?changes=latest_beta)。
