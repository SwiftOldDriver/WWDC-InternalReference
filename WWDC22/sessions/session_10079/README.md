...
DNS 对于移动开发者来说像是遗忘在角落里而很少被关注的知识领域，然而他自身的安全性问题逐渐引起人们的重视，WWDC2020 中的 [Enable encrypted DNS](https://developer.apple.com/videos/play/wwdc2020/10047) 介绍了苹果平台支持的 DoT 和 DoH 两种加密 DNS，在一定程度上改善了互联网隐私。
...
按照层次可将 DNS 服务器分为三种类型：根服务器、顶级服务器和权威服务器

![dns_level_12](./images/dns_level_12.png)

本地域名解析服务器严格来说不属于上述层次体系。每个 ISP 有一个本地域名服务器即默认域名解析服务器。当主机进行 DNS 查询时，查询被发送到本地域名服务器，其作为代理，将查询转发给层次式域名解析服务器。

#### DNS 解析查询流程

当在浏览器中输入网址时，DNS 解析查询过程大致如下，查询方式分为递归查询和迭代查询。DNS 客户端和本地域名服务器是递归，本地域名服务器和其他域名服务器之间是迭代。

![dns_resolver_11](./images/dns_resolver_11.png)
...
3. 攻击机器伪装成 DNS域名解析服务器，向转发器回复伪造的响应报文，并实现投毒
...
SSL/TLS 是一种使用较为广泛的密码通信框架，综合运用了密码学中的对称密码、消息认证码、公钥密码、数字签名、伪随机数生成器等。设计目的主要有三个：
...
![dns_tls_13](./images/dns_tls_13.png)
...
握手协议是 TLS 协议中非常重要的协议，通过客户端和服务端的交互，共享必要信息，从而生成共享密钥和交互证书。主要流程如下：

![dns_tls_shake_14](./images/dns_tls_shake_14.png)

### 二、什么是 DNSSEC

> DNSSEC 是由 IETF 制定的一套扩展规范。许多 DNS 服务提供商已经支持它了，但是客户端支持还需要加快速度。

iOS16  和 macOS Ventura 现在支持客户端 DNSSEC 验证。 DNSSEC 通过添加数字签名来确保数据的身份验证，以此来保护数据完整性，当应答者不存在时，它会验证拒绝存在，还提供加密身份验证。DNSSEC 通过在响应中附加签名来保护数据完整性。如果响应被攻击者篡改了，那么篡改后的数据签名将与原始数据不匹配。这种情况下，客户端可以检测到响应已经被篡改然后将其丢弃。
...
![dns_nsec_15](./images/dns_nsec_15.png)
...
![dns_ddr_16](./images/dns_ddr_16.png)
