...
首先是层级结构：TextKit 2 与 TextKit 1 一样，都是以 CoreGraphics 为基础，UIKit 和 AppKit 等文本相关控件均以 TextKit 为渲染内核。TextKit 实际上为 CoreGraphics 更加面向对象的封装，使得上层开发者也可以方便的脱离系统文本控件来定制个性化的文本组件。![](./images/IMG_1_1.png)比如 DinamicX 就以 TextKit 1 为基础，实现了自测自绘的富文本组件，由于 TextKit 的相关 API 均可以在异步线程访问，基于此，我们利用 TextKit + 异步绘制能力大大提升了信息流场景下多文本情况下的滑动流畅度。
