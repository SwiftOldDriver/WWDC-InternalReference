...
插件是在沙盒中运行的，权限是受限的。不允许发送网络请求，只允许写入文件系统的几个位置，例如 build、outputs 目录。可以派生子进程，比如执行 /usr/local/bin/git 相关操作。但是无法执行系统服务访问（例如剪切板，使用 NSWorkspace 启动其他应用程序等）。
...
运行具体插件（例如：generate-contributors-list）命令。
...
大体流程是线程启动，系统线程相关一些操作，SwiftPM 执行相关方法，最上层 API 调用 NSConcreteTask launchWithDictionary:error: 方法。看来包插件通过 NSConcreteTask 来启动调用的。
