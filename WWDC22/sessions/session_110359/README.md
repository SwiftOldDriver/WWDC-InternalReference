...
除了 构建工具 plugins，Xcode 14 还支持 command plugins。下面分别介绍下。
...
构建工具 plugins 可以作用于单个 target，而 command plugins 是作用于整个包或项目。
...
下面分别介绍下 command plugins 和构建工具 plugins 。
...
- capability：能力，分为 command plugins 方法 .command()，跟构建工具 plugins 方法 .buildTool()
