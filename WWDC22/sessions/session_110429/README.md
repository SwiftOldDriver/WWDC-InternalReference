...
- Face-driven AF/AE：Camera 自 iOS 15.4 起支持基于人脸驱动的自动对焦和曝光功能，并设置为默认配置。
- Advanced Streaming：iOS 16 起支持同时对多个 VideoDataOutput 做不同的配置，包括分辨率、旋转方向、稳定模式以及像素格式。
- Multitasking  camera access：Camera 自 iOS 16 起支持多任务处理接口，iPad 分屏模式下也可访问相机。
...
通过设置相机的 DeviceType 为 `.builtInDualCamera` 、 `.builtInDualWideCamera` 或 `.builtInDualWideCamera` 类型，将其作为 Session 的 videoInput，然后在 output 中将深度数据开关 `isDepthDataDeliveryEnabled` 设置为支持即可。
...
在 iPhone X 发布时，苹果宣布了 iPhone 的人脸解锁方案，也就是 Face ID。该技术就是基于深感相机（TrueDepthCamera）来实现。TrueDepthCamera 包括了红外镜头+点阵投射器+RGB 摄像头的组合，是一种基于红外结构光的深度相机。
...
TrueDepthCamera 在 ARKit 以及 AVFoundation 中都有相关 API，主要用于 3D 人脸模型、动画表情、AR 自拍等场景中。
...
类似的，苹果这次新增的 LiDAR Camera DeviceType 类型为 `.builtInLiDARDepthCamera` ，它会开启广角(主摄)摄像头并配合 LiDAR Scanner 来获取带深度信息的照片/视频。
...
|  ----  | ----  |  ----  | ----  |  ----  |
| LiDARDepthCamera  | ![LiDAR_depth_camera](./images/LiDAR_depth_camera.png) | ToF | 1. 获取到的深度信息为绝对深度信息 2. 有效距离约 5m 范围 | 1. 计算机视觉 2. AR 房间场景模型构建等
| Dual/DualWide/TripleCamera  | ![duar_camera](./images/duar_camera.png) ![dual_wide_camera](./images/dual_wide_camera.png) ![triple_camera](./images/triple_camera.png)  | 多角立体成像 | 1. 相对深度信息 2. 耗电更少 | 拍照/摄像特效
| TrueDepthCamera  | ![true_depth_camera](./images/true_depth_camera.png) | 结构光 | 1. 相对深度信息 2. 耗电更少 3. 投射超 30000 个点阵，精度高 4. 前置,有效距离短 | 1. FaceID/3D 人脸模型 2. 动画表情

随着这次苹果对 LiDAR Camera 的支持，我们可以利用 AVFoundation 获取更加准确的深度信息。除了在已支持的 AR 方面的应用，未来在计算机视觉领域，比如定位、接近检测、碰撞避免等方面都可以有很大的发挥空间。

## Face-driven AF/AE

AF/AE 是 iPhone 相机支持的自动对焦和自动曝光功能，它们能使你的相机对焦在拍摄主体上，并且通过平衡场景中最亮和最暗的区域，来保持拍摄主体的可见性。
但有时候，我们还是会碰到不大符合预期的情况，比如自动对焦和自动曝光并不能保证拍摄主体人物的脸处于对焦状态或者合适的曝光状态，如下图所示：

![af](./images/af.png)
![ae](./images/ae.png)

为了避免在拍摄人物时出现如上对焦不准或者过度曝光的情况，苹果在 iOS 15.4 开始支持 Face-driven AF/AE 的功能，也就是通过跟踪人脸来自动对焦到拍摄人物身上，调整曝光为合适的程度。并且该功能从 iOS 15.4 开始，将会作为默认配置。

我们可以通过以下开启人脸驱动和关闭人脸驱动的自动对焦和曝光对比图来感受一下效果的差异。比如图一中，在开启人脸驱动自动对焦后，在人物面部背对镜头时，相机焦点会对焦在远处景物上，但当人物面部转过来面对镜头后，相机焦点又会自动对焦到人物面部。
...
该功能只支持相机的自动对焦/曝光模式，如果切换到手动模式则失效。
...
但从 iOS 16 开始，苹果可以允许同时使用多个 AVCaptureVideoDataOutput。我们可以对不同的 AVCaptureVideoDataOutput 设置不同分辨率、Stabilization 稳定模式、Orientation 方向、Pixel Format 像素格式。

举个例子，当我们对视频流预览的分辨率要求不高，但是对写入的视频文件分辨率有较高要求时，就可以分别配置低分辨率和高分辨率的两种 AVCaptureVideoDataOutput，分别用于预览和文件的输出。

![advanced_process](./images/advanced_process.png)

或者我们在启用相机防抖时(Stabilization 稳定模式)，由于需要对每一帧视频流进行防抖算法处理，会导致画面存在一定的延迟，这并不利于拍摄过程中画面的捕捉。那么这种情况我们就可以通过设置两种 AVCaptureVideoDataOutput 配置，分别打开和关闭防抖设置。在拍摄的时候使用无防抖功能的视频流预览，同时可以写入防抖处理之后的视频流到文件中，便于提高之后的视频观看体验。

### 如何为你的 APP 选择最合适的 Pixel Format

同样的，我们也可以为多个 AVCaptureVideoDataOutput 设置不同的像素格式。那么，我们需要如何来选择最合适的像素格式呢？
...
> YCbCr 或 Y'CbCr 有的时候会被写作：YCBCR 或是 Y'CBCR，是色彩空间的一种，通常会用于影片中的影像连续处理，或是数字摄影系统中。Y'为颜色的亮度(luma)成分、而 CB 和 CR 则为蓝色和红色的浓度偏移量成份。Y'和 Y 是不同的，而 Y 就是所谓的亮度(luminance)，表示光的浓度且为非线性，使用伽马修正(gamma correction)编码处理。
>
> Bits Per Channel: 指每个像素可以表示的色阶数，并以 bit 为单位，也可以称为色深。如我们常见的 RGB 格式中，以 0 到 255 级共 256 个灰阶来表示色阶的变化。「256」刚好是 2 的 8 次方 (2^8)，即 8-bit 位深。而 10-bit 位深则能够表示 1024 的三次方 (1024^3) 种色阶数量。色深的位数越高，越能细腻地表示色彩的色阶变化。
...
在 iPad 上，我们可以利用分屏，同时打开 Safari 和备忘录两个 app 进行工作，这就是所谓的多任务处理。除了分屏模式（Slide Over mode），还有侧拉模式（Split View mode）以及画中画模式（Picture in picture）。前面两者是只有 iPad 支持的模式，而画中画的模式 iPhone 也可以支持。
...
现在 AVCaptureSession 新增了两个属性，`.isMultitaskingCameraAccessEnabled` 和 `.isMultitaskingCameraAccessSupported`，用来表示是否支持相机的多任务处理以及是否开启相机多任务处理的功能。
