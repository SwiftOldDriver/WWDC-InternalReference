...
通过设置相机的 DeviceType 为 `.builtInDualCamera` 、 `.builtInDualWideCamera` 或 `.builtInTripleCamera` 类型，将其作为 Session 的 videoInput，然后在 output 中将深度数据开关 `isDepthDataDeliveryEnabled` 设置为支持即可。
...
|  ----  | ----  |  ----  | ----  |  ----  | 
| LiDARDepthCamera  | ![LiDAR_depth_camera](./images/LiDAR_depth_camera.png) | TOF | 1. 获取到的深度信息为绝对深度信息 2. 有效距离约5m范围 | 1. 计算机视觉 2. AR 房间场景模型构建等
...
| TrueDepthCamera  | ![true_depth_camera](./images/true_depth_camera.png) | 结构光 | 1. 相对深度信息 2. 耗电更少 3. 投射超30000个点阵，精度高 4. 前置,有效距离短 | 1. FaceID/3D人脸模型 2. 动画表情
...
为了避免在拍摄人物时出现如上对焦不准或者过度曝光的情况，苹果在 iOS 15.4 开始支持 Face-driven AF/AE 的功能，也就是通过跟踪人脸来自动对焦到拍摄人物身上，调整曝光为合适的程度。并且该功能从iOS 15.4开始，将会作为默认配置。
