...
没有对其内容进行修改的内存页。包括内存映射文件，例如纹理或音频资产，以及加载到进程中的 Frameworks 等。系统可以随时收回或从磁盘中重新加载，因此它们不会计入游戏的内存占用。然而，它们可能长驻于内存中，过度使用会减慢系统和游戏应用的运行速度。
...
除了 Xcode 内存仪表盘外，Mac 上的活动监视器也可以查找内存占用。此外也可通过 API 来快速查询当前足迹和可用内存：通过调用 `os_proc_available_memory` 查看 iOS、iPadOS 或 tvOS 游戏的可用系统内存：
...
对于 Mac 游戏，您还可以使用命令行工具，通过 `leaks PID` 命令捕获内存图，进程 PID 可以在活动监视器中找到。这对于需要运行全屏并需要保持专注的 Mac 游戏较为有用：
...
在这里，堆分配转到几个 MALLOC_ 前缀的类别，系统将堆分配按大小分组到大小池以提高性能（MALLOC_TINY、MALLOC_SMALL、MALLOC_MEDIUM、MALLOC_LARGE 标签管理不同大小的分配空间）。这些对象可能来自许多地方，例如用于游戏里的音效或物理模拟的第三方插件或库。
...
使用 `vmmap` 命令处理内存图，以查看 Compressed/Swapped 大小，而不是两者组合。DIRTY SIZE 列包括当前未交换或压缩的常规 Dirty Memory，而 Swapped SIZE 列包括压缩或交换内存的原始大小。系统将这两列添加到一起，以确定内存足迹。由于 Swapped SIZE 列中的内容没有经常使用，因此它是优化游戏内存的良好指标。

![](./images/p3-1-step2-2.png)

方便的是，vmmap 通过单独的表显示堆分配。在输出底部，vmmap 按区域对堆内存进行分组。这些区域反映了它们在游戏中的使用或生命周期。这里因为开启了 Malloc Stack Logging，也显示在堆上的分配在工具的区域内。否则，根据分配大小，它们将位于两个默认区域：MallocHelperZone 和 DefaultMallocZone。通常，您可以跳过较小的系统库区域，如 QuartzCore 区域。此外，如果您怀疑碎片大小（DIRTY + SWAP FRAG SIZE）或百分比(% FRAG)过高，例如数十或数百兆字节，可以通过 WWDC 2021 会话了解更多关于碎片化问题的信息^[1][2]。
...
这里使用 `-quiet` 参数来跳过有关某些元数据的信息。今年起 `heap` 工具在识别对象类型方面更智能^**新特性**。它使用 Malloc Stack Logging 记录的信息来显示调用者或响应库，新的工具使非对象类别 `non-object` 成为过去式。

>在此之前的 `heap` 工具导出结果中，通常可以看到一个或多个巨大的 `non-object` 类，占用内存极大，实际为无法细分识别的内存对象。

![](./images/p3-1-step3-1.png)

上面按照对象类别排序，有时，按分配大小排序比按类实例计数排序更有帮助。以 [Modern Rendering with Metal](https://developer.apple.com/documentation/metal/metal_sample_code_library/modern_rendering_with_metal) 示例项目的内存图为例，要在该示例中寻找较大的对象，可以使用堆按类总大小与 `-sortBySize` 对对象进行排序，并使用 `—showSizes` 列出所有对象，而不是每个类的摘要。在示例项目内存图 Bytes 列中有一个 NSConcreteMutableData 对象内存占用最大，值得一看。通过添加 `—address` 参数并输入 NSConcreteMutableData 跟通配符以列出 10M 及以上的对象：
...
**小结：**`heap`工具按类名对分配的资源进行分组，改进了实例的对象识别，也可以通过设置参数，对资源按内存大小分组排序。
...
随着疑似可优化对象的发现，下一步是获得其分配调用堆栈。在上面 [Modern Rendering with Metal](https://developer.apple.com/documentation/metal/metal_sample_code_library/modern_rendering_with_metal) 的最大内存对象中，使用 `-callTree` 模式，并将其地址传递给 `malloc_history`。加上额外的 `--invert` 参数，可以专注于最接近分配的函数，下面就是分配的调用栈：
...
不同于常规应用程序的断点调试，Xcode 通过其帧捕获工作流程专门为 Metal 提供了一个调试器：Metal Debugger。Metal Debugger 是调试 Metal 游戏的一站式解决方案。在 Xcode Scheme > Run 中，将 GPU Frame Capture 设置为 Automatically Enabled 以启用调试。运行 Xcode 项目，点击调试区域的相机按钮捕获 GPU frame 后，可以得到一个摘要页面，摘要提供了有关捕获的一些通用统计数据：

![](./images/p4-1-summary.png)

摘要下部分为分为四列的 Insights 列表。Memory 一列显示了游戏潜在的可节省内存建议，每个建议右侧都附有对问题的描述。以上面的捕获记录为例，在解决列出的潜在问题后能节省几 M 的内存。
...
**Insights 列** 与我们刚刚在摘要页面上看到的相似。按此列对表格进行排序时，您可以快速查看所有资源 Insights 建议信息，检查建议附带的潜在问题。点击 Insights 列显示的紫色警告图标将显示一个弹出窗口，解释该问题并提供一些可能的操作。更多关于 Insights 调试细节可参考 [wwdc20-10605: Gain insights into your Metal app with Xcode 12](https://developer.apple.com/videos/play/wwdc2020/10605/)。

### 2. 检查资源大小

**Allocated Size 列** 显示对象已分配内存大小。可以按此列排序以查看最大的资源，以检查一些资源是否真的充分利用了分配给它们的内存。例如，在不会影响游戏的视觉效果下，可以尝试调整一些纹理为较小的分辨率，对一些加载在缓冲区中的模型尝试减少网格的面数。稍后提到一些替代方法来保存纹理内存。
