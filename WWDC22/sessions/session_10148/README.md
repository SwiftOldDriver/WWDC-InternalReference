...
> 作者：龙韵翔 ，目前就职于字节跳动中国音乐团队，负责音乐播放器相关业务。
>
> 审核：曾铭，老司机技术社区成员，就职于字节音乐团队。

## Foreword

提起音乐消费类 APP，大家马上能想到 QQ 音乐、网易云、酷狗音乐等等，同时也会联想起最近几年，各大厂之间关于音乐版权之争的新闻。随着知识版权领域法律法规越来越完善，一张图片甚至一个小 icon 可能都需要支付不菲的版权费，音乐更是如此。在产品的研发过程中，若想给某个场景加上一个背景音乐，或者加一段知名的音效，亦或者想给应用内嵌音乐消费的能力，这时，你面临最大的压力和阻碍可能不是技术或者市场，而是音乐版权。像网易云、腾讯 TME 等大厂，每年都要花费巨大的人力、物力、财力来和各大音乐版权方协商，购买歌曲版权。这对个人开发者或者中小团队来说，无疑使雪上加霜。

不过办法总比困难多，Apple Music 似乎有着“完美”的解决方案，开发者可以选择使用 Music Kit 来接入 Apple Music ，可以通过订阅 Apple Music 服务，和 Apple 共享万千音乐资源，再也不用担心音乐内容背后的版权问题！通过此文，让我们一起来看看如何接入强大的 Apple Music 吧！

## About MusicKit & Music API

2017 年，Apple 首次推出 MusicKit 框架，并在 2021 年推出了一组在 Swift 中访问和播放音乐的 API ，使对整个 Apple Music 访问和控制变得可能。

MusicKit 是一个将客户端框架（iOS 、Android、Web）和 Apple Music API 结合起来的工具包。客户端通过 MusicKit 可以在   Apple Music 资料库中获取或搜索单曲、专辑、歌单，还可以在验证订阅者身份后，访问订阅用户的播放列表、播放历史，以及个性化推荐的内容。
...
本文简要介绍了 MusicKit & Apple Music API 的背景和能力，并通过一个简单的需求，详细介绍了在播放列表、搜索等场景下 Apple Music API 的使用。

## Integration

Apple Music API 是提供给开发者访问和搜索曲库内容的一些服务端接口。（Music API 可以在多平台使用，这里我们只聚焦于 iOS 平台）
...
在配置好令牌之后，MusicKit 会在使用 API 的时候自动帮我们鉴权等授权验证。
...
`api.music.apple.com /v1`     ``/catalog /us``    ``/ playlists``  ``/p1.2b0e6e332fdf4``  ``? include=curator``

​                  API 版本       xx 地区的曲库        内容类型     资源唯一标识     需要额外关联的元数据

该 URL 可以描述为`使用V1版本的 API 向 Apple Music 请求美国地区资料库中有关xxx标识的播放列表类型资源，且关联该资源管理员信息的元数据`
...
其中，歌单的封面艺术图信息在 artwork 实体中，并包含了一个可以指定宽高参数的图片 URL，这是 Apple Music API 提供的新特性。也就是说我们可以根据设计稿中封面图的尺寸，通过设置 URL 宽高参数来获取定制的图片。
...
通过分析 relationships 实体中的 tracks ，不难发现，Music API 返回的数据结构十分套娃，因此通过 attributes 可以很轻松的获取到单曲的摘要信息，这样我们已经轻松完成了大部分需求。

![05](./images/05.png)

### 资源控制

有些同学可能已经想到了一个 badcase ，当歌单中存在 1000 条甚至是更多的歌曲时，Music API 会一股脑返回吗？不用担心，Music API 为我们提供了分页能力。
...
到这里，我们需求的完成度已经达到了预期，但是 PM 临时要求给我们的 App 增加搜索功能！需要通过用户的兴趣词搜索单曲和专辑内容！不要慌，Apple Music API 也提供了曲库的搜索能力。

## Explore Search

> Apple Music API 提供了使用搜索词在曲库中查找内容的能力。
...
本文简单介绍了 MusicKit 和 Apple Music API，并通过一个简单的例子探索了一下将 Apple Music 集成到我们自己 APP 中的过程。

回顾一下文章开头提到的 NIKE RUN CLUB 这款 APP，在日常生活中，我们在进行跑步、散步、骑行等运动时，通常会使用运动 APP 记录锻炼状态，同时也会打开音乐 APP 来些喜欢的节奏，而 NIKE RUN CLUB 将状态记录和听音乐巧妙的结合在了一起，使得用户在一个 APP 中完成了多种需求，简化了操作路径也提升了用户的体验。同样，在阅读、健身、策略、养成等主题的 APP 中，也可以根据用户行为习惯集成音乐模块，而集成 Apple Music 无疑是一个最好的选择！使用 MusicKit 可以很低成本的解决播放能力，同时 Apple Music 曲库内容多，无任何版权成本，还可获取用户已收藏歌单、播放历史，以及个性化推荐媒体流。
近两年，Apple 一直在增强和扩展 Music 相关的接口和功能，甚至推出了跨平台的使用方案！相信在不久的将来集成 Apple Music 会更加轻松和方便；相信在不久的将来会有越来越多开发者选择 Apple Music 在更多 APP 中创造出更好体验的音乐场景~
