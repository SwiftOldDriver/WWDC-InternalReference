...
Web Push 的需求来源和 App 的通知/推送（Notification）一样。支持远程推送是 App 相比 Web 最不同的特性，通知提供了一种全新高效，和用户交流的方式。用户访问网页时，网页以 Poll 或者拉数据的形式来更新数据；而 App 利用 os 提供的双工或者长连接来实现主动向用户更新数据的能力。纵使 Web 技术不断发展和优化，使得 Web App 和 native App 在交互体验上差距越来越小，而主动推送消息的能力是 Web App 心口的痛，即使是微信小程序也受限于此，只得指望微信为它分配的每日配额的推送数量。
...
1. **[Service Worker API](https://developer.mozilla.org/en-US/docs/Web/API/Service_Worker_API)**，ServiceWorker 并不是为了 Web Push 而创造的，而是为 Web App 提供离线、多线程能力而引入的技术。工作组开始做 Web Push 时发现 ServiceWorker 是个很好的技术，所以在此基础上添加了一些专门为 Web Push 服务的接口（ additional interfaces）。
...
> Safari 16 需要在  macOS 13 上运行。因为 Safari 16 遵循标准推送协议，所以代码在 Chrome 上可以运行，意味着也可以在 Safari 16 上运行。

另外，Session 里代码示例中， JS 使用了 async/await 语法，并不是所有浏览器都支持，本文并没采用。
...
在我们得到推送权限之后， Service Worker 就可以运行了，Service Worker 的作用有两个；
...
```json
{
  ...
  "expirationTime":null,
  "keys":{"p256dh":"BIUijBro****X_v3Ah5GOE",
          "auth":"GmY-6E-jnPSg4XnjztsEtA"
         }
}
```

> 从域名 `fcm.googleapis.com/fcm`上可推测，Chrome 的 Web Push 是基于 Firebase 能力建设的

可以看到 endpoint 和浏览器本身相关，会随着浏览器类型发生变化，而 native 发送 APNs 的 URL 一直是 `gateway.push.apple.com` 不会变。根据  [Voluntary Application Server Identification (VAPID)](https://datatracker.ietf.org/doc/html/draft-ietf-webpush-vapid-00) 定义，这里的 keys 除了后续 Web App 后端用来解密 [JSON Web Key](https://datatracker.ietf.org/doc/html/rfc7517) (JWK) 外，还可以区分用户，日后可以通过筛选 key 值来表达定向推送的意图。
...
上述步骤 3，4，5 需要后台配置 https 证书、处理请求参数中的组装、加解密，步骤繁多，属于 Web App 后端部署范畴，相关代码，本文一并略过，强烈推荐以下系列文章，详细描述了操作步骤，也解释了很多原理。
