...
- 第二部分是从使用 Object Capture 到构建 AR App 的 demo 的整体流程；
- 第三部分则详细解读了 demo 中的 shader 实现。
...
关于这些增强的更多细节，可以参看今年的 [WWDC Session 10126：*探索 ARKit 6*](https://xiaozhuanlan.com/topic/2439056187)。
...
这一步中需要给画面加上泛光的**后处理效果**。后处理，也叫后置处理，后期处理，或全屏特效处理。顾名思义，它是在物体渲染后执行的 shader(s)，输入包括颜色与深度信息，根据需要处理后再得到目标颜色（如下图）。
