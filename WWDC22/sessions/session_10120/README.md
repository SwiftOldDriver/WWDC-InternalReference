...
Mapping Model 包含了把数据从源 Model 迁移至目标 Model 所需要的信息。 轻量迁移中，Core Data 会自动推断中 Mapping Model。如果想事先取得源 Model 和目标 Model 之间的 Mapping Model，可以使用 [`NSMappingModel.inferredMappingModel`](https://developer.apple.com/documentation/coredata/nsmappingmodel/1506468-inferredmappingmodel) 方法。
...
> 今年的 Session 10119 中有对 CloudKit 和 CoreData 使用的进阶讲解，感兴趣的读者可以[点击查看](https://xiaozhuanlan.com/topic/5821964073)。
...
sqlite> .schema
CREATE TABLE ZAIRUNIT ( Z_PK INTEGER PRIMARY KEY, Z_ENT INTEGER, Z_OPT INTEGER, ZNUMENGINE INTEGER, ZTIMESTAMP TIMESTAMP, ZTYPE VARCHAR, ZNAME VARCHAR );
CREATE INDEX ZAIRUNIT_Z_ENT_INDEX ON ZAIRUNIT (Z_ENT);
