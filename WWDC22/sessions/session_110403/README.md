...
苹果在 `WWDC2015` 推出 `ODR（On-Demand Resources）` 技术，用于资源动态加载，目的是为了帮助开发者减少 `APP` 安装包大小，开发者可以将应用的一部分资源提交到 `AppStore`，用户打开 `APP` 进入某个页面时会触发资源下载，常见的应用场景有：游戏关卡、相机应用的贴纸、滤镜等。
