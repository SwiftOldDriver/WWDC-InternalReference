...
苹果为了解决这类问题推出新框架：`Background Assets`，使用新框架后，用户打开 `APP` 后可以直接使用，没有额外的资料包下载，没有漫长的等待，用户可以享受更好的产品体验。

`Background Assets`（后续简称 `BA`）框架可以与现有的资源管理流程快速对接，不需要将资源提交到 `AppStore`，可以为开发者提供应用生命周期之外更新资源的能力，应用下载完毕后，`BA` 通过 `Extension` 启动资源下载，用户打开 `APP` 后无需等待，所有内容都已准备就绪。

苹果在 `WWDC2015` 推出 `On-Demand Resources`（后续简称 `ODR`）技术，用于资源动态加载，目的是为了帮助开发者减少 `APP` 安装包大小，开发者可以将应用的一部分资源提交到 `AppStore`，用户打开 `APP` 进入某个页面时会触发资源下载，常见的应用场景有：游戏关卡、相机应用的贴纸、滤镜等。

`BA` 与 `ODR` 的区别在于：

1. `BA` 支持开发者现有的资源管理流程，可以直接使用 `CDN` 上存放的资源，而 `ODR` 需要将资源上传到 `AppStore`，与开发者已有资源管理流程不兼容，需要对流程进行改造；
2. `BA` 提供应用生命周期之外更新资源的能力， 而`ODR` 只能在应用生命周期内更新资源；

`ODR` 的优势在于所有资源都是通过 `AppStore` 托管的，资源的下发是走的官方渠道，不存在审核风险；`BA` 的优势是兼容已有的资源管理流程，并且提供应用生命周期之外更新资源的能力。
