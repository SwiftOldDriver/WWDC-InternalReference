...
我们通过请求来读取图片内容，读取的结果会以数组的形式给出，然后我们需要训练可以识别这些内容的 CoreML 模型，再接下来运行 CoreML 模型滤除不好的特征，把所有这些字符放到一个字符串中，然后修正结果，最终形成句子和单词。
...
文本在识别时有两种识别方式：快速路径（Fast Path）和精确路径（Accurate Path）：

- 快速路径: 是实时通过查找字符来识别文本，文本检测器主要通过一个小型的机器学习模型，来逐个识别字符。对自定义字体或者旋转后的文本识别效果较差，运行所需内存较小，整体适合实时高帧率采集画面的识别场景。
