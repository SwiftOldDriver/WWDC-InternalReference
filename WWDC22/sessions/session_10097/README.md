...
需要注意的是，如果需要兼容 iOS 16 以下版本，App Clip 的解压缩二进制包体积仍然需要保持 10 MB 以下。

![](./images/new_size_limit.png)

速度，是用户获得出色体验的关键。用户不会永远都处于网络良好的环境，因此优化 App Clip 包体积大小是十分重要的。接下来，介绍下优化 App Clip 包体积的一些方式。

### 设置最优的 Build Setting

在 Xcode 选中 App Clip 对应的 target，在 Build Setting 找到 Apple Clang - Code Generation 选项，选中 Fatest, Smallest [-0s] 这一最佳组合来保证构建包提交最小且运行速度最快。

![](images/build_setting_suitable.png)

### 使用 Asset catalogs 来保存媒体资源

苹果推荐使用 Asset catalogs 来保存媒体资源，这样有有两个好处

- 添加进 Asset catalogs 的媒体资源，会在 Xcode 构建过程中自动优化。
- 当用户下载的时候，最终下载的包到只包含适合于用户设备的媒体资源。

### 检查 Compile Sources 文件和  .ipa

记住 App Clip 是 App 的轻量级版本，所以要确保只编译和链接 App Clip 需要的东西。不建议直接复用 App 的 Asset catalogs 和 本地化字符串文件，很容易会引入 App Clip 非必须文件。而且要经常检查和过滤这两类文件，随着 App Clip 迭代，可能会充斥着重复和未使用的资源。

> 可以认为适用于 App 包体积大小优化的方案同样适用于 App Clip。

更多 App Clip 包体积优化方案请参考 [WWDC21 Build light and fast App Clips](https://developer.apple.com/videos/play/wwdc2021/10013/)
...
![](./images/two_type_of_diagnostics.png)
...
![](./images/cloud_kit_difference.png)

这也兑现了 Apple 当初对用户的承诺，当用户不再使用 App Clip 的时候，iOS 将会删除 App Clip 及其数据。

在阅读过程中，你也可能会跟我有一样的疑惑。App Clip 只有读取的权限，而且 App Clip 通常是在没有 App 的情况下使用的，那么读取的数据从哪里来呢？如果你有这个疑惑的话，可能是对数据的定义和来源有些偏差。并不是说只能读取到自己保存的数据，CloudKit 的公共数据库的数据是面向所有用户的。
举个例子，我开发了一个 App，功能是将我公司楼下的每一个小餐馆的菜单上传并做展示。我的同事可以下载这个 App 的 Clip 来订阅自己喜欢的餐馆，这样就更方便讨论中午吃什么了。而且当订阅的餐馆菜单更新的时候，他们也会及时收到通知。如果当同事们中午吃饭的时候，发现餐馆增加几道新菜式，他们也想更新 App 中的菜单。此时他们需要下载 App 来解锁全部的使用体验。
...
![](./images/key_chain_difference.png)
...
![](./images/kinds_of_experience.png)
...
![](images/toast_app_clip.png)

在去年我做 App Clip 内参的时候，我觉得 App Clip 可能会在点餐支付这种场景中大放异彩，但是这一年却始终趋于沉寂。因为摆在面前的还有一座大山，就是无法兼容 Android 用户。我希望在明年做有关 App Clip 内参的时候，我可以骄傲的跟大家介绍我们身边优秀的 App Clips。
