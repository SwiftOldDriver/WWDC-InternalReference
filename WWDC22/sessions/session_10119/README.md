...
在应用开发完成之后获得日志是我们排查线上与测试中问题的重要手段。接下来介绍如何从任何设备上获取日志。
...
作为对 `CloudKit` 重度依赖的独立开发者，本 Session 给我个人带来的了一些启发。比如通过`单元测试`提前验证数据结构设计的合理性。更重要的是日志收集与分析工具的介绍给我提供了很大的帮助，曾有几次面对用户的问题反馈，却缺乏可用日志，难以跟进排查问题的尴尬局面。`CloudKit` 控制台的一些小更新也是方便了开发者。
