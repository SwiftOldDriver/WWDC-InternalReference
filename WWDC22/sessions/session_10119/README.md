...
对于大多数 iOS 开发者来说 `CloudKit` 是一个比较陌生的东西，但它却是很多独立开发者/团队的好伙伴。免费、安全、集成度高等特点让我们没有不选择他的理由。

对于初上手 `CloudKit` 的同学，一、二章节有助于帮助你设计出高效的数据结构与了解 `CloudKit` 同步数据的运行机制，同时还有我的项目实践案例。

如果你已经有了相关开发经验，那么三、四章节可以帮助你提升调试与处理问题的效率。
...
总体来讲此次和 `CloudKit` 相关的更新的重心在于提高开发者的使用效率，但缺少一些惊艳的能够激发创造性的点。同时作为独立开发者也很开心看到 `Apple` 还持续的在为开发者持续提供免费安全的云存储服务，并不断提升便捷性与效率。期待后续能带来更多相关新的功能，造福广大充满创造性的独立开发者们，创造出更多有趣的产品。

### 我期待的

对于使用 `CloudKit` 的开发者来说，虽然本次相关更新在一定程度上提升了开发体验，但依旧需要开发者在 `Xcode` 与`网页控制台`间进行不断的切换。既然 `Xcode` 早已集成了 `Container` 的管理入口，如果能够继续将 `Zone` 与 `Records` 相关模块也集成进 `Xcode` 相信开发体验能够获得极大的提升。

前文有提到的异步加载 `CloudKit` 资源的问题，我的扫雷项目排行榜列表是通过 `creatorID` 的方式进行资源一对一绑定的，并不能通过我们熟悉的 `URL` 的形式使用图片，导致了在使用时的开发成本很高，尤其是列表这种需要显示大量不同图片的时候。我功能开发时，在开源社区没有找到基于 `CloudKit` 的类似 `SDWebImage` 的开源框架，只能基于本项目的数据逻辑简单封装了一套，但泛用性不强。如果 `Apple` 能够就资源加载提供更佳便捷的方式的话那就解决了一个巨大痛点。

> 顺手挖个坑：把我之前封装的 `CloudKit` 图片加载工具优化封装一下，更具通用性。
