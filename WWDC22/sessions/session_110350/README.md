...
- 更多内容可以参考 WWDC21 Session [Meet async/await in Swift](https://developer.apple.com/videos/play/wwdc2021/10132/) 和对应的内参文章 [【WWDC21 10132】认识 Swift 的 Async/Await](https://xiaozhuanlan.com/topic/9307851264)。
...
- 更多内容可以参考 WWDC21 Session [Explore structured concurrency in Swift](https://developer.apple.com/videos/play/wwdc2021/10134/) 和对应的内参文章 [【WWDC21 10134】 探索 Swift 结构化并发](https://xiaozhuanlan.com/topic/3625784190)。

### Actor 模型

- Actor 模型是为了解决多线程访问共享数据造成的数据竞争问题，它将数据隔离在 Actor 外，从外部访问 Actor 数据时（我们称之为隔离域外），只能够异步地访问，并且 Actor 内部有串行执行器来保证访问是串行的。
- 更多内容可以参考 WWDC21 Session [Protect mutable state with Swift actors](https://developer.apple.com/videos/play/wwdc2021/10133/) 和对应的内参文章 [【WWDC21 10133】并发编程新利器 - Actor](https://xiaozhuanlan.com/topic/3204768519)。

## Instrument 14 的 Swift Concurrency Template

- 去年加入的这些 Swift 并发的新特性确实令人兴奋，这让我们开发者更容易写出安全高效的并发代码。然而编码方式的转变也意味着我们需要用新的视角和新的方式去排查问题，优化代码。
- 今年新发布的 Xcode 14 的 Instrument 里增加 `Swift Concurrency` 的 Template 来帮助我们更好地理解我们所写的 Swift 并发代码是如何执行的，提供了可视化的工具，帮助我们更快地定位到潜在的问题。
- 首先，让我们先对这个工具快速整体地概览一下，后面我们再结合具体的例子分析如何使用该工具排查问题。
...
接下来我们会结合最常见的几种 Swift 并发问题和具体的例子来看看如何使用这个工具：
