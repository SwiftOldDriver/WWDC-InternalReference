...
苹果首先对“怎么用Apple Pencil在PDF页面上画画”这一个很多人问过的问题做出了解答：那就是使用叠加视图（overlay views）。
...
在PDF页面上安装叠加视图非常简单，但是有个问题，PDF可能包含着数百（甚至上千）个页面，所以在打开PDF时，我们不可能为所有的页面都创建叠加视图。如果用户快速来回滚动，我们如何知道何时创建叠加视图？
...
图中的黄色部分就是使用Apple pencil在PDF绘制的内容，其本质上就是往PDF上添加了一些叠加视图。
那么我们如何去保存这些绘制内容呢？
同时，对于保存的结果，我们还希望能实现：
- 以高保真度去匹配屏幕外观。
