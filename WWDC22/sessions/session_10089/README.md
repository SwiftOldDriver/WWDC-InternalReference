...
本文基于 [Session 10089](https://developer.apple.com/videos/play/wwdc2022/10089) 梳理。
...
`PDFAnnotation`：是 PDF 文档中所有标注类型的包装类。它是一个直接映射到 PDF 文件格式的键值对的容器，在 Adobe PDF 规范（1.7）中，你可以通过制定键值对来实现标注的属性。
...
`PDFAction`：是一个抽象类，用于表示用户可以触发的事件与所属标注的交互，这些事件可以是 PDFActionGoTo、PDFActionNamed、PDFActionRemoveGoTo、PDFActionResetForm、PDFActionURL 等，通常这些事件主要用于移动 PDF 视图的视口、打开网页浏览器、清除表单等。
...
以上代码创建了一个 PDFView，设置了文档居中，并且背景颜色是灰色的；最特别的是，只要设置了`pdfView.acceptsDraggedFiles = true`，我们就可以手动拖动 pdf 文件到视图范围内加载文档了。
...
```
// Create a PDFDoument from a file
...
document.write(to: documentURL, withOptions: [.ownerPasswordOption: "apple"])
```
...
```
// For each page, create a image
for pageIndex in 0..<document.pageCount {
    if let page = document.page(at: pageIndex) {
        let pageThumbnail = page.thumbnail(of: CGSize(width: 100, height: 100), for: .cropBox)
        // pageThumbnail ready to user
    }
}
```
...
```
// 1. Register PDFDocumet delegate
...
当 PDFAnnotation 为 link 类型时，可以设置 action 属性，用户通过触摸或者点击 PDFAnnotation 来响应事件，响应事件的类型是 PDFAction 的各种子类：
...
![](./images/PDFAnnotationBestPractices.png)
...
- overlayViewForPage：是必须要实现的协议方法，我们只需要提供一个视图的实例对象，PDFKit 就会通过使用适当的约束来调整它的大小。如果页面是一个旋转视图，那么我们提供的覆盖视图也会跟着旋转。
