...
![Overview.png](./Images/01-overview.png)
...
![PlatformUpdate.png](./Images/02-platform_update.png)

### 设备名称权限

UIDevice API  允许我们获取设备名称的几种情况大概是这样的

![DeviceNameEntitlement.png](./Images/03-device_name_entitlement.png)
...
![DeviceNameAPI.png](./Images/04-device_name_API.png)
...
![MapPrivacy.gif](./Images/05-map_privacy.gif)
...
![SecurityAndPrivacy.png](./Images/06-GateKeeper.png)
...
隔离是下载应用程序（或其中的磁盘映像）的结果。当我们使用浏览器下载文件时，浏览器会附加 ``com.apple.quarantine`` 属性，表明它是来自不受信任的网络来源。其他类型的互联网应用程序（电子邮件、聊天等）也会将该属性附加到下载的文件上。

但是并不是所有的网络下载方式都会将应用附加隔离属性。例如：通过文件共享链接的文件（带有 Finder 的 AFP 或 SMB）不会将其标志位隔离。

那么被标志为隔离属性的应用程序是会被严格执行 GateKepper 检查的，所以会出现【"X.app" 无法打开，因为无法确认开发者身份】现象。解决方案就是在隐私中进行授权（如图 SecurityAndPrivacy.png 所示）
...
![VerifyOptimization.png](./Images/07-verify_optimization.png)
...
![VerifyOptimization-2.png](./Images/08-verify_optimization_2.png)
...
![VerifyOptimization-3.png](./Images/09-verify_optimization_3.png)

在设置中，我们也可以对这个权限进行查看、修改、管理

![VerifyOptimization-4.png](./Images/10-verify_optimization_4.png)
...
![UserNotification.png](./Images/11-user_notification.png)

#### 系统设置

通过点击【通知消息】或者直接去【系统设置】，用户就可以直接管理开机时所有 App 的自定义操作了，需要注意的是如果你的 App 申请了【守护程序】等操作时，则需要更高的管理员权限才可以。

![LoginItemsSettings.png](./Images/12-login_items_settings.png)
...
在 iOS 16 之前当 App 内发生粘贴时，会有一个半透明的通知弹窗让用户知道何时发生了粘贴操作。在 iOS 16 之后，系统会确认所有访问其他用 App 的粘贴意图。甚至如果 App 继续使用 UIPasteboard API 进行粘贴操作，系统会弹窗提示

![PasteBoardPermission.png](./Images/13-paste_board_permission.png)

只要使用粘贴就要弹窗提示也是一件好可怕的事情，好消息是官方提供了 3 个方法来避免这个弹窗的问题：
...
![FeatureToAdopt.png](./Images/14-feature_to_adopt.png)
...
![UIKitPasteControls.png](./Images/15-uikit_paste_controls.png)
...
![MediaDeviceDiscovery.png](./Images/16-media_device_discovery.png)

通过 Media device discovery 可以将流媒体信息传输到选定的设备上，但是不需要显示获取网络或者蓝牙等方位权限弹窗。

![MediaDeviceDiscovery-2.png](./Images/17-media_device_discovery_2.png)

流媒体与 AirPlay 一样都是显示在选择控制器中，并且 App 只能看到能够将流媒体信息传输上去的设备

![MediaDeviceDiscovery-3.gif](./Images/18-media_device_discovery_3.gif)
...
![PHPicker.png](./Images/19-PHPicker.png)
...
在互联网的环境中，我们离不开的就是账号和密码，随着种类繁多的 App 的出现、办公环境的需求我们有永远记不完的账号和密码。其中密码是验证账户和保护个人数据安全的核心，因此如何保护密码是非常重要的。但是在日常生活中为了能够记住密码，我们可能存在的操作是存在危险的，你是否中招了呢？
...
- 更加健壮的身份验证解决方案
- 具有非常有好的 UI/UE 样式，例如通过使用 FaceID、TouchID 就可以让密码自动填充

![Passkey.png](./Images/20-Passkey.png)
...
![EmergencyReset.png](./Images/21-emergency_reset.png)
...
![SharedRightsManagement.png](./Images/22-shared_rights_management.png)
...
![QuickExit.gif](./Images/23-quick_exit.gif)

## 小编有话

以上内容就是今年 Apple 对于隐私内容的更新，即使这些更新对于我们开发者来说，新增了很多需要适配的工作，但是同时作为用户，这无疑让我们的隐私更加的安全。这一篇文章主要是展示了关于隐私架构相关的概况，如果要了解更多的原理相关内容，可以根据文中链接尽情的阅读吧！保护用户隐私任重道远，让我们一起努力，愿我们的互联网环境的隐私越来越美好。
