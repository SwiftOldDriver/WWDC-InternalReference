...
> 作者：Chensh，iOS 独立开发者。
>
> 审核：黄骋志（橙汁），老司机技术社区核心成员，现于西瓜视频负责稳定性 OOM/Watchdog 相关工作。
...
如果我们是采用断点或者 `sleep()` 来进行阻塞进程，就会发现一个有趣的现象，对于 `ILMessageFilterCapabilitiesQueryRequest` 的问询，基本上容许长时间不响应（小编测试了等待 8 分钟后再返回 `ILMessageFilterCapabilitiesQueryResponse` 对象，结果发现系统依旧进行响应，并且正常更新了信息 App 的界面。如果不返回任何 `SubAction` 类型，那么系统依旧会显示两种父过滤文件夹，但不显示任何子文件夹。

如果是对于 `ILMessageFilterQueryRequest` 的问询，则会有相应的时效性，根据多次测试，发现大概在 `15` 秒左右如果没有得到响应，系统会默认不进行过滤操作，即使后期再返回 `completion(response)`，依旧无法对该条信息进行过滤操作。
