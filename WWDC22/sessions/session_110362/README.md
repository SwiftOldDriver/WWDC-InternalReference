...
第一个行为是：当你的 App 链接一个静态库中的代码时，可能会发现某些代码并没有出现在最终的 App 二进制中 (例如，一些标记了 `__attribute__((used))` 的函数，或者 Objective-C Category)。这是由于链接器的选择性加载行为导致的，如果静态库中的那些对象文件没有定义链接期间需要的某些符号，那么链接器就不会加载那些对象文件（注：Category 不是通过符号被解析加载的，而是在运行时通过元信息加载，所以得通过「-ObjC」这个链接器参数来强制加载所有 Category 实现。但是这也带来一些副作用，比如有一个库给 Foundation 的类添加了很多 Category 方法，那么在开启「-ObjC」后即使没有调用这些 Category 方法，它们也会被链接进最终的二进制）。
