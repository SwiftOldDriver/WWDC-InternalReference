...
当应用请求需要经过多次往返才能获得响应的时候，因积压导致的网络延迟问题会被放大。举例来说，一个常见的 https 请求需要经过 4 次往返才能获得响应（tcp 1 次，tls 2 次，http 1 次，如下图），而且每次往返都会受到积压队列的影响，这个响应的总时间最终变得非常长。
...
L4S 是苹果和网络社区一起提出的一种新技术，它本质上是一种全新的网络架构，核心思想是希望通过更好的队列控制算法来实现更低的网络延迟、更少的丢包和对吞吐量的弹性控制。L4S 没有采用丢弃数据包的策略（这种策略会带来重传，从而增加 RTT 的次数）来解决拥塞，而是使用基于网络拥塞反馈自适应的发送速率来达到维持较少的排队的效果。目前我们可以在 iOS 16 和 macOS Ventura 中使用 L4S（还处于 beta 阶段）。
