...
某行的 tag 是一个与该行视图相关联的哈希值（Hashable value）。一个行可选中的 `List` 利用这个值来定位某行，并标记其是否被选中。大多数情况下，SwiftUI 可以帮助我们为每行自动生成 tag。这些 tag 有点像行的标识符（identifier），但并不完全一样。当使用 `ForEach` 时，SwiftUI 自动从视图的标识（identify）中取得这个 tag，而在选中某行时，SwiftUI 使用 `TableColumn` 的 `RowValue` 作为 tag。更多详情，请移步 [WWDC 21 Demystify SwiftUI](https://developer.apple.com/videos/play/wwdc2021/10022/)。
