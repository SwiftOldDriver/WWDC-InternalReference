...
Swift Charts 是基于 SwiftUI 的图表框架，同样提供一种简洁的声明式的 API 来实现各种样式的图表，用法甚至比开源的 Charts 还要精简许多。并且 Swift Charts 有对辅助功能 Accessibility 提供丰富的支持，包括读出图表的内容，语音播报数据值，这些能力是 danielgindi/Charts 不具有的，这也是 Swift Charts 的一大优势。

如下图所示，从简单的柱状图与饼图到复杂的向量图和热力图，Swift Chart 都可以通过少量的代码来实现：
...
实现以上代码后，大家可以发现，柱状图的大小、标注标签、坐标的尺度、乃至图表的颜色都自动实现了，Swift Charts API 的一个关键优势就在这里，能够基于 Mark 数据的内容和 SwiftUI 上下文，自动化协调图表中的各种因素，以更美观的方式呈现。Swift Charts 还自动支持 Accessibility 的 VoiceOver 等辅助功能；
...
每一个 Mark 都有丰富的 Property 来控制其表现样式。比如我们想要在单个图表中，同时显示两个数据项，只需在 Chart 中插入两个 Mark 即可，直接变更 Mark 类型就可以切换 Mark 样式，请看下面的例子：
...
设计师由于更关于视觉而不是具体实现，从而可以跳脱出工程师的「实现原理限制」，并提出更符合直觉的建议；了解和学习设计师的建议和指南，有助于我们从更全面的视角实现 Charts；
...
当我们使用条形图来呈现月销售数据时，加上数据的描述与结论作为辅助，可以让用户更直观的感受到数据的变化，同时了解数据的概括性结论。
