...
让我们结合一个 Demo 来探究 autoreleasepool 对象堆积这一经典问题。Demo的逻辑很简单，在 viewDidLoad 中增加一个 UIButton，每次点击后执行 doSomeThings 函数，for 循环创建一百万个 NSString 对象，具体代码如下：
...
呢嗯？不是上面说 54 MB么？为什么这边这变成了 280 MB？因为此处显示的是累积创建内存带下，包含了已经释放和没有释放的部分，而前面的 54 MB 的口径是创建了但未释放的部分。重新回到 statistic 类型的统计，按 Total Bytes 进行排序，我们可以看到有四类相关的申请加起来刚好是 280 MB。
...
简单而言，使用 weak 比较安全，但消耗更多内存、访问速度更慢。而使用 unowned 速度更快、不消耗额外内存，但需要注意的是当指向的对象被销毁后，再次访问指正会导致crash问题。
