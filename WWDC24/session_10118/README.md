...
在 iOS 18 之前我们想要使用`Trait System`需要使用相关的监听函数，在 iOS 18 之后使用和处理特征系统变得更加容易，现在 UIKit 支持常见视图和视图控制器的更新方法例如`layoutSubviews`和`drawRect`方法内的自动特征跟踪。在 UIKit 调用这些方法的时候，系统会记录在方法中访问的特征，当这些访问特征值改变时，UIKit 会自动执行方法关联的方法例如`setNeedsLayout`或者`setNeedsDisplay`，从而实现UI刷新。
