...
![voiceOver_icon](./images/voiceOver_icon.png)
...
接下来会通过 3 个例子，讲解如何通过可访问性修饰符来改进应用程序的 VoiceOver 体验。
...
因为 VoiceOver 不支持悬浮弹框等交互方式，对于需要多次交互的场景，需要提供更简单的方法来呈现可访问性信息。

当视图出现悬停、按键、手势等交互方式时，看看如何通过增强功能来提升可访问性体验。
...
但 VoiceOver 必须执行很多步骤才能导航到这个视图：
- 悬停交互必须使用命令触发，并且悬停视图必须被聚焦。
- 当悬停视图出现，其子视图也必须被导航到。
- 交互结束后焦点必须返回到原始视图。
...
应用程序意图（App Intent）允许 Widget 创建像 Buttons 和 Toggles 这样的交互视图，但可能有一些地方可以通过额外的自定义操作来提升 Widget 的体验。下面会在 Widget 中实现一个功能，让用户可以快速评价和发布去过的海滩日记。只需点击一下，用户甚至可以更新我最喜欢的海滩的排名。每个按钮都由 SwiftUI 标记，并且可以由 VoiceOver 激活。

![widget_ui](./images/widget_ui.png)

`accessibilityAction` 修饰符现在可以接受一个应用程序意图，当修饰符被调用时将执行该意图并更新开发者的 Widget。
