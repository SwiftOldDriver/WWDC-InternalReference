...
- 生成逻辑
    ...
    2. 用户点击「继续」之后， iOS 系统会验证用户 FaceID 或 TouchID 的生物验证信息，并返回公钥，私钥则存储在 iCloud KeyChain。
...
- 签名逻辑
    ...
    3. 生物识别验证完成后，App 就会获得私钥的签名信息，该公私钥验证签名的算法是 ES256  secp256r1 曲线
...
### 目前现行 iOS 17 的 Passkey 具体的创建与登录代码
...
3. ASAuthorizationControllerDelegate 回调处理
...
当今互联网环境中，钓鱼攻击和其他形式的机密信息被盗是目前最普遍的帐户丢失原因。要想预防钓鱼攻击，根本性的方法是移除所有潜在能被钓鱼的登录因素，本着这种思路，最佳的帐户安全模式是，在用户创建帐户时，就给用户设置好 Passkey 登录。

Passkey 既不能被忘记，也一般无需重置，假设一个帐户创建时原生就没有密码，只有 Passkey，那么这种帐户可以有效防范钓鱼攻击。

但现今的大部分已存在帐户往往基于可被钓鱼攻击的秘钥因子，例如密码、短信验证码、邮箱验证码、App 推送验证码、2FA 验证码，将这些因子组合起来，使用多因子验证，可以提高帐户的防钓鱼安全性，但仍然无法根本性解决面对钓鱼攻击的脆弱性问题，因为帐户的整体安全性取决于帐户最脆弱的因子。

目前整个互联网行业正处在，从现行帐户安全因子（密码与 2FA 等）往新的 Passkey 架构上迁移的过程中。从所有秘钥因子都有被钓鱼风险，逐步转换为部分秘钥因子有钓鱼风险（Passkey 混合现有登陆方式），最终移除所有有风险秘钥因子后，只保留诸如 Passkey 之类的无风险秘钥因子，此时的帐户登录更安全，且完全没有钓鱼风险。
...
有了这些静默创建 API 可以加速 Passkey 的普及并升级用户帐户的安全性，与此同时在用户体验层面 Passkey 所提供的单次点击单步骤登录，也会提高用户的满意度。

上文提到过，要想最终消除帐户登录被钓鱼和泄露攻击的不安全性，需要完全移除不安全的登录因子，如静态密码。因此当用户升级至 Passkey 之后，开发者可以考虑关闭单纯静态的密码登录，只保留多因子验证登录，甚至完全移除静态密码因子，如静态密码。当所有不安全因子都被移除，帐户安全就会被实质性的提高，从根本上抵御钓鱼和泄露的攻击。但这个过程需要开发者进行深思熟虑，结合自身的帐户登录环境，建议是如果所有渠道的登录都可以基于 Passkey 进行，那么就可以考虑采取激进的升级策略。
