...
> 摘要：本文先介绍了 SwiftUI 实现过渡效果的两种方式 AnyTransition 和 Transition。逐步剖析了自定义 SwiftUI 过渡效果的步骤和注意事项。最后通过多个示例展示了视觉、着色器、颜色、文字、滚动和导航等不同类型过渡效果的应用，以启发读者创建令人印象深刻的过渡效果。
...
过渡效果，又叫过渡动画，是指在用户界面中，视图元素从一种状态转换到另一种状态时所采用的动画效果。它能够在视图被插入（显示）或移除（隐藏）时，为用户提供视觉上的平滑过渡，从而使界面更加动态和吸引人。过渡效果的主要目的是提升用户体验，通过更自然的视觉变化让用户更轻松地理解界面状态的变化。
...
具体到 SwiftUI，过渡效果处理视图在添加到和移除出视图层次结构时所产生的变化。当视图从无到有，或者从有到无的状态之间变化时，SwiftUI 会插值中间的状态，这和动画机制类似。过渡效果可以看成是一种特殊的动画。SwiftUI 提供了以下三种方式来实现过渡效果：
...
AnyTransition 是 SwiftUI 中的一个类型擦除结构体，它使我们能够定义视图在出现和消失时的过渡效果。借助 AnyTransition，我们不仅可以灵活地创建各种自定义过渡效果，还能方便地使用 SwiftUI 提供的内置过渡效果，快速实现丰富的视觉变化。

为了将 AnyTransition 定义的过渡效果应用到视图上，我们需要使用 `.transition` 修饰符。然而，仅仅使用 `.transition` 修饰符并不足以实现过渡效果，还必须与 `.animation` 修饰符配合使用，才能让过渡效果在视图状态变化时生效。

要使过渡效果生效，需要按照以下三步操作：

1. 使用 `.transition` 指定一种过渡效果。
2. 确保过渡视图的状态从无到有，或从有到无发生变化。
3. 关联一个动画来驱动过渡效果的执行。
...
默认情况下，过渡效果在视图添加到层次结构时以一种方式应用，而在视图移除时则产生相反的效果。例如，`.slide` 在视图添加时会从一侧滑动到屏幕内，而在视图移除时会从屏幕内滑动到相反一侧，这是一种对称的效果。然而，这种默认行为是可以改变的。

如果想为视图的添加和移除指定不同的过渡效果，可以使用 `.asymmetric` 修饰符。例如，可以在视图添加时使用滑动效果，而在移除时使用淡出效果，如下所示：
...
AnyTransition 允许开发者将多个过渡效果组合在一起，以实现更复杂的动画效果。这可以通过使用 `.combined(with:)` 方法来实现。例如，通过将透明度变化和滑动过渡效果组合，您可以得到更加丰富的动画效果：
...
甚至，您可以通过将多个不同的效果叠加，形成更复杂的过渡动画。例如，将非对称的插入滑动效果、移除淡出效果，结合滑动和缩放效果：
...
由于 AnyTransition 是一个结构体，并且 `customSlide` 不需要参数，所以给它扩展了一个计算属性。然而，由于我们无法直接获取当前视图的大小，使用 `UIScreen.main.bounds.width` 来移动，效果和默认的 `slide` 有出入。为了解决这个问题，可以将当前视图的大小传递给 `customSlide`，这时候就需要扩展一个函数了，如下所示：
...
这样很不方便，而且在第一帧时，由于 size 默认为零，视图会静止在屏幕中间不动。幸运的是，从 iOS 17.0+ 起，SwiftUI 提供了一个叫 `visualEffect(_:)` 的修饰符，将视图的布局信息暴露出来的同时，还能给视图添加视觉效果。

接下来我们了解一下 `visualEffect(_:)` API，并说明如何应用它实现自定义的。
...
`.visualEffect` 中的视图同时拥有当前视图的几何信息和过渡阶段，非常实用。不过需要注意的是 `visualEffect` 所接受闭包的返回值类型是 `some VisualEffect` 而不是 `some View`。这意味着在 `.visualEffect` 中，并不能使用所有 View 上的修饰符。那么，可用的修饰符有哪些呢？

#### visualEffect 上可用的修饰符

在 SwiftUI 中，您可以使用一些特殊的视觉效果修饰符来改变视图的视觉效果，而不改变其父视图或子视图的布局特性。这些修饰符主要分为以下几类：
...
使用 `.visualEffect`, 可以从 `GeometryProxy` 获取当前视图的大小，从而避免了在 `CustomSlideTransition` 中传递大小参数的麻烦，代码如下：
...
这时，我们自定义的 `.myCustomSlide` 和系统默认的 `.slide` 用法和效果都一样了。
...
学过 C/C++ 的话，写这种函数没有任何难度，当然也可以阅读 [Metal Shading Language Specification][Metal-Shading-Language-Specification]。接着在 SwiftUI 里调用它, 你就能得到一张灰度图：
...
学习 API 最简单的方式就是复刻系统的 API。如果我们能做一个一模一样的实现，说明理解的差不多了。当然，如果做完之后能对比一下系统 API 是如何实现的会更有益，但对于闭源的 SwiftUI 来说，目前还无法做到这一点。

接下来，我们选择一个最简单的 `opacity` 来复刻。在 SwiftUI 端的实现非常直接：
...
和 `.layerEffect` 的区别是，原先需要采样的地方，现在直接返回了位置坐标。效果也是一模一样的。
...
合理地利用颜色，可以实现基于颜色 API 的过渡效果。实际上，`.opacity` 也是颜色过渡的一种表现形式，它通过调整透明度来产生过渡效果。iOS 18.0 新引入了一种网格渐变工具，这个工具通过设置网格的关键点和对应颜色，能够实现丰富的渐变效果。接下来，我们探讨如何将这种网格渐变工具应用于过渡效果中：
...
iOS 18.0 引入了一个新的 API —— TextRenderer。TextRenderer 是一个功能强大的新协议，允许开发者替换默认的文本绘制方式，从而实现全新的文本自定义绘制效果。虽然它提供了广泛的可能性，但我最感兴趣的是如何使用它来实现过渡效果。

### TextRenderer API

TextRenderer 协议的核心是 `.draw(layout:in:)` 方法。它的参数分别是 `Text.Layout` 和 `GraphicsContext`。

`Text.Layout` 允许我们访问文本的各个组成部分，包括行 (`Line`)、连续单元（`Ru`n）和最小单元 (`RunSlice`)。最小单元通常指字、字符和图片等。具有相同类型和属性的最小单元连在一起构成一个连续单元 (`Run`)。例如：
...
开关 `toggle`，你会发现期待的过渡效果并没有如期而至。这是因为 `KeyboardEffectRenderer` 不是可动画属性，它不知道要随着 SwiftUI 插值而重绘，给它实现 `Animatable` 协议，就解决问题了：
...
受限于移动设备的屏幕尺寸，滚动视图被广泛使用，以便显示更多的内容。近两年，SwiftUI 推出了一系列新的滚动相关的 API，极大地扩展了滚动视图的自定义能力。这里主要介绍一个与过渡有关的 API。
...
本文深入探讨了 SwiftUI 中实现过渡效果的多种方法，从基础的 AnyTransition 和 Transition 开始，到高级的自定义效果。通过详细解读视觉过渡、着色器过渡、颜色过渡、文字过渡、滚动过渡和导航过渡等多种类型的过渡效果，并结合实际示例展示了每种效果的实现过程和应用场景。笔者相信还有诸多 API，比如 Geometry、Shape、Canvas 等也可以用于过渡效果，期待你的创造。
