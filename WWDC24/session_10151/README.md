...
`Text.Layout` 允许我们访问文本的各个组成部分，包括行 (`Line`)、连续单元（`Run`）和最小单元 (`RunSlice`)。最小单元通常指字、字符和图片等具有相同类型和属性的最小单元连在一起构成一个连续单元 (`Run`)。例如：
...
我要衷心感谢本文的审核者 Jake Lin，他的宝贵建议和修改对于这篇文章的质量提升起到了关键作用。
