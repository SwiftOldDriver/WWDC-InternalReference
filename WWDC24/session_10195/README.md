...
我们稍后会详细展开这些特性，但在此之前，我会从 0 开始介绍如何使用 Swift Testing 编写测试代码，包括框架提供的 API 以及代码案例。如果你对 XCTest（Apple 旧的测试框架）比较熟悉，我还准备了一个 Swift Testing 和 XCTest 的对比，帮助大家快速了解两者的差异。接下来我会就上面提到的几个特性展开解读。最后我会就 Unit Test 以及 Swift Testing 做一些额外的拓展思考。
...
下面展示的代码是一个最简单的单元测试 demo 。我们就以此来介绍 Swift Testing 中的 API。
...
> 此处被 `#` 标记的就是我们前面提到的独立宏（freestanding macro）。这两种宏都是 Swift 5.9 引入的新特性，它们减少了重复的模版代码，也减少了新关键字的需求。这里推荐大家阅读我们去年的讲座[初见 Swift 宏](https://mp.weixin.qq.com/s?__biz=MzI2NTAxMzg2MA==&mid=2247501904&idx=1&sn=8d07cffb151db8540921bf9ccc4e355c&chksm=eaa1575bddd6de4d62d466e3b793aa4af00b28ccca71a81317ff36d572a8f8aa8f4b750d416b&token=1279939580&lang=zh_CN#rd)来进一步了解 Swift 宏。
...
> Test 支持多个条件判断意味着一个方法可能同时被标记为 `enable` 和 `disabled`，那么如果这样，方法到底会不会执行？经过测试，我们发现 `disabled` 的优先级要高于 `enable` ，意味着只要 `disabled` 生效，那么即便也标记了 `enable` ，方法也一定不会执行。
...
Tag 和 Suite 都是用来对测试代码进行整理归类的，但是 Suite 因为和测试类强绑定，所以和测试方法有上下级的关系。而 Tag 则更加分散，也更语义化，可以跨 Suite、文件甚至测试目标，与任意数量的其他测试共享。
...
前面提到，Swift Testing 默认会将所有测试方法并行执行，这与 XCTest 不同。虽然框架提供了 `serialized` 参数可以禁用并行执行，但是我还是希望大家尽量不使用它，而选择让方法能够在并行环境中正确执行（比如使用最新的 `@MainActor`）。并行执行不仅会提高测试的执行速度，也能测试代码的健壮性，比如能够暴露出更多的问题（如多线程）。
...
> 正是因为 XCTestCase 的 API 实在是太反人类了，因此开发社区才会在 XCTestCase 的基础上开发了非常多的第三方框架（例如 [expecta](https://github.com/specta/expecta) 、[Quick](https://github.com/quick/quick) 等）。使用第三方框架几乎成了单元测试的标配。
...
> 我们看到了社区中已经出现了类似的框架（[package-benchmark](https://www.swift.org/blog/benchmarks/)）. 这个框架目前被官方推荐，有希望成为官方的性能测试框架。
