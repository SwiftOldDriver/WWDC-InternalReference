...
- Edit
  ...
  - 实时预览优化
    - 使用 `@Previewable` 简化了依赖 `@Binding` 的 `View` 的预览
    - 使用 `PreviewModifier` 简化了为预览提供数据的流程
    - 预览的响应速度更快
- Build
...
等不及的同学们可以参考 [Enable XcodeLLM for ChinaSKU Mac on macOS 15 Beta 1](https://gist.github.com/Kyle-Ye/4ad1aa92df3a31bd812487af65e16947) 文中的方法进行尝试，需要非常注意的是，该方法需要关闭 macOS 的系统完整性保护 (SIP)，**请谨慎操作**。
...
`PreviewTrait` 是和 `#Preview` 宏一起，跟随去年的 Xcode 15 一起发布的新特性，用于在预览时进行一些自定义行为，除了上面演示的包装 `PreviewModifier` 的用法以外，系统还提供了一系列和布局相关的 `Trait`，例如使用 `.landscapeLeft` 将刚刚编写的代码在横屏设备上预览：

``` swift
#Preview(traits: .sampleNamer, .landscapeLeft) {
    RobotNameSelectorView()
}
```

关于 `PreviewTrait` 的更多信息可以查看官方文档 [PreviewTrait](https://developer.apple.com/documentation/developertoolssupport/previewtrait)。

通过使用 `PreviewModifier` 为预览提供数据时，不仅可以减少重复代码，还可以使不同的预览重用这些数据。特别的，`PreviewModifier` 可以跟 `SwiftData` 非常好的结合：
...
3. 调试器的响应速度得到了极大的提升：之前我们在调试时执行 `p`/`po`/`expr` 命令，调试器会对依赖的模块进行编译。现在调试器可以复用编译器编译好的模块缓存，无需再次编译，从而提升调试器的响应速度。
...
当**编译目标为 macOS Sequoia 或者 iOS 18 时**，默认会使用 DWARF 5 格式，这使得调试符号信息文件更小，符号查找速度更快。

![配图](./images/debug_minimum_deployments_ios.png)

经过测试若使用 Xcode 16 构建的项目需要支持更低版本的 iOS 时，可以通过给 `Other C Flags` 增加 `-gdwarf-5` 开启 DWARF 5 格式。不过苹果并未在文档中提及该方法，不确定是否有兼容性问题以及其他副作用，还是建议优先遵循 Xcode 的默认设置（该编译选项在 Xcode 15 中不起作用）。
