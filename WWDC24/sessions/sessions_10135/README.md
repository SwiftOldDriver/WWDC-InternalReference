...
> 非常感谢卓立补充：其实 Swift 很早就可以支持 DWARF 5 了，只不过 Clang 没有兼容。之所以现在才完整支持，主要是因为工具存在向后兼容问题，DWARF 5 不能被老版本的 dsymutil/llvm-nm 等工具良好消费，因此存在推广时间差。另外，Swift 编译器曾经编写的一些自定义字段也需要迁移到标准的 DWARF，也花费了一些时间。
