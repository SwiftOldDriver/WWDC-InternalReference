...
![SwiftUI](/Users/wbear/Desktop/WWDC/WWDC24/session_10144/images/SwiftUI.png)
...
虽然 SwiftUI 目前能满足大部分页面开发需求，但总有一些复杂或者特殊的功能是 SwiftIUI 目前难以实现的，这个时候我们通常会用 `UIViewRepresentable` 来桥接 UIView 和 SwiftUI，让我们可以直接在 SwiftUI 中使用 UIView 来完成这些页面功能。在 iOS18 中，苹果同时也带来了 SwiftUI 与 UIKit 之间全新的交互，进一步提升框架之间协作度和联动性。



### UIKit 与 AppKit 可以使用 SwiftUI 的动画效果了

在SwiftUI中，我们可以通过非常简单的API来实现一些复杂的动画效果。但是在 UIKit 中，我们想要实现一些复杂的动画则是相对麻烦的一件事情，我们不仅要考虑代码的编写，同时还要配合约束和视图的刷新时机等。
...
在今年 iOS 18中，新增 `UIGestureRecognizerRepresentable` 协议，使得我们可以单独的对手势进行桥接了，这样我们就只需对手势进行扩充，而不用对整个页面进行桥接了。其使用方法与已有的 `UIViewRepresentable`  协议内容类似，在 `makeUIGestureRecognizer` 方法中创建我们想用的手势，在 `updateUIGestureRecognizer` 中处理事件。
...
因为 SwiftUI 是未来苹果主推的跨平台 UI 框架，所以每年都会加快更新进度来追赶 UIKit 的进度。今年在 iOS 上最重要的更新是 ScrollView优化，让我们自己终于可以自己监听和控制 Scroll View 了。同时新增的 TextRenderer 方法可以让我们更加简单完成富文本的样式。SwiftUI 和 UIKit 的交互更新，使得两边更加相辅相成，使用更加便捷。但是上述功能最低版本仅支持 iOS18，所以要在项目上想要使用可能还需要多等上一段时间。
