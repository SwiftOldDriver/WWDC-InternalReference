...
首先，Metal 编译器将 Metal 源代码转换为Metal IR，这个过程包括词法分析、语法分析和语义分析等步骤。Metal IR 是一种高级表示形式，通常是一种中间抽象语言，用于表示Metal代码的结构、语义和计算逻辑。然后，Metal IR 经过优化和转换，可以生成针对具体 GPU 架构的最终可执行代码。Metal IR 允许编译器进行各种优化，例如指令调度、寄存器分配和内存访问优化等。Metal 编译器可以根据具体的硬件和优化目标，对 Metal IR 进行针对性的优化，以提高性能和效率。
...
- 它支持将 DXIL 转换为 Metal IR。我们可以将它与开源 DXC（DirectX Compiler） 编译器工具一起使用，以构建端到端着色器 pipelines。从 DXIL 转换为 Metal IR 非常快，因为 Metal 着色器转换器在二进制级别执行转换。这会减少着色器资源构建时间。
DXIL（DirectX Intermediate Language）是一种中间语言，用于在 DirectX 编译器中表示和处理着色器程序的中间表示形式。它类似于 Metal IR，在 DirectX 编译流程中起到类似的作用。
...
设置 DXC 和着色器转换器后，首先将 HLSL 着色器编译为 DXIL。DXC 要求您指定要编译的入口点、着色器类型和输出文件。接下来，在刚刚创建的 DXIL 文件上调用着色器转换器，并指定要创建的输出 Metal 库。默认情况下，着色器转换器为最新版本的 macOS 生成一个 Metal 库，以及包含有用反射数据的 JSON 文件。在运行时，将此 Metal 库传递给 Metal 设备以加载它并构建 pipeline state 对象。
...
如果我们的着色器是由一些游戏引擎构建出来的。或者，当我们为 Metal 引导游戏时，我们可能还想在提前将着色器完全转换为 Metal 库之前，看看它们在平台上的工作情况。在这两种情况下，可以使用 Metal 着色器转换器动态库（libmetalirconverter）。它具有与 CLI 工具相同的所有功能。该库提供了纯 C 接口，与 CLI 一样，它在 macOS 和 Windows 上都可用，因此很容易集成到现有的工作流程中。
...
Metal 着色器转换器可以帮助我们恢复此模型，因为 Metal 具有非常灵活的绑定模型。它将这些资源布置到参数缓冲中。在这个模型中，您将一个参数缓冲直接绑定到 pipeline 中，并通过它引用所需要的资源。这个“顶级”参数缓冲有两种布局模式可供选择。
...
由于图形 API 的差异，映射某些着色器阶段可能具有挑战性。例如，您可能有利用传统几何体和镶嵌（geometry）阶段的 pipeline。Metal 是一个现代的 API，它提供了诸如视口 ID 和放大（amplification）等功能，这使得其他图形 API 中较旧、效率较低的阶段变得不必要。然而，当你的游戏依赖于这些 pipeline 来获得增强某些表面的传统效果时，手动转换它们的成本很高。
...
虽然构建这些 mesh pipelines 很简单，但每条 pipeline 都必须按照精确的顺序遵循一系列步骤。着色器转换器运行时可帮助您构建这些复杂的 pipeline。它甚至通过调度网格着色工作来模拟绘制调用。有关详细信息，请参阅 Metal 着色器转换器文档。

### 性能优化

现在，您的着色器已在 Metal 上，并且正在运行 pipeline state，下面是一些提示，可以帮助您获得出色的性能和视觉正确性。
...
3. 利用 GPU 二进制缓存
   ...
   当 Metal 第一次编译 pipeline 对象时，pipeline 对象会被缓存，从而自动减少后续运行游戏时基于编译的挂接。二进制归档（binary archives）也可以在这里帮助你。
...
5. 混合着色器语言
   ...
   本文前面提到，着色器转换器加入 Metal 编译器作为从现有 Metal IR 生成 Metal 库的另一种机制。Metal 使用这些来提供各种图形 pipeline 阶段。由于一切都是 Metal IR，您可以在单个应用程序中甚至在单个 pipeline 中混合和匹配来自 Metal 着色器转换器和 Metal 编译器的 Metal 库。
...
二进制存档（Binary archives）工具允许您控制编译发生的时间。为了提前生成 GPU 二进制文件，您可以提供现有的 Metal 库以及引用这些库的 pipeline 配置脚本。然后通过 metal tt 生成一个带有 GPU 二进制文件的二进制归档。

![metal tt](images/metal_tt.png)


为了开发 pipeline 脚本，您需要生成一个带有类似于 Metal API 的 pipeline配置的 JSON 脚本。此 Metal 代码生成一个渲染 pipeline descriptor，旁边是其 JSON 等效表示。对于 pipeline 脚本，添加 Metal 库路径及其片段和顶点函数名称。您还可以指定任何其他 pipeline state 配置。

![json](images/json.png)

提前着色器编译工作流可能不适合生成 pipeline 脚本文件。但我们可以在设备上运行游戏时录制 Metal 二进制归档。这些归档文件包括相应的 pipeline 脚本。若您从设备中获取这些二进制归档，那个么您就可以使用 “Metal-source” 工具来提取它们的嵌入式 pipeline 脚本。然后在提取的脚本中更新到 Metal 库的路径。
...
您可以使用 “FailOnBinaryArchiveMiss” 选项来测试您的二进制文件归档是否包含我们期望的 pipeline。
...
一旦您的二进制文件归档准备好支持所有目标设备，就可以进行部署了。并非所有玩家都使用最新的操作系统。为了确保所有用户都能从二进制文件归档中受益，请为每个主要操作系统版本生成一个文件归档，并将其存储到您的应用程序中。要做到这一点，请检查玩家设备的操作系统版本，并选择适当的二进制文件归档与 pipeline descriptor 相关联。
