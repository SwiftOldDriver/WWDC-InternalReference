...
SwiftUI 对动画的定义是：通过响应状态的变化来创建流畅的视觉更新（Create smooth visual updates in response to state changes）。即随着状态变化，通过动画使得视图的更新变得更加平滑与流畅。

接下来，我们将按照动画的定义，首先不借助 SwiftUI 的动画机制，实现一个简单的动画效果。具体来说，我们将模拟一个小球沿直线运动的动画。通过使用一个周期为一秒的定时器，并以每秒 30 帧（30 fps）的频率来更新小球的位置。我们将此版本命名为"定时器版本"，以下是相关的代码示例：
...
那么，如何使用 SwiftUI 的动画机制来生成相同的动画效果呢？
...
- 动画的初始状态和结束状态，例如，小球的初始位置 `(x: 0.0, y: 0.0)` 到结束时的位置 `(x: 150.0, y: 0.0)`
...
这三种方式，都需要关联变化的状态，设置一个动画过渡函数来实现动画效果。
...
SwiftUI 内置了三种常用的弹簧动画函数，并提供 `spring` 函数来自定义弹簧函数：
...
所谓高级动画函数是指在其它动画函数的基础上，叠加了延迟、重复、翻转和速度等功能的方法，如下所列：
...
在 SwiftUI 中，Animation 结构决定了数据随时间的变化方式，而另一个结构 Animatable 决定了哪些数据可以参与到动画处理中。
...
在代码中，`selected` 是一个状态（State），而 `position` 是一个属性（Property）（在此使用同名修饰符将视图与属性进行关联）。当状态发生变化引起可动画属性变化时，动画效果就产生了。

在 SwiftUI 中，许多内置的视图修饰符都可以视作可动画属性，能进行动画处理。例如，前文提到的 `position` 修饰符。若想使得自定义视图具备动画能力，可以让其符合 `Animatable` 协议并实现其 `animatableData` 属性，该属性会接收来自 `Animation` 动画过渡函数生成的插值数据，并将该数据传递给视图，从而刷新视图以实现动画效果。值得注意的是，`animatableData` 必须符合 `VectorArithmetic` 协议。
...
`VectorArithmetic` 符合数学课本中对向量的定义，支持向量加法和标量乘法。如果您对向量感到陌生或不太熟悉，请不要灰心。向量本质上只是一系列固定长度的数字列表。在 SwiftUI 动画中，处理向量的主要目的是为了将该列表的长度进行抽象化。例如，`CGFloat` 和 `Double` 是一维向量，而 `CGPoint` 和 `CGSize` 定义了二维向量，`CGRect` 定义了四维向量。通过处理向量，SwiftUI 能够使用统一的实现来处理所有这些类型以及其它更多类型的动画。
...
在 SwiftUI 中，动画过渡函数生成的插值数据将不断调用 `animatableData` 的 `set` 方法。然后，`animatableData` 将插值数据传递给 `position` 属性，当 `position` 属性发生变化时，视图的位置也会随之改变，从而产生动画。
...
然而，请记住，自定义的可动画属性相比于内置的属性来说更耗时，因为它会在每帧动画中运行 `body`。因此，只有在无法通过使用内置的动画属性来实现期望效果时，才应该使用自定义的可动画属性。
...
这样做会引发什么问题呢？在下面的示例中，`BindingAvatarView` 使用 `BindingAvatar`，由于 `selected` 是一个绑定属性，子视图的修改将导致父视图的刷新。尽管在 `BindingAvatarView` 中没有显式添加动画，但位置 `position` 的动画效果仍然会隐式地传递到上层视图中。
...
struct BindingAvatar: View {
  ...
  var body: some View {
    Image(pet.type)
      .animation(.bouncy(duration: 2), body: { // New
        $0.shadow(color: selected ? .blue : .yellow, radius: 32)
      })
      .animation(.bouncy, body: { // New
...
请注意！在目前的 SwiftUI Beta 版本中，某些可动画组件可能存在渲染上的 Bug。尽管通过使用 `dumpTransaction` 打印事务上下文可以看到上下文信息是正确的，但在某些情况下，替换 `scaleEffect` 为 `position` 可能会导致动画效果失控。建议大家进行自行测试。
...
到目前为止，我们介绍的动画都是在初始状态和结束状态之间进行插值处理，即只在两个状态之间插值。然而，现实要复杂得多，许多动画并不只有两个状态。

[WWDC23 10157][10157] 给我们介绍了两个 SwiftUI 新引入的、用于构建复杂的多步骤动画的新工具：

- 阶段动画 (Phase Animation)。
...
接下来，首先我将介绍阶段动画。通过使用阶段动画，SwiftUI 能够自动在一组预先计划的状态之间进行转换，从而形成动画。
...
我们可以使用阶段动画 `.phaseAnimator` 来在同一个 Circle View 里面实现红黄绿三色变换的效果，如下所示:
...
如果我们想独立地对每个属性使用不同的动画过渡函数，那应该怎么办呢？这就是关键帧动画发挥作用的地方。接下来我们看看如何使用关键帧动画来创建更加复杂的动画效果。
...
通过定义单独的动画轨迹，并为每个轨迹指定独立的时间，关键帧允许您同时独立地对多个效果进行动画设置。您可以使用关键帧来驱动 SwiftUI 中的任何修饰符。

在接下来的例子中，我们将使用关键帧来更新 `.scaleEffect` 和 `.offset`，这与之前的阶段动画处理方式相同。首先，我们创建一个新的结构体，其中包含将要独立进行动画处理的属性。值得注意的是，关键帧可以对任何符合 `Animatable` 协议的属性值进行动画处理。在此，所有属性都使用符合 `Animatable` 协议的 `Double` 类型。在动画的执行过程中，SwiftUI 将在每一帧中为您计算此结构体的值，以便您能够更新视图。
...
接下来，我们添加 `keyframeAnimator` 修饰符。这个修饰符类似于之前使用的阶段动画修饰符，但接受关键帧。注意，我们提供了自定义的结构体实例作为初始值，我们定义的关键帧将刷新它的值。然后，我们将它的每个属性应用到视图上。最后，我们开始定义关键帧。代码如下：
...
正如我们之前所提，关键帧使您能够为各种属性构建复杂的动画，并为不同的属性指定各自的关键帧。为了达到这个目的，我们使用 `KeyframeTrack` 来定义每个属性的变化路径，这样在动画过程中，属性就能按照您设定的路径随时间变化。这里，我们为缩放属性添加了关键帧。首先，我们添加了一个 `LinearKeyframe`，该关键帧重复初始的缩放值并保持 0.36 秒。如果您好奇我是如何确定这个 0.36 的，这是通过实验得出的，这也体现了关键帧的重要特点——为了创建适合您应用的动画，可能需要进行一些尝试和调整。利用 Xcode 的预览功能实时微调动画是一个非常好的方法。接下来，我们添加了一个 `SpringKeyframe`，它使用弹簧函数将值拉向目标值，并为其指定了一个持续时间。对于设置了持续时间的弹簧关键帧，这意味着弹簧函数只会在该持续时间内对值进行插值，之后就会开始插值到下一个关键帧。最后，我们添加了另一个弹簧关键帧，将缩放动画返回到 1.0。不同类型的关键帧会使用不同的插值方法，我们已经看到了 `LinearKeyframe` 和 `SpringKeyframe`。实际上，SwiftUI 提供了四种不同类型的关键帧：
...
关键帧动画类似于预设的动画序列。当动画启动时，它将按照预先设定的轨迹进行。这意味着，它们并不能在需要流畅交互性的用户界面环境下，替代普通的 SwiftUI 动画的解决方案。反而，您可以将关键帧动画视作一段可播放的视频剪辑。它们提供了高度的控制，但也伴随着一些权衡。由于您需要精确地指定动画的执行方式，关键帧动画无法像弹簧动画那样优雅地处理重定向，因此通常最好避免在动画执行过程中更改关键帧。

关键帧动画是根据您定义的值进行的，然后您可以将这些值应用到视图上。您可以使用单个关键帧行轨迹来驱动单个修饰符，或者使用不同修饰符的组合，这完全取决于您的需求。由于动画是基于您定义的值进行的，所以更新将在每一帧中发生，因此在将关键帧动画应用于视图时，应避免执行任何耗时的操作。

#### KeyframeTimeline

我们已经见过了 `.keyframeAnimator` 修饰符。在修饰符之外，您可以使用 `KeyframeTimeline` 类型来捕获一组关键帧和轨迹。您可以使用一个初始值和定义动画的关键帧轨迹来初始化这个类型，就像使用视图修饰符一样。

使用 `KeyframeTimeline` 提供的 API，您可以获得持续时间，该持续时间等于最长轨道的持续时间。您可以计算动画范围内任何时间点的值。这使得使用 Swift Charts 可视化关键帧变得容易，之前展示过的曲线可视化就是使用它实现的。这也意味着您可以按照自己的方式使用关键帧定义的曲线，或者将关键帧与其它 API 进行创造性结合，例如使用 `GeometryProxy` 根据滚动位置调整关键帧驱动的效果，或者使用 `TimelineView` 根据时间进行更新。
...
只要有想象力，它就能被用到任何地方。如果您不确定何时使用它，那也没关系，这是一个高级工具，大多数开发人员更喜欢使用视图修饰符。但它作为一个基础模块，我很期待看到您如何创造性地将其整合到您的应用程序中。
...
- 阶段动画用于处理连续的动画，它在每个阶段为每个属性设定离散的值，并使用相同的动画过渡函数将所有属性值过渡到下一个阶段。
- 关键帧动画则用于处理更复杂的动画，具有更高的灵活性。它允许每个属性值独立地在不同时间点使用不同的动画过渡函数进行处理。
- 使用 `KeyframeTimeline` 能计算关键帧动画范围内任何时间点的值。

## 致谢

我要衷心感谢本文的审核者 Jake Lin，他的宝贵意见和建议对于这篇文章的质量提升起到了关键作用。
