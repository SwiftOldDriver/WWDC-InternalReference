...
接下来，根据动画的定义，我们将首先不使用 SwiftUI 的动画机制，来实现一个简单的动画效果。我们将实现一个小球沿直线运动的动画。我们可以使用一个持续一秒钟的定时器，以每 1.0/30 秒（30 fps）的速度来更新小球的位置。我们将称此版本为"定时器版本"，以下是相应的代码示例：
...
我们可以使用阶段动画 `.phaseAnimator` 来实现只有一个灯的、简单的红绿灯效果，如下所示:
...
通过 `.phaseAnimator`， SwiftUI 能够自动在红、黄和绿这三个状态之间进行转换，当状态改变时，颜色变化，形成动画。此处使用 `easeInOut` 来做插值，可以根据不同的阶段来设置不同的动画过渡函数。
...
在接下来的例子中，和之前阶段动画一样，我们使用关键帧来刷新 `.scaleEffect` 和 `.offset`。第一步，我们先创建一个新的结构体，其中包含将要独立进行动画处理的属性。关键帧可以对符合 `Animatable` 协议的任何属性值进行动画处理。请注意，几个属性使用了 "Double"，它符合 `Animatable`。在动画进行中，SwiftUI 将在每一帧为您计算次结构体的值，以便您可以更新视图。
...
接下来，我们添加 `keyframeAnimator` 修饰符。这个修饰符类似于之前使用的阶段动画器，但接受关键帧。注意，我们提供了自定义的结构体实例作为初始值，我们定义的关键帧将刷新它的值。然后，我们将它的每个属性应用到视图上。最后，我们开始定义关键帧。代码如下：
...
正如之前提到的，关键帧允许您为不同的属性构建复杂的动画，为不同的属性指定不同的关键帧。为了实现这一点，使用 `KeyframeTrack` 来定义每个属性的变化轨迹，在动画时，属性按照您提供的轨迹随时间而变化。在这里，我们为缩放属性添加关键帧。首先添加一个线性关键帧 `LinearKeyframe`，重复初始缩放值并保持 0.36 秒。如果您想知道我是如何确定 0.36 的，我是通过实验获得的，这是关键帧的一个重要点。创建适合您应用的动画可能需要一些试验。利用 Xcode 的预览功能实时地微调动画是一个很好的方式。接下来，我们添加一个 `SpringKeyframe`。它使用弹簧函数将值拉向目标值。我们还指定了一个持续时间。对于设置了持续时间的弹簧关键帧，这意味着弹簧函数只会在该持续时间内对值进行插值。之后，将开始插值到下一个关键帧。最后，我将添加另一个弹簧关键帧，将缩放动画返回到 1.0。不同类型的关键帧使用的插值方式不同，我们已经见过了 `LinearKeyframe` 和 `SpringKeyframe`。实际上，SwiftUI 提供了四种不同类型的关键帧:
...
前三个正如其名称所示，使用不同的动画过渡函数来插值，最后一个 `MoveKeyframe` 立即跳转到一个值，无插值。每种类型的关键帧都支持自定义，以提供完全的控制，并且可以在动画中混合和匹配不同类型的关键帧。SwiftUI 自动处理关键帧之间的过渡，以确保动画的连续性。如代码所示，我们使用不同类型的关键帧来为 `verticalOffset` 创建动画轨迹。
...
关键帧对您定义的值进行动画处理，然后您可以将这些值应用到视图上。您可以使用单个关键帧轨道来驱动单个修饰符，或者使用不同修饰符的组合。这都取决于您。由于动画是基于您定义的值进行的，因此更新将在每一帧上发生，因此在将关键帧动画应用于视图时，请避免执行任何耗时的操作。
...
- 阶段动画处理链式动画，在每个阶段给每个属性定义离散的值，所有属性值使用相同的动画过渡函数过渡到下一个阶段。
