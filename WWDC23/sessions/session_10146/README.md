...
Result Builder （结果生成器）SwiftUI 声明式语法使用的重要特性之一。在前几个版本中，Swift 编译器需要花费很长的时间来确定错误，因为类型检查器搜索了大量无效路径。
...
对于这种尴尬的场景，在旧版本的 Swift 中就需要继续增加参数定义，从而兼容 7 个入参的场景。但是 Swift 5.9 将会简化这个流程，官方引入了 **Type Parameter Pack 这个概念**。

```swift
func evaluate<each Result>(_: repeat Request<each Result>) -> (repeat each Result)
```

当引入 Type Parameter Pack 概念之后开发者将如何修改这个场景呢？

1. `<each Result>` - 这里代表创建一个名字叫 `Result` 的 Type Parameter Pack；
...
通过以上定义方式，传入参数没有个数限制，并且可以保证每一个入参都是独立的类型。
...
此处的 `@CaseDetection` 是我们自定义的一个 Attached Macro，这个 Macro 使用 `@` 符号作为语法关键字，与 Property Wrappers 的写法类似。当使用的时候，直接写在声明的 type 或者 class 之前即可。它会根据你的实现来生成代码。

Attached Macro 会根据不同的参数来确定修改目标。在上面这个例子中，我们的目标是 `Path` 这个 enum 类型的每一个 Member ，所以我们用 `@attached(member)` 来标记。Attached Macro 提供了五种修改目标，可以查看以下列表：
...
其次，Member Attribute Macro 会为所有的属性添加 `@ObservationTracked` ，这个 Property Wrapper  会为属性添加 `getter` 和 `setter` 方法。
...
Swift Macro 为 Swift 实现更具可读性的 API 、构建更简洁的实现语法提供了一种新的模式。Macro 可以对于输入类型的检测，生成繁琐的 Swift 代码，并在程序中使用 `@` 或者 `#` 语法快速集成，大大提高了代码可读性。
...
另外，我们在这里使用了很多 C++ 类型，C++ 中的 MasterData 类型使用了引用计数智能指针。通过在 C++ 中注释类型，Swift 编译器可以像任何其他类一样使用该类型，自动管理引用计数。
