...
在 autoscroll 中，```popup.js```只负责两段逻辑，一是当用户点开弹窗时，发送 checkStatus 消息，查询开关的状态，从而设置开关，另一个是当用户点击开关时，发送 setStatus 消息，改变存储的值。
...
#### Extension App 与 backgroud、Extension App 与 popup
...
苹果生态的浏览器插件经历了 Safari Extension，Safari App Extension 到 Safari Web Extension 的发展，其中除了 Safari Extension 已经废弃了，Safari App Extension 和 Safari Web Extension 仍然都苹果生态中的浏览器插件支持的实现方式。除此之外，Safari 还支持 Content Blocker（内容拦截器），这也是一种浏览器插件，但是其功能有限，只能拦截网络请求，但这是 Safari 上绝大多数的广告拦截器的实现方式。
