...
Attached Macro 会根据你的声明作为生成代码的维度。在上面这个例子中，我们的维度是 `Path` 这个 enum 类型的每一个 Member ，所以在使用的时候，起维度表达是 `@attached(member)`。Attached Macro 提供 5 中维度提供给开发者来描述使用，可以看看下面的表格：
