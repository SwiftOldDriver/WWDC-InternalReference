...
跟焦员当然也不是谁都可以直接上岗的，一是需要有专业的理论知识，二是需要熟练的测焦和对焦技术，这些都需要手工完成，只有长期的练习才能做到指哪打哪的跟焦技术。

如今，只需要一台 iPhone 13 以上的设备，打开相机 App，进入电影效果模式，动动手指，就可以轻松使用焦点转移，以电影的方式叙述属于自己的故事。甚至，你不需要干预对焦，完全交给设备，电影效果模式系统会全自动完成对焦和焦点转移，系统会根据入镜的主体自动对焦，人物移动的时候自动跟焦，当人物看向另一个方向时，自动对焦到注视的物体，新人物进入时还会根据谁在说话自动地完成移焦……这一切都是利用机器学习算法，通过通过 A15 芯片和感光元器件的配合完成的。这就是电影效果给我们带来的，一个藏在口袋中的微型电影摄制组，一台漂亮浅景深、焦点自然过渡的相机、一位导演（通过改变焦点来引导观众注意力和叙事），以及一位跟焦员（提前预测关键帧，并在不同焦点之间平滑过渡）。电影效果模式可以在 iPhone 13 系列及以上机型的相机 App 中直接录制，并实时预览。

不仅如此，相比传统电影拍摄需要不断地 NG 甚至补拍来获得理想对焦效果，使用电影效果模式拍摄的视频，还允许在录制后进行无损的编辑。可以在后期自由地修改光圈来调整背景虚化效果（bokeh）。自由地改变对焦的对象，系统将自动完成跟焦，自由地在想要的时间点添加焦点转移，系统将平滑地完成焦点过渡。使用候选的智能检测来改变焦点和叙事。
...
电影效果模式的数据实际上由两个文件，以及文件间的数据流组成。我们可以从 iPhone 把电影效果模式视频 AirDrop 分享到 mac，文件组成更加明显。**注意在分享前，在面板的选项中，勾选“所有照片数据”，否则传过来的只是个渲染后的视频，这个配置在跨设备分享电影效果模式视频都要选上。**
...
- `IMG_1960.MOV`、`IMG_E1960.mov` 是直接可以播放的影片文件；
- AAE 后缀的文件则是 plist 文件，是相册 App 的图像调整文件，保存用户的编辑修改信息。我们在相册 App 中对图片视频的编辑修改信息不会保存到源文件中，而是额外地保存到 AAE 文件中。

![](images/PICSEW_52A1.JPEG)

我们简单把 `IMG_1960.MOV`、`IMG_E1960.mov` 丢到播放器（上图是 IINA 的效果）。可以看到：

- 左：`IMG_1960.MOV` 视频不带景深虚化效果。
- 右：`IMG_E1960.mov` 视频有明显的景深虚化效果。

### Rendered asset

渲染后的 asset。这是一个应用了电影效果模式的普通影片文件，可以作为常规的 QuickTime 影片导出、分享和播放。如上面的 `IMG_E1960.mov`，可以看到画面包含了电影效果模式中的移焦景深效果。
...
>
> 他检查了下队友。他也准备好了。焦点转移到队友身上，强调他也准备好了。
>
...
同样，我们继续拿导出到 mac 的电影效果模式视频文件进行分析。这次我们使用 ffprobe（FFmpeg 的一个命令行工具）分析其轨道和元数据。

首先是 `IMG_E1960.mov`，该影片渲染了电影效果，有明显的景深虚化效果。该文件只包含音频轨和视频轨，可见它只是个普通 Rendered asset，常规的 QuickTime 影片。
...
视频轨道，这是原始采集的 QuickTime 影片，即不包含电影效果的原始影片，如上 `IMG_1960.MOV` 的视频轨。它可以是 HDR/SDR，1080p @ 30 fps，在 iPhone 14 上甚至可以是 4K @ 24/25/30 fps 。这个轨道可以作为普通视频播放，但与渲染后的素材相比，它没有浅景深虚化效果，缺乏美感和叙事能力，没有悬念、喜剧效果，我们像是麻木地旁观几个人在后巷玩游戏。
...
视差是两个不同位置的相机观察同一场景时产生的像素偏移（pixel shift），近距离的物体比远距离的物体偏移更多。你可以自己感受视差，保持一个方向注视物体，使用单个眼睛轮换观察，看看不同距离的物体是如何移动的。
...
视差与深度紧密关联，可以这么说，视差是用于计算得出深度的，但视差得出的深度是相对的，目前只用在拍照和录像，像 AR 场景就需要前置的 TrueDepth 镜头或后置的 LiDAR，或后置单摄 + ML 来获取深度。有了深度信息，渲染景深就有了数据支撑，也是这样，才能实现自然的对焦平面（垂直于相机拍摄方向），而不是简单地抠像做 mask 模糊。所以在电影效果模式中的背景虚化效果与人像模式中的虚化效果基本一致。
...
然后是 Cinematic script，其中包含所有自动场景检测识别。这个场景显示了面部、头部和躯干，如果可能的话，它们通过 ID 进行分组，随着时间的推移它们将链接在一起。该脚本还保存焦点决策（focus decision），决定如何对焦到检测对象中。焦点决策可以在后期进行修改，以改变跟踪的检测对象，从而改变叙事和渲染。
...
AVVideoComposition 是视频帧在时间点上的合成描述，即画面编辑的描述（虽然与 AVComposition 仅一词之差，但表达却是两个不同维度的信息）。使用 AVAsset 创建，一般也使用其 mutable 版本，即 AVMutableVideoComposition。通过指令 `instructions: [AVVideoCompositionInstructionProtocol]` 接收用户数据模型。配置自定义合成器类 `customVideoCompositorClass: AVVideoCompositing.Type?` 可实现自定义的合成器。内置的合成器只能实现 opacity、transform、cropRectangle 的修改，要更多的编辑能力都需要自定义合成器。
...
AVAssetExportSession 用于导出。AVAssetImageGenerator 用于视频抽帧。它们都是使用 AVAsset 创建，也支持配置 `videoComposition: AVVideoComposition`。这意味着，上面用于预览编辑画面的 AVVideoComposition 对象也能复用到导出，所见即所得。
...
首先，我们需要获取一个 Cinematic asset。从照片库选择 Cinematic asset 非常简单，只需要使用照片选择器并设置过滤出电影效果模式的视频。选择器将得到所选文件的本地标识符，可以在请求它之前获取带有关于 Cinematic asset 信息的照片 asset。
...
获取 PHAsset 后，异步请求获得对应的 AVAsset。需要确保设置请求选项以获取原始版本，并允许网络访问，以防 asset 在 iCloud 上。
...
后续的操作都是基于这个 AVAsset。从上面的 `IMG_1960.MOV` 数据分析，我们也得知该视频文件就包含了电影效果模式所需的所有轨道数据。
...
渲染过的 asset 可以使用 AVPlayer 和 AVPlayerItem 作为常规影片进行播放，电影效果模式内的视频轨道也同样适用。但要发挥电影效果模式的威力，我们还需要添加一个自定义视频合成器（video compositor），它可以处理多个轨道、用户修改，并最终调用 Cinematic 渲染器来合成输出。这个自定义合成器也可以用于缩略图生成和导出。
...
2. 用这些属性设置渲染会话。渲染会话使用 GPU，所以还需要提供一个 Metal 命令队列。
...
自定义合成器需要一个 composition，其中包含渲染输出所需要的 Cinematic asset 信息。在典型情况下，需要多个步骤来添加多个轨道，现在有更简便的方式，通过 AVMutableComposition 的 `addTracks(for:preferredStartingTrackID:)` 方法，一步到位直接把所有轨道从 CNAssetInfo 添加到 composition 中。
...
自定义指令描述了如何组合渲染电影效果。要播放视频，还需要一个 video composition（注意不要与上面的 AVMutableComposition 混淆），向其中添加来自 Cinematic composition 的轨道 ID，添加一个自定义合成器，这是渲染器被调用以合成效果的地方，并添加指令。
...
一旦知道如何绘制检测框，实际上使用手势点击点修改脚本非常类似。同样，遍历所有检测，如果点击点在检测矩形内，则获取其相应的检测 ID 并创建一个新的决策。决策强度可以在 UI 中设置。
...
那么更新后的 Cinematic script 又是如何驱动焦点的呢？图中每个方块是对应三个用户决策及其对应的焦点检测轨道，根据时间和距离在二维图中排列焦点轨道。

因为 Cinematic 引擎知道整个更新后的脚本，所以它可以提前进行平滑的跟焦。它看起来像这样，跟焦提前开始，在每个关键帧的开头达到焦点。这非常神奇，它就像一个跟焦员，通过了解设置的标记，可以提前引导注意力并提前变焦。更新后的 Cinematic script 中的帧，可以从中直接读取转移的焦点视差数据。

### 使用焦点视差

修改焦点的帧属性与修改光圈类似，一旦从更新后的脚本中获得了一个脚本帧，就可以直接更新焦点视差帧属性。这将根据更新后的脚本把焦点以平滑过渡的方式（线性插值）传递给渲染器。
...
执行 Demo，并通过点击来修改脚本和焦点。从播放模式切换到编辑模式，来更新 Cinematic script。通过单击以获得弱决策（显示为黄色虚线框），或者双击以获得强决策（显示为黄色实心框）来修改焦点决策。当点击不同的角色时，可以看到焦点和渲染会根据用户输入进行修改。

![](images/session_10137_19.17.JPEG)

### 存取修改

现在完成了编辑，如何保存这些编辑修改呢？Cinematic script 修改可以保存在独立的数据文件中，意味着可以完全不修改源文件，并始终可以恢复为原始状态。因此，向 CNScript 获取 CNScript.Changes 修改对象，修改信息支持使用紧凑二进制表达，这样就可以把修改写入数据文件中了。
...
CNObjectTracker 的使用封装在 SampleObjectTracking 中，跟踪是针对一个时间区间的，同步遍历每一帧来完成。

1. 通过命令队列构建 CNObjectTracker。这也说明 CNObjectTracker 需要使用 Metal 做 GPU 计算。
...
或许我们可以格局大一点，上面的电影效果模式的视频编辑可以使用 Cinematic API 完成了，那电影效果模式的视频能否可以脱离相机 App，使用 AVCapture API 在自己的 App 内独立完成呢？ 我们换个思路，要“拍出”电影效果模式的视频的视频，就是合成 Cinematic asset，导出到文件的过程。回顾下 Cinematic asset 的组成：
...
我们可以使用 AVCaptureVideoDataOutput 获取画面；使用 AVCaptureDepthDataOutput 获取包含视差图的 AVDepthData；但我们却无法获取和构建渲染属性和 Cinematic script。上文也说了，Cinematic asset 的元数据不透明，不了解其中的数据结构，我们也不能直接码子节码进去。所以我们还是无法完整“自己”采集电影效果模式的视频，或许我们可以期待 Cinematic API 或 AVFoundation 的 Capture API 未来哪个版本支持相关轨道数据的采集，我们拭目以待吧。
