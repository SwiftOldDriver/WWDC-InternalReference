...
在此介绍苹果官方开源库 [Swift Distributed Tracing](https://github.com/apple/swift-distributed-tracing) ，该工具可以透明的与现有服务端逻辑共存，主要设计用途是追踪分布式多线程系统性能。Swift 的并发任务树非常适合管理复杂任务层级，`Swift Distributed Tracing` 允许您利用跨平台的任务树的优势来深入追踪性能和任务之间的关系；其具有 `OpenTelemetry` 协议的实现，因此对于现有的 Zipkin 或 Jaeger 等性能追踪解决方案能做到开箱即用。
