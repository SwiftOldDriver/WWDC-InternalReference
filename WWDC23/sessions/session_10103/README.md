...
> 摘要：本文主要是探索 App Intents 在 iOS17 中带来的新特性。包含以下三部分内容：Widgets 交互性和配置的新体验；对开发者体验改进。包含框架支持，以及对静态提取的增强；Shortcuts 与 App Intents 集成的更新。

本文基于 [Session 10103]([SampleSession](https://developer.apple.com/videos/play/wwdc2023/10103/)) 梳理。

> 作者：
>
> 王守楷、刘欢，iOS 开发者。目前任职于京东 App 黄金流程团队。
>
> 审核：
>
...
Swift 编译器会从 App Intents 实现中提取有关代码中可用类型的信息。相关编译器工具路径为 `Xcode.app/Contents/Developer/usr/bin/appintentsmetadataprocessor`。
...
图中 `extract.actionsdata` 文件，是一个 json 格式的内容文件，其内容是与代码中的定义相对应的。`extract.packagedata` 也是 json 文件，定义了需要暴露给 AppIntentsPackage 的相关内容。
