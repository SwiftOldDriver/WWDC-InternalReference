...
在 iOS13 中 `viewIsAppearing`方法就以私有 API 的形式出现，但直到 iOS17 才可以正式使用，如果我们想要去在旧版本中使用 `viewIsAppearing`，需要创建一个 UIViewController 的扩展并将 `viewIsAppearing` API 显式的暴露出来(由于 UIKit 这个部分还是用 OC 来实现的所以还是需要用 OC 来做声明，如果想在 Swift 中使用需要加对应的桥接)。
