...
更加方便的是，如果我们想要设置混合的颜色的透明度，只需要设置要混合颜色的比例不超过 100%，相当于透明度等于 100% - 要混合的比例之和。如上图所示，白色和蓝色的混合比例各自 30%，最后得到的颜色就会拥有 40% 的透明度。
...
`text-box-trim` 在某种程度上和我们前文提到的 `margin-trim` 类似，都可以帮助我们消除不需要的间距。不过 `text-box-trim` 专注于消除文字容器中的多余 的上下间距。
