...
DocC 也可以导出为静态网络托管解决方案，允许直接发布到 GitHub Pages和 Netlify 等服务。同时生成的文档也将通过 Index 系统会出现在 Xcode 的内置文档窗口中，并集成到了 Xcode 的源代码编辑器中。
...
同时在 Xcode 15 中通过和 DocC 的紧密结合，提供了文档预览编辑器。可以和 SwiftUI 类似，在编写文档的工程中提供实时的渲染视图。

![Preview](images/preview.png)

> 该 Feature 在去年8月的 SDWG 会议上进行了讨论，想法是提供一个和 SwiftUI Preview 类似的能力
> 
> 最终在今年的 Xcode 15 上完成了实现，但是其他 IDE 如 VSCode 也是可以通过目前的接口支持 Live Preview 的
> 
> https://forums.swift.org/t/documentation-workgroup-meeting-notes-august-1st-2022/59432

除此之外，借助全新的创作功能，如基于网格的布局、视频支持、自定义页面图标，甚至完全自定义的主题，我们可以制作真正体现框架特色的定制文档。

> 本文将重点讨论 Swift-DocC 的一些高级的功能，如果此前对 DocC 不太了解，建议可以看往期内参专题和相关 WWDC 视频：
> - WWDC21内参: [DocC: 未曾设想的苹果文档](https://xiaozhuanlan.com/topic/0483621759)
> - WWDC22内参: [Swift-DocC 新特性](https://xiaozhuanlan.com/topic/3658492071)
> - WWDC21: [Meet DocC documentation in Xcode](https://developer.apple.com/videos/play/wwdc2021/10166/)

## 文档编写

这里以 SlothCreator 这个在前几年的 Session 中出现的示例项目为例，来演示如何使用 DocC 编写文档。

SlothCreator 是一个并用于创建管理树懒的软件包。

这里首先使用Xcode 15中打开该软件包，通过 `Build for Documentation` 选项我们可以评估当前的文档状态。

Swift-DocC 会自动为项目中的相关 API 创建页面，同时也包括前面提到的代码注释，在没有进行过多手动适配的情况下，DocC也是开箱即用的，会提供一个基本的文档页面。

![Basic Documentation](images/basic-documentation.png)

![Basic Documentation2](images/basic-documentation2.png)
...
![Extended Documentation](images/extended-documentation.png)

> 相关的社区提案原文
> 
> https://forums.swift.org/t/document-extensions-to-external-types-using-docc/56060
>
...
使用 Xcode 15的文档预览编辑器，我们将对它添加一些额外的文档。

打开对应的 Swift 文件并开启文档预览（编辑器右上角开启 Assistance 模式，并选择 Documentation Preview）

![Assistance](images/assistance.png)

![Assistance2](images/assistance2.png)
...
> DocC 会在文档目录中寻找到最合适的图片变种，比如 2x和3x图片、浅色模式图片和深色模式图片等
> 
> 我们只需要按照对应的格式命名（eg. `iceSloth~dark@2x`）并在文档注释中使用原名称引用即可

![Documentation Preview](images/extend_preview.png)

### Swift-DocC Directive

Swift-DocC 的语法基于 Github Flavored Markdown，但是也提供了一些额外的指令来支持更多的功能。（如 `Image` 和 `Video` 指令分别提供了对图片和视频的支持。）

在最初版本的 DocC 中指令只在 Tutorial 格式中可用， 在后续的迭代中我们可以在任何地方使用 DocC 指令。

我们将使用其中一些指令来润色 SlothyCreator 的示例代码文章，但请记住这些指令旨在创造性地使用，没有一种绝对正确的方法来使用它们。
...
4. 页面底部用3张截图展示了不同语言的示例，但是它们之间没有关联且占用了太多空间。

![issues](images/issues.png)

对于问题1和问题2，我们使用 `Metadata` 指令来自定义改页面的元数据，其中包括页面的类型、页面行动和页面图片。
...
对于问题3，我们使用 `Row` 和 `Column` 指令来重新组织页面内容，可以通过 `size` 参数来覆盖默认大小，让我们更好地控制页面布局。
...
对于问题4，我们使用 `TabNavigator` 指令来重新组织页面内容
...
最后我们希望消费该文档的受众能够更容易地发现它，所以我们回到 SlothCreator 文档的顶级页面，通过 `Links` 指令并定义视觉参数来指向它们。
...
- [Swift-DocC Project Doc](https://www.swift.org/documentation/docc)
- [Apple's Swift-DocC Doc](https://developer.apple.com/documentation/swift_docc)

> Tips:
> 
> 二者目前并无差别，前者通过 Github Action 部署在 Swift 开源项目上
> 
> 后者为 Apple 的私有部署并使用了 Apple 风格的主题
...
2. Swift-DocC 主题是针对网页部署的，在 Xcode 中会被忽略生效。
   如果确实希望自定义同时出现在Xcode和Web中，最好使用指令来完成。

Swift-DocC 中的主题由具有特定名称 "theme-settings.json" 文件定义，我们可以将其放入文档目录中来生效。

![theme settings](images/theme-settings.png)

对于 SlothCreator，我们会自定义它的颜色和字体来匹配对应整体网站的设计。这些定制只是Swift-DocC主题的开始，如果有兴趣更进一步，可以阅读Swift-DocC的相关文档了解。
...
![Quick Nav](images/quick_nav.png)

> Swift-DocC Quick Navigation 也是一个完全社区驱动的成果
>
> 感兴趣的同学可以在这里了解更多的上下文 https://forums.swift.org/t/quick-navigation-in-docc-render/55942
...
> Tips:
>
> 补充一个和本文无关的小提醒，在本文创作过程中的 macOS 13 beta 2 上，打开 Xcode 文档浏览器在触发滑动后会必现崩溃
> 
