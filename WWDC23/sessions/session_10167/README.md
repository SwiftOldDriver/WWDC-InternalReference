...
Swift 宏是 Swift 5.9 中引入的新特性，希望开发者可以用更简单的方式减少模版代码的编写，让源码保持简洁。开发期间依赖 SwiftSyntax 将源码解析成原型树，供开发者编写宏及其实现来完成特定功能，编译期间 Swift 编译器（Swift Compiler）会将源码中的宏展开成目标代码，替换原本的宏完成编译。

光看上面的描述是不是两眼一黑，别急，随着文章的深入上面的内容会慢慢向你揭示。

> 虽然原 session 中详细提到了如何实现一个宏，以及错误诊断、测试方面的内容，但限于篇幅，本文不会涉及这些，而将偏重于介绍 Swift 宏的基础概念。如果你希望了解这方面的内容，可以阅读内参的这篇文章（链接待补充）。
...
下面我们来看一下用 SwiftSyntax 解析生成 AST 的例子。首先编写一个仅声明了 Suit 枚举的文件：
...
SwiftSyntax 每个节点的具体含义不是本文重点，想要了解更多细节，推荐日本 iOS 开发者 Kishikawa Katsumi 开发的 [Swift AST Explorer](https://swift-ast-explorer.com)，它可以实时解析你的源码并生成 AST。NSHipster 的文章 [SwiftSyntax](https://nshipster.com/swiftsyntax/) 虽然有些年头了，但也推荐阅读。
...
根据不同的使用场景苹果定义了独立宏和关联宏，而在这两种类型宏的背后，苹果设计了角色（role）的概念，进一步明确不同宏的使用范围、代码展开的形式，以及宏展开后是如何嵌入到源码中的。角色的声明以「@」开头，独立宏和关联宏都有各自对应的角色。
...
展开宏的实现，可以看到展开后的代码中竟然自动引用了我们传入的 downloadedImage 作为变量名：
...
虽然上面代码优化了字典参数的写法，但也引入了另外一个问题：所有参数前面必须加上 `@DictionaryStorage` 模板代码。要解决这个问题，需要引入下面这个新角色 `@attached(memberAttribute)`。

### @attached(memberAttribute)

这个角色可以为类型或扩展添加属性。要解决上面模板代码的问题，我们无需新建一个宏，可以直接在上面宏的基础上做改动：
...
`@attached(member)` 的例子里我们用到了 names 参数。对于需要在宏之外访问这些声明的情况，就需要使用到名称标识符了，目前有 5 种名称标识符：
...
注意，这里说的实现使用的是另一个宏，它可以是你自己声明实现的宏，大部分时候使用的是系统提供的 `#externalMacro` 指向真正的实现。
...
当编译器发现代码中使用了宏时，便会单独开启一个进程启动编译器插件，来获取宏对应的展开代码，而 `#externalMacro` 正是用来关联宏和实现的系统宏，通过 module 参数告诉编译器需要运行哪个插件，通过 type 告诉编译器具体的实现类。插件运行完返回结果后，编译器会把结果替换原来宏所在的位置，并源码一起进行编译。
