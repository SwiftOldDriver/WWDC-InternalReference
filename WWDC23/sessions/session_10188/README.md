...
当构建新应用程序时，用户总是期望他们的数据能够同步。他们在 iPhone 上创建了一些东西，当他们打开 Mac 时，他们也期望它在那里。从外部看，这似乎是魔法。他们的数据在一个地方，然后就到处都有了。但对于我们来说，情况并没有那么简单。
...
但是，如果您认为 `CKSyncEngine` 中的新功能可以满足您的需求，欢饮提交反馈与建议。毕竟，同步引擎的一些最佳创意来自像您这样优秀的开发人员。

### 1. 同步引擎的工作原理

![同步引擎的工作原理](images/同步引擎的工作原理-01.png)
...
首先，将待 `处理 RecordZone 更改` 和 `待处理 Database 更改` 添加到同步引擎中。这将提醒同步引擎应该安排一次同步。同步引擎将确保一致性并去重这些更改。
